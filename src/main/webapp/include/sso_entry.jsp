...
<%@page import="kr.co.gocle.oklms.commbiz.login.service.CommBizLoginService"%>
...
%>
<script>
	window.location.href="/commbiz/login/logout.do";
</script>
<%
		    ...
		    paramMap.put(RELAY_STATE_NAME, "/lu/login/goLmsUserLogin.do");
		...
		String sesssionCheckYn = (String) session.getAttribute("SSO_SESSION_CHECK_YN");
		String notUserYn = StringUtils.defaultString((String) session.getAttribute("NOT_USER_YN"), "");
	
		if(!notUserYn.equals("Y")){
			if( !"".equals(memId) && "Y".equals(ssoUseYn) ){
				response.sendRedirect("/commbiz/login/ssoLoginProc.do");
			} else {
				response.sendRedirect("/lu/login/goLmsUserLogin.do");
			}
		}
