...
<!--<title>파일목록</title> -->
	...
	<table>
		<tr>
		<td style="border:0px solid #eb5c01; text-align: left;">
		<c:forEach var="fileVO" items="${fileList}" varStatus="status">
		
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					<c:out value="${fileVO.orignlFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
					<img style="cursor:pointer;" src="<c:url value='/images/egovframework/com/cmm/fms/icon/bu5_close.gif' />" 
				...
				<c:otherwise>
					...
					<img src="/images/oklms/std/board/file_btn_01.png" align="bottom" alt="첨부파일." />&nbsp;<c:out value="${fileVO.orignlFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]<br/> 
					</a><br/>
				</c:otherwise>
			</c:choose>
		</c:forEach>
