...
<script type="text/javascript">
	...
	/* 화면이 로드된후 처리 초기화 */
	function initHtml() {
		$("#email1").val('${fn:substringBefore(memberVO.email, "@")}');
		$("#email2").val('${fn:substringAfter(memberVO.email, "@")}');
	...
	function fn_formSave(){
		...
		if($("#memPasswordEncript").val()){
			if($("#memPasswordEncript").val() != $("#memPassword").val()){
		...
		<c:if test="${fn:indexOf(memberVO.authGroupId, '2016AUTH0000006') != -1}">
		
			var deptNosCnt = $("input[name=deptNos]:checkbox:checked").length;
	
			if(deptNosCnt == 0){
				alert("힉과을 선택하지 않았습니다.");
				$("#deptNo1").focus();
				return false;
			}
		...
		var reqUrl = CONTEXT_ROOT + targetUrl + "updateMember.do";
	...
	function fn_receiveNoEmail(){
		alert("이메일 수신거부시\n\n학습관리통합시스템 교육사이트에 보내는 각종정보를 받아볼수없습니다..");
	}
	//SMS 수신거부
	...
	<tbody>
		<tr>
			...
			<th width="132px"><span class="requare">*</span>성별</th>
	  		<td>
	  			<input type="radio" name="sex" value="F" class="choice" ${memberVO.sex == 'F' ? 'checked' : ''}/>
      			여자 &nbsp;
      			<input type="radio" name="sex" value="M" class="choice" ${memberVO.sex == 'M' ? 'checked' : '' }/>
      			남자 
	  		</td>  
		</tr>
		<tr>
			...
			<td>
				<%-- <input type="text" id="memId" name="memId"  value="${memberVO.memId}" style="width:80%" />
		...
		<tr>
			...
			<td>
				<input type="password" id="memPasswordEncript" name="memPasswordEncript"  value=""  style="width:99%"  />	
				<form:errors path="memPasswordEncript" style="width:99%" />			
			</td>      
			<th><span class="requare">*</span>비밀번호확인</th>
	  		<td>
	  			<input type="password" id="memPassword" name="memPassword"  value=""  style="width:99%"  />
		...
		<tr>
			<th>이메일 수신여부</th>
			<td>
				<input type="radio" name="receiveMailYn" id="receiveMailYn" value="Y" class="choice" ${memberVO.receiveMailYn == 'Y' ? 'checked' : ''}/>
      			예 &nbsp;
      			<input type="radio" name="receiveMailYn" id="receiveMailYn" value="N" class="choice" ${memberVO.receiveMailYn == 'N' ? 'checked' : ''} onclick="javascript:fn_receiveNoEmail();" />
      			아니오			
			</td>      
			<th>SMS 수신여부</th>
	  		<td>
	  			<input type="radio" name="receiveSmsYn" id="receiveSmsYn" value="Y" class="choice" ${memberVO.receiveSmsYn == 'Y' ? 'checked' : ''}/>
      			예 &nbsp;
      			<input type="radio" name="receiveSmsYn" id="receiveSmsYn" value="N" class="choice" ${memberVO.receiveSmsYn == 'N' ? 'checked' : ''} onclick="javascript:fn_receiveNoSMS();" />
      			아니오	
	  		</td>  
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				...
				<c:forEach var="authGroupCd" items="${authGroupCode}" varStatus="status">
					<input type="checkbox" name="authGroupIds" id="authGroupId${status.index}" value="${authGroupCd.codeId}"  <c:if test="${fn:indexOf(memberVO.authGroupId, fn:split(authGroupCd.codeId, '_')[0]) != -1}">checked</c:if>/><label for="authGroupId${status.index}">${authGroupCd.codeName}</label>
				</c:forEach>	 
				<form:errors path="authGroupId" style="width:99%" />  
			</td>      
			<th>소속</th>
			<td>
				<input type="text" id="compNm" name="compNm" value="${memberVO.compNm}" style="width:80%" readonly />
				<input type="hidden" id="companyId" name="companyId" value="${memberVO.companyId}" readonly/>
		...
		<c:choose>
			<c:when test="${fn:indexOf(memberVO.authGroupId, '2016AUTH0000006') != -1}">
				<tr>
					<th><span class="requare">*</span>학과</th>
					<td colspan="3">
						
						<c:forEach var="result" items="${deptCodeList}" varStatus="status">
							<input type="checkbox" name="deptNos" id="deptNo${status.index}" value="${result.codeId}"  <c:if test="${fn:indexOf(memberVO.deptNoStr, fn:split(result.codeId, '_')[0]) != -1}">checked</c:if>/><label for="deptNo${status.index}">${result.codeName}</label>
						</c:forEach>
						<%-- <select name="deptNo" id="deptNo" style="width:110px;">
							<option selected value=''>--선택--</option>
							<c:forEach var="result" items="${deptCodeList}" varStatus="status">
								<option value="${result.codeId}" ${fn:split(result.codeId, '_')[0] == memberVO.deptNo ? 'selected="selected"' : '' }>${result.codeName}</option>
							</c:forEach>
						</select> --%>
						<form:errors path="deptNo" style="width:99%" />
					</td>      
				</tr>
			</c:when>
			<c:otherwise>
