...
<script type="text/javascript">
	...
	function fn_formSave(){
		...
		if(!$("#memPassword").val()){
			alert("비밀번호확인을 하지 않았습니다.");
			$("#memPassword").focus();
			return false;
		} 
		if($("#memPassword").val()&&$("#memPasswordEncript").val()){
	...
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<br/><font color="red">*교직원일 경우 교직원번호 입력</font>
		...
		<tr>
			...
			<c:forEach var="authGroupCd" items="${authGroupCode}" varStatus="status">
				<input type="checkbox" name="authGroupIds" id="authGroupId${status.index}" value="${authGroupCd.codeId}"/><label for="authGroupId${status.index}">${authGroupCd.codeName}</label>
			</c:forEach>	
			...
			<td colspan="3">
				<select name="deptNo" id="deptNo" style="width:110px;">
					...
					<c:forEach var="result" items="${deptCodeList}" varStatus="status">
						<option value="${result.codeId}" ${result.codeId == memberVO.deptNo ? 'selected="selected"' : '' }>${result.codeName}</option>
					</c:forEach>
				</select>
