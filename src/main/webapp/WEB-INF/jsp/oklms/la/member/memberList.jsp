...
//         com.pageNavi( "pageNavi", totalCount , pageSize , pageIndex );
	...
	/* 로그인 */
	function fn_logIn(memId){
	    var reqUrl = "/commbiz/login/adminLoginProc.do";
	    $("#memId").val(memId);
	    $("#frmMember").attr("method", "post" );
		$("#frmMember").attr("action", reqUrl);
		//$("#frmMember").attr("target","_new");
		$("#frmMember").submit();
	}
	
	
	function fn_mergeContent(){
		
		var reqUrl =  "/la/online/mergeContent.json";
		var param = "";
	...
	function fn_mergePw(){
		...
		var reqUrl =  "/la/member/mergePw.json";
		var param = "";
		com.ajax(reqUrl, param, function(obj, data, textStatus, jqXHR){			
		if (200 == jqXHR.status ) {
	...
	<li>
		...
		<select name="searchAuthGroupId" id="searchAuthGroupId" style="width:120px">
	  			...
	  			<c:forEach var="searchAuthGroupCd" items="${searchAuthGroupCode}" varStatus="status">
					<option value="${searchAuthGroupCd.authGroupId}" ${searchAuthGroupCd.authGroupId == memberVO.searchAuthGroupId ? 'selected="selected"' : '' }>${searchAuthGroupCd.authGroupName}</option>
				</c:forEach> 
		 </select>
		 &nbsp;
...
<ul class="search-list-1 mt_15">
	...
	<li>
		<span>파일 업로드</span>
		<input name="uploadExcelFile" class="form_01 checkFileType" id="uploadExcelFile" type="file"/>
	</li>
</ul><!-- E : search-list-1 -->
</form>
...
<ul class="board-info">
	...
	<li class="btn-area">
		<!-- <a href="#fn_mergeContent" onclick="javascript:fn_mergeContent( ); return false" onkeypress="this.onclick;" class="btn">콘텐츠동기화테스트</a>
...
<table border="0" cellpadding="0" cellspacing="0" class="list-1">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<a href="#fn_logIn" onclick="javascript:fn_logIn( '<c:out value="${result.memId}"/>'); return false" onkeypress="this.onclick;" class="btn-1">로그인</a>
			</td>
		</tr>
		</c:forEach>
