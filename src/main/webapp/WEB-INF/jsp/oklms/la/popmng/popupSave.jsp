...
$(document).ready(function() {
	...
	$("table").find("input[name=popupType]:first").prop("checked", true);
	$("table").find("input[name=isCloseViewSettings]:first").prop("checked", true);
	$("table").find("input[name=contentType]:first").prop("checked", true);
	$("table").find("input[name=isUse]:first").prop("checked", true);
	...
	$("table").find("input[name=contentType]").bind("click", function() {
		...
		if (contentType == "I") {
			...
			$("table").find("tr.trContent").hide();
			$("table").find("input[name=popupWidth]").val("0");
			$("table").find("input[name=popupHeight]").val("0");
			$("table").find("input[name=popupWidth]").removeAttr("validateType");
			$("table").find("input[name=popupHeight]").removeAttr("validateType");
			$("table").find("input[name=pageUrl]").val("http://");
		} else {
			$("table").find("input[name=pageUrl]").val("");
			$("table").find("tr.trImageUpload").hide();
...
function fn_formSave(){
	if (fn_formValidate() && confirm("저장 하시겠습니까?")) {
		var reqUrl = "/la/popmng/insertPopup.do";
...
<form:form modelAttribute="frmPopup" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" id="popupId"   name="popupId"  value="${comcodeVO.codeId}" />
				...
				<table border="0" cellpadding="0" cellspacing="0" class="view-1" style="margin:0;">
					<tbody>
						<tr><th>노출위치<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">	
							<select name="pageType" id="pageType" class="selectpicker btn-xs reset" data-width="130px">
								...
								<c:forEach items="${ccFocusList}" var="ccInfo" varStatus="status">
									<option value="${ccInfo.codeCode}" ${param.pageType == ccInfo.codeCode ? 'selected="selected"' : ''}>${ccInfo.codeName}</option>
								</c:forEach>
							</select>
						</td>	</tr>
						<tr>	<th>제목<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3"><input type="text" id="title"  name="title"	 style="width:100%;" /></td>	</tr>
						<tr>	<th>게시일<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">
						<input type="text" id="startDate" name="startDate" value="${param.startDate}" style="width: 70px;" readonly="readonly" />
						...
						<input type="text"  id="finishDate" name="finishDate" value="${param.finishDate}" style="width: 70px;" readonly="readonly" />
...
<%-- 						<input type="text" id="popupType"       name="popupType"	value="${comcodeVO.popupType}"  style="width:100%;" /> --%>
						<input type="radio" id="popupType1" name="popupType" value="L"/><label for="popupType1">layer</label>
						<input type="radio" id="popupType2" name="popupType" value="W"/><label for="popupType2">windows</label>
						</td>	</tr>
...
<%-- 						<input type="text" id="frm_codeNameEng"	name="codeNameEng"		value="${comcodeVO.codeNameEng}"  style="width:100%;" /> --%>
						<input type="radio" id="isCloseViewSettings1" name="isCloseViewSettings" value="D"/><label for="isCloseViewSettings1">하루동안 그만보기</label>
		        		<input type="radio" id="isCloseViewSettings2" name="isCloseViewSettings" value="W"/><label for="isCloseViewSettings2">일주일간 그만보기</label>		
						</td>	</tr>
						<tr>	<th>컨텐츠종류<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">
						<input type="radio" id="contentType1" name="contentType" value="H"/><label for="contentType1">HTML</label>
						<input type="radio" id="contentType2" name="contentType" value="I"/><label for="contentType2">이미지 업로드</label>
					    </td>	</tr>
						<tr class="trPopupSize">	<th>팝업창 크기<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>	<td colspan="3">
						가로&nbsp;<input name="popupWidth" id="popupWidth" class="form-control inline wd60 numric" min="350" max="1000" validateMsg="팝업 창 크기는(은) {1} ~ {2} 사이로 입력해 주십시오." maxlength="4" value="0" />&nbsp;px
						&nbsp;&nbsp;&nbsp;&nbsp;
						세로&nbsp;<input name="popupHeight" id="popupHeight" class="form-control inline wd60 numric" min="100" max="1000" validateMsg="팝업 창 크기는(은) {1} ~ {2} 사이로 입력해 주십시오." maxlength="4" value="0" />&nbsp;px
						</br>※팝업창의 크기를 설정하여 주십시오.
						</td>	</tr>
						<tr>	<th>팝업위치</th>		<td colspan="3">
						Top&nbsp;<input name="positionTop" class="form-control inline wd60 numric" validateType="required" validateMsg="tit.popupPositionType 항목은 필수 입력 항목입니다." maxlength="4" value="0" />&nbsp;px
						&nbsp;&nbsp;&nbsp;&nbsp;
						Left&nbsp;<input name="positionLeft" class="form-control inline wd60 numric" validateType="required" validateMsg="tit.popupPositionType 항목은 필수 입력 항목입니다." maxlength="4" value="0"/>&nbsp;px
						...
						<tr class="trImageUpload">	<th>링크 URL<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">
							<input id="pageUrl" name="pageUrl" placeholder="http://" class="form-control inline wd400" maxlength="255" />
...
<!-- 									multiple -->
						...
						<tr class="trContent">	<th>내용<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">
							<textarea name="textAreaContent" id="textAreaContent" style="width: 100%;height: 500px;" rows="20" cols="80"></textarea>
						</td>	</tr>
						<tr>	<th>사용여부<img src="${ImgUrl}icon/required.gif" alt="필수입력" title="필수입력" width="15" height="15"></th>		<td colspan="3">
						<input type="radio" id="isUse1" name="isUse" value="Y"/><label for="isUse1">예</label>
		        		<input type="radio" id="isUse2" name="isUse" value="N"/><label for="isUse2">아니오</label>	
						</td>	</tr>
					</tbody>
				</table><!-- E : view-1 -->
</form:form>
<div class="page-btn">
	<a role="button" href="#fn_formSave" onclick="javascript:fn_formSave();return false;" onkeypress="this.onclick;">등록</a><a href="/la/popmng/listPopup.do">목록</a>
</div>	
<script type="text/javaScript" language="javascript">
<!-- Smart Editor -->
