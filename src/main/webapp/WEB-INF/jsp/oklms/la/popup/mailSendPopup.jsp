...
<script type="text/javascript">
	...
	function fn_send_mail(){
		if( com.isBlank( $("#receiveEmail").val() ) ){
			alert("받는사람 이메일 주소가 없습니다.");
			return false;
		}
		...
		if(confirm("메일을 발송하시겠습니까?")){
		  	...
		  	var reqUrl = CONTEXT_ROOT + "/la/member/member/saveMemberMailSend.do";
...
<input type="hidden" name="returnUrlGubun" id ="returnUrlGubun" value="${returnUrlGubun}"/>
<input type="hidden" name="recvEmailSet" id ="recvEmailSet" value="${mailModel.recvEmailSet}"/>
<input type="hidden" name="recvMemSeqSet" id ="recvMemSeqSet" value ="${mailModel.recvMemSeqSet}"/>
<input type="hidden" name="senderMemSeq" id ="senderMemSeq" value ="${mailModel.senderMemSeq}"/>
 
<div id="popup-wrarpr">
			<div id="popup-header">
				<h1><img src="/images/oklms/adm/inc/pop_border_02.png" alt="" />메일보내기</h1>
...
<table border="0" cellpadding="0" cellspacing="0" class="view-1" style="margin:0;">
	<tbody>
		<tr>
			...
			<td>
				<input type="text" id="sendName" name="sendName" value="${mailModel.sendName}" style="width:99%" readOnly/>		
			</td>      
		</tr>
		<tr>
			<th><span class="requare">*</span>받는 사람</th>
			<td>
				<input type="text" id="receiveEmail" name="receiveEmail" value="${mailModel.recvEmailSet}" style="width:99%" readOnly/>
			</td>      
		</tr>
		<tr>
