...
function previewImages() {
  ...
  $("#bannerImage").val($('#file-input').val());
  var filesize = 0;
  if (this.files) {
...
function fncBannerInsert() {
    var varFrom = document.getElementById("banner");
    varFrom.action = "<c:url value='/la/banner/addBanner.do'/>";
...
<form:form modelAttribute="banner" method="post" action="${pageContext.request.contextPath}/la/banner/addBanner.do' />" enctype="multipart/form-data"> 
<input type="hidden" name="posblAtchFileNumber" value="1" >
<table width="100%" cellpadding="8" class="view-1" summary="배너정보를 입력하는 항목">
...
<%--     	<input name="bannerId" id="bannerId" title="배너ID" type="text" value="<c:out value='${banner.bannerId}'/>" size="30" class="readOnlyClass" placeholder="자동생성됩니다." readonly > --%>
<!--     </td> -->
<!--   </tr> -->
  <tr>
    ...
    <td class="lt_text" nowrap><input name="bannerNm" id="bannerNm" title="배너명" type="text" value="<c:out value='${banner.bannerNm}'/>" maxLength="30" size="30" >&nbsp;<form:errors path="bannerNm" /></td>
  </tr>
   <tr>
    ...
    <td class="lt_text" nowrap>
    	<select name="bannerType" class="selectpicker btn-xs reset" data-width="130px">
				...
				<c:forEach items="${ccBannerTypeList}" var="ccBannerType" varStatus="status">
					<option value="${ccBannerType.codeCode}">${ccBannerType.codeName}</option>
				</c:forEach>
		</select>
    </td>
  </tr>
...
<div class="page-btn">
	<a role="button" href="#fn_formSave" onclick="javascript:fncBannerInsert();return false;" onkeypress="this.onclick;">저장</a>
