...
<script type="text/javascript">
	...
	function fn_egov_insert_addClbInf(){
		...
		document.frm.action = "<c:url value='/la/cop/clb/addClubInf.do'/>";
		document.frm.submit();
	}

	function fn_egov_select_clbInfs(pageIndex){
		document.frm.pageIndex.value = pageIndex;
		document.frm.action = "<c:url value='/la/cop/clb/selectClubInfs.do'/>";
		document.frm.submit();
	}

	function fn_egov_inqire_clbInf(clbId){
		document.frm.clbId.value = clbId;
		document.frm.action = "<c:url value='/la/cop/clb/selectClubInf.do'/>";
		document.frm.submit();
	}
</script>
<title>동호회 목록</title>
		...
		<c:forEach var="result" items="${resultList}" varStatus="status">
		  <tr>
		    ...
		    <td class="lt_text3" nowrap>
		    	...
		    	<!-- 2010.11.1
				<form name="subForm" method="post" action="<c:url value='/la/cop/clb/selectClubInf.do'/>">
				...
				<a href="<c:url value='/la/cop/clb/selectClubInf.do'/>?clbId=<c:out value='${result.clbId}'/>" onclick="event(this.href); return false;" style="width:320px;border:solid 0px black;text-align:left;">
