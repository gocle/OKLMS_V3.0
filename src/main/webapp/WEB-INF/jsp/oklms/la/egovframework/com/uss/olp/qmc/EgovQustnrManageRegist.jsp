...
function fn_egov_save(){
	...
	if (fn_egov_validateForm() && confirm('<spring:message code="common.regist.msg" />')) {
		varFrom.action = "<c:url value='/la/uss/olp/qmc/goInsertEgovQustnrManage.do' />";
		varFrom.submit();
	}
...
<!--  상단타이틀 Start -->
<div class="title-name-1">설문관리 등록</div>
<!--  상단타이틀 End -->
...
<caption>등록 을 제공한다</caption>
  <tr>
    ...
    <td width="80%" nowrap>
      <input type="text" name="qestnrSj" id="qestnrSj" size="106" maxlength="250" title="설문제목 입력"/>
      <form:errors path="qestnrSj"/>
    </td>
  </tr>
  <tr>
    <th scope="row" width="20%" height="23"  nowrap ><label for="qestnrPurps">설문목적</label><img src="<c:url value='/images/egovframework/com/cmm/icon/required.gif' />" width="15" height="15" alt="필수입력표시"></th>
    <td width="80%" nowrap>
    	<textarea id="qestnrPurps" name="qestnrPurps" rows="3" cols="20" title="설문목적 입력" style="width:95%;"></textarea>	
  ...
  <tr>
    ...
    <td>
    	<textarea id="qestnrWritngGuidanceCn" name="qestnrWritngGuidanceCn" rows="3" cols="20" title="설문작성안내 내용 입력" style="width:95%;"></textarea>
  ...
  <tr>
    ...
    <td width="80%" nowrap>
      <input type="text" name="qestnrBeginDe" id="qestnrBeginDe" size="12" maxlength="10" readonly="true" title="설문대상 시작일 입력"/>
      <form:errors path="qestnrBeginDe"/>
      ~<input type="text" name="qestnrEndDe" id="qestnrEndDe" size="12" maxlength="10" readonly="true" title="설문대상 종료일 입력"/>
    ...
    <td width="80%" nowrap>
		<select name="qestnrTmplatId" id="qestnrTmplatId" style="width:110px;">
			...
			<c:forEach var="surveyTmplatCd" items="${surveyTmplatCode}" varStatus="status">
				<option value="${surveyTmplatCd.codeId}">${surveyTmplatCd.codeName}</option>
			</c:forEach>
		</select>
