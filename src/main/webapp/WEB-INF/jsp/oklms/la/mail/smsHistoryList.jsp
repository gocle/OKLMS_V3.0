...
<ul class="search-list-1">
	...
	<li>
		<span>발신자</span>
		<input type="text" name="searchSendName" id="searchSendName" value="${mailVO.searchSendName}" style="width:415px;" onkeypress="press(event);"/>
	</li>
	<li>
		<span>수신자</span>
		<input type="text" name="searchReceiveName" id="searchReceiveName" value="${mailVO.searchReceiveName}" style="width:415px;" onkeypress="press(event);"/>
	</li>
        ...
        <c:when test="${!empty listSmsHistory}">
          <c:forEach var="listSmsHistory" items="${listSmsHistory}" varStatus="status">
            <tr>
              ...
              <td><a href="#fn_read" onclick="javascript:fn_read('${listSmsHistory.trNum }');">${listSmsHistory.sendName }</a></td>
              <td>${listSmsHistory.trCallBack }</td>
              <td>${listSmsHistory.receiveName }</td>
