...
//         com.pageNavi( "pageNavi", totalCount , pageSize , pageIndex );
	...
	function fn_read( param1 ){
		...
		$("#traningProcessId").val( param1 );
		
		var reqUrl = CONTEXT_ROOT + targetUrl + "getTraningProcess.do";
		$("#frmProcess").attr("action", reqUrl);
		$("#frmProcess").submit();
	}
	...
	function fn_delt(){
		if (confirm("삭제 하시겠습니까?")) {
			...
			$("#traningProcessId").val( checkedVals );
	...
	/* 신규 페이지로 이동 */
	function fn_insert_page(){
		
		var reqUrl = CONTEXT_ROOT + targetUrl + "insertTraningProcessPage.do";
...
<form id="frmProcess" name="frmProcess"  method="post">
	...
	<input type="hidden" id="traningProcessId" name="traningProcessId" value="" /> 
<ul class="search-list-1">
	<li>
		<span>훈련과정/훈련과정번호</span>
		&nbsp;&nbsp;&nbsp;<input type="text" name="searchKeyword" id="searchKeyword" value="${param.searchKeyword}"  maxlength="40" style="width:410px;" />
	</li> 
</ul><!-- E : search-list-1 -->
</form>
...
<table border="0" cellpadding="0" cellspacing="0" class="list-1">
	<thead>
		<tr>
			<th width="15%">훈련과정번호</th>
			<th width="30%">훈련과정명</th>
			<th width="5%">회차</th>
			<th width="20%">훈련시작일</th>
			<th width="20%">훈련종료일</th>
			<th width="10%">등록일</th>
		</tr>
	</thead>
	<tbody>
		...
		<tr>
			...
			<td>
			<a href="javascript:fn_read('${result.traningProcessId}')" ><span color="blue"><c:out value="${result.traningProcessName}" /></span></a>
			</td>
...
<div class="page-btn">
	<a href="#" onclick="fn_insert_page();">등록</a>
</div><!-- E : page-btn -->
