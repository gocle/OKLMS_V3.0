...
<input type="hidden" id="pointNotUseMigYn" name="pointNotUseMigYn" value="${currProcVO.pointNotUseMigYn }" />
...
<!-- 팝업 사이즈 : 가로 최소 650px 이상 설정 -->
		<div id="pop-wrapper" class="min-w650">
			...
			<div class="search-box-1 mb-010">
				...
				<select id="searchYyyyCd" name="searchYyyyCd" onchange="" style="width:150px;" >
				...
				<select id="searchTermCd" name="searchTermCd" onchange="" style="width:150px;" >
					<option value="" selected>::선택::</option>
				...
				<select id="searchSubClassCd" name="searchSubClassCd" onchange="" style="width:150px;" >
					<option value="" selected>::선택::</option>
				...
				<input type="text" id="searchSubjectName" name="searchSubjectName" value="${currProcVO.searchSubjectName }" placeholder="개설강좌명 입력" />
			...
			<table class="type-2">
				...
				<tbody>
					...
					<tr>
						<td><input type="checkbox" name="tempSubjectCode" id="tempSubjectCode${status.index}" value="${result.yyyy}||${result.term}||${result.subjectCode}||${result.subClass}||${result.subjectName}||${parmCount}||${result.termName}"></td>
						<td>${result.departmentName}</td>
						<td>${result.yyyy}</td>
						<td>
						<c:choose>
							<c:when test="${result.term eq  '1'}">1학기</c:when>
							<c:when test="${result.term eq  '2'}">2학기</c:when>
							<c:when test="${result.term eq  '3'}">여름학기</c:when>
							<c:otherwise>겨울학기</c:otherwise>
						</c:choose>
						</td>
						<td class="left"><label for="tempSubjectCode${status.index}">${result.subjectName}</label></td>
						<td>${result.subjectCode}</td>
						<td>${result.subClass}</td>
						<td>${result.traningMappingCnt}</td>
					</tr>
					</c:forEach>
