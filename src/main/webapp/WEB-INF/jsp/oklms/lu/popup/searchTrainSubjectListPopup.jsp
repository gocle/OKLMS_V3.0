...
<input type="hidden" id="companyId" name="companyId" value="${currProcVO.companyId }" />
<input type="hidden" id="traningProcessId" name="traningProcessId"/>
<input type="hidden" id="memSeq" name="memSeq" value="${currProcVO.memSeq }"/>
...
<!-- 팝업 사이즈 : 가로 최소 650px 이상 설정 -->
		<div id="pop-wrapper" class="min-w650">
			...
			<div class="search-box-1 mb-010">
				...
				<select id="searchDepartmentNo" name="searchDepartmentNo" onchange="">
					<option value="" selected>학과명</option>
					<c:forEach var="result" items="${deptCodeList}" varStatus="status">
						<option value="${result.codeId}" <c:if test="${currProcVO.searchDepartmentNo eq result.codeId }" > selected="selected"  </c:if>>${result.codeName}</option>
					</c:forEach>
				</select>
				...
				<select id="searchYyyyCd" name="searchYyyyCd" onchange="">
					...
					<c:forEach var="i" begin="0" end="6" step="1">
						<option value="${nowYear-i}" <c:if test="${currProcVO.searchYyyyCd eq nowYear-i }" > selected="selected"  </c:if>>${nowYear-i}학년도</option>
					</c:forEach>	
					<!-- 
					...
					<option value="2015" ${'2015' == currProcVO.searchYyyyCd ? 'selected="selected"' : '' }>2015학년도</option>
					 -->
				...
				<select id="searchTermCd" name="searchTermCd" onchange="">
					...
					<option value="3" ${'3' == currProcVO.searchTermCd ? 'selected="selected"' : '' }>여름학기</option>
					<option value="4" ${'4' == currProcVO.searchTermCd ? 'selected="selected"' : '' }>겨울학기</option>
				</select>
				...
				<select id="searchSubClassCd" name="searchSubClassCd" onchange="">
					<option value="" selected>분반</option>
			...
			<table class="type-2">
				...
				<thead>
					<tr>
						<th><input type="checkbox" id="check0" name="check0" onclick="javascript:com.checkAll('check0','check1');"/></th>
						<th>학과명</th>
						<th>학년도</th>
						<th>학기</th>
						<th>교과목명</th>
					...
					<c:forEach var="result" items="${resultTrainSubjectSeachList}" varStatus="status">
					<tr>
						<td><input type="checkbox" id="check1" name="check1" value="${result.traningProcessMappingId}"></td>
					...
					<c:if test="${fn:length(resultTrainSubjectSeachList) == 0}">
					<tr>
