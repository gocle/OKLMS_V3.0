...
<script type="text/javascript">
	...
	//선택 버튼을 클릭시 부모창에 필요항목 셋팅
	function fn_company(companyId,companyName){
		if( opener ){
			$(opener.document).find("#companyId").val(companyId);
			$(opener.document).find("#companyName_text").text(companyName);
			window.close();
		}
	}
...
<!-- 팝업 사이즈 : 가로 최소 650px 이상 설정 -->
		<div id="pop-wrapper" class="min-w650">
			...
			<div class="search-box-1 mb-010">
				<input type="text" id="searchWord" name="searchWord" value="${param.searchWord}" placeholder="기업명" style="width:150px;" /> 
			...
			<table class="type-2">
				...
				<thead>
					...
					<tr>
						<td><input type="radio" id="check1" name="check1" value="${result.companyId}" onclick="fn_company('${result.companyId}','${result.companyName}');"></td>
						<td>${result.companyName}</a></td>
