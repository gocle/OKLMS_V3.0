...
<script type="text/javascript">
	...
	/* 화면이 로드된후 처리 초기화 */
	function initHtml() {
		$("#deptNm").text($("#deptNo option:checked").text());
	}
	...
	function fn_popup(yyyy, semstrCd, groupMemIds, companyName){
		...
		$("#frmGradePop_companyName").val(companyName);
...
<form id="frmGradePop" name="frmGradePop" method="post">
	...
	<input type="hidden" name="companyName" id="frmGradePop_companyName" />
	...
	<input type="hidden" name="submitIds" id="submitIds" />				
			...
			<div class="group-area mt-020">
							<div class="search-box-1">
								...
								<label for="deptNo" class="hidden">학과 선택</label>
								<select id="deptNo" name="deptNo"  onchange="javascript:fn_search();" > 
										<c:forEach var="result" items="${listMyDept}" varStatus="status">
									      	<option value="${result.deptNo}" <c:if test="${gradeVO.deptNo eq result.deptNo }" > selected="selected"  </c:if>>${result.deptName}</option>
									    </c:forEach>								
								</select>			
							...
							<table class="type-2 mt-020 wp100">
								<caption>학년도 학기 학과에 대한 정보 제공</caption>
								<colgroup>
									<col style="width:33%" />
									<col style="width:33%" />
									<col style="width:*" />
							...
							<div class="table-responsive mt-010">
								<table class="type-2 ">
									<caption>참여기업명 인원 발송여부 조회에 대한 정보 제공</caption>
									<colgroup>
										<col style="width:10%" />
										<col style="width:*" />
										<col style="width:20%" />
									...
									<thead>
										<tr>
											<th  scope="col"><label for="checkbox">선택</label><br/><input type="checkbox" name="checkbox" id="checkbox"></th>
											<th  scope="col">참여기업명</th>
											<th  scope="col">인원</th>
									...
									<c:forEach var="result" items="${statusList}" varStatus="status">
										<tr>
											<td><label for="checkIds${status.count}" class="hidden">선택</label><input type="checkbox" name="checkIds" id="checkIds${status.count}"  value="${result.companyId }|${result.yyyy}|${result.term}|${result.deptNo}" <c:if test="${result.sendYn eq 'Y' or result.gradeCnt eq '0'}">disabled="disabled"</c:if>  /></td>
							...
							<div class="btn-area mt-010">
								...
								<label for="submitComment" class="hidden">전송 사유 입력</label>
								<input type="text" name="submitComment" id="submitComment" placeholder="전송시 사유 필수 입력" maxlength="30" value="" /><a href="#" onclick="fn_submit();" class="orange">전송</a>
								</div>
							</div><!-- E : btn-area -->
						</div>
						...
						<div class="table-responsive mt-040">
							<table class="type-2">
								<caption>전송일 기업명 사업자등록번호 수신인 확인여부 확인일 전송사유 IP에 대한 정보 제공</caption>
								<colgroup>
									<col style="width:6%" />
									<col style="width:8%" />
									<col style="width:*" />
								...
								<c:forEach var="result" items="${sendList}" varStatus="status">
									<tr>
										...
										<td>${result.confirmDate}</td>
										<td class="left">${result.submitComment}</td>
										<td>${result.ip}</td>
									</tr>
								</c:forEach>
