...
<script type="text/javascript">
	...
	function fn_delt(){
		...
		var selectInfo = $("input:checkbox[name='tmpQestnrIdArr']:checked").val();
...
<div id="">
	...
	<h4 class="mb-010"><span><c:if test="${currProcReadVO.onlineTypeName eq '온라인'}">[ONLINE]</c:if>${currProcReadVO.subjectName} / ${currProcReadVO.subjectCode} (${currProcReadVO.subClass}분반)</span> ㅣ  ${currProcReadVO.yyyy}학년도 / ${currProcReadVO.termName}</h4>
		...
		<table class="type-1 responsive-tr">
			<caption>설문조사 교과형태, 과정구분</caption>
			<colgroup>
				<col style="width:15%" />
				<col style="width:*px" />
				<col style="width:15%" />
...
<h2 class="mt-040">(${currProcReadVO.subjectName}) 설문 목록</h2>
<div class="search-box-1 mt-010 mb-020">
	<label for="searchCondition" class="hidden">검색 내용 선택1</label>
	...
	<label for="searchStatus" class="hidden">검색 내용 선택2</label>
	...
	<a href="javascrpt:void(0)" onclick="javascript:fn_search(1); return false" onkeypress="this.onclick();">검색</a>
</div><!-- E : search-box-1 -->

<ul class="page-sum mb-007">
	<li class="float-left"><span>총 </span><span id="totalRow">0</span><span> 건</span> </li>
	<li class="float-right">
		PAGE VIEW :  <label for="rowNumber" class=""><input type="radio" id="rowNumber" name="rowNumber" value="10" <c:if test="${rowNumber eq 10 || rowNumber eq ''}">checked</c:if>> 10</label>
		 <label for="rowNumber" class=""><input type="radio" id="rowNumber" name="rowNumber" value="20" <c:if test="${rowNumber eq 20}">checked</c:if>> 20</label>
		 <label for="rowNumber" class=""><input type="radio" id="rowNumber" name="rowNumber" value="50" <c:if test="${rowNumber eq 50}">checked</c:if>> 50</label>
		Lines
	</li>
</ul>
...
<div class="table-responsive">
	<table class="type-2">
		<caption>설문 목록 제목, 작성자, 등록일, 참여자, 상태 제공</caption>
		<colgroup>
			<col style="width:6%" />
			<col style="width:6%" />
			<col style="width:*" />
		...
		<tbody>
			...
			<tr>
				<td><input type="checkbox" id="tmpQestnrId" name="tmpQestnrIdArr" value="${result.qestnrId}"></td>
				<td>${status.count}</td>
...
<div class="btn-area">
	<div class="float-right">
		<a href="#선택 삭제" onclick="javascript:fn_delt(); return false" class="black">선택 삭제</a>
		<a href="#수정" onclick="javascript:fn_updt(); return false" class="black">수정</a>
		<a href="#작성" onclick="javascript:fn_cret(); return false" class="black">작성</a>
	</div>
</div>
