...
function fn_search(){
	...
	var reqUrl = "/lu/traningstatus/listTraningstatusCot.do";
	$("#mode").val("0");
...
function fn_offPopup(){
	...
	var reqUrl = "/lu/traningstatus/popupTraningstatusCot.do";
...
function smsB(){		
	...
	fn_send_sms(${traningStatusVO.weekCnt},arr_value,'','','${currProcVO.subjectCode }');
...
function emailB(){	
	...
	fn_send_mail(${traningStatusVO.weekCnt},arr_value,'','','${currProcVO.subjectCode }');	
}
...
//--> 
						...
						<dl id="tab-type">
							...
							<dd id="tab-content-11" style="display:block;">
								<h4 class="mt-020 mb-010"><span>${currProcVO.subjectName } / ${currProcVO.subjectCode } / ${currProcVO.subClass }반 </span>  ${currProcVO.yyyy}학년도 / ${currProcVO.termName}</h4>
								...
								<div class="progress-area mb-030">
									<p></p>
									<div class="sub_text blue_text mb-010">
		 								 <i class="xi-label"></i> 권장 진도율 (<fmt:formatNumber value="${ getProgress.allTimeHourNow/getProgress.totalTime }"  type="percent" />)
		 							</div>
									<progress value="${ getProgress.allTimeHourNow/getProgress.totalTime *100 }" max="100" style="width: 100%"></progress>
								
									
									<div class="sub_text blue_text mt-020 mb-010">
		 								 <i class="xi-label"></i> 실제 진도율 <c:if test="${ getProgress.realProgressRate > 0}" >(<fmt:formatNumber value="${ getProgress.realProgressRate/100 }"  type="percent" />)</c:if>
		 							</div>
...
<input type="hidden" id="searchCondition" name="searchCondition" value=""/>
								...
								<div class="search-box-1 mt-020 mb-020">
									<label for="searchType" class="hidden">강의실진도</label>
								...
								<table class="type-2">
									<caption>번호 분반 기업명 학번 이름 학년 학습현황 출석 지각 결석 진도율에 대한 정보 제공</caption>
									<colgroup>
										<col style="width:4%" />
										<col style="width:4%" />
										<col style="width:4%" />
									...
									<thead>
										<tr>
											...
											<th scope="col" colspan="4">학습현황</th>
										</tr>
										<tr>
									...
									<c:forEach var="resultlists" items="${resultlist}" varStatus="status">
										<tr>
											...
											<td><img src="/commbiz/photo/getAunuriUserImage.do?memId=${resultlists.memId }" width="65" height="55" alt="${resultlists.memName }"  />&nbsp;&nbsp;&nbsp;${resultlists.memName }</td>
...
<form name="frmTraningstatusDetail" id="frmTraningstatusDetail" method="post">		
									...
									<c:forEach var="resultBottomlist" items="${resultBottomlist}" varStatus="status">
										<tr>
											...
											<td>
												...
												<a href="/lu/activity/listActivityOt.do?weekCnt=${traningStatusVO.weekCnt}&memId=${resultBottomlist.memId}&classId=${resultBottomlist.classId}" class="btn-line-gray">작성</a>
												</c:if>
												<c:if test="${empty resultBottomlist.content}">
												<a href="/lu/activity/listActivityOt.do?weekCnt=${traningStatusVO.weekCnt}&memId=${resultBottomlist.memId}&classId=${resultBottomlist.classId}" class="btn-line-orange">미작성</a>
												</c:if>
								...
								<div class="btn-area  mt-010">
									<div class="float-right">
										<a href="javascript:smsB();" class="gray-2">SMS 발송</a>
										<a href="javascript:emailB();" class="gray-2">E-MAIL 발송</a>
									</div>
								</div><!-- E : btn-area -->
							</dd>
						</dl>
