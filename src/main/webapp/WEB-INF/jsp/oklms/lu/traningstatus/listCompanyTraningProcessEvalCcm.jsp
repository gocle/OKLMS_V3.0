...
function fn_searchYear( pageIndex ){
		...
		var reqUrl = "/lu/traningstatus/listCompanyTraningProcessCcm.do";
...
<form id="frmTraningstatus" name="frmTraningstatus" method="post">
	...
	<div class="group-area mt-020">
				 ...
				 <dt class="tab-name-11"><a href="javascript:fn_tab('note');">훈련일지</a></dt>
                  <dd id="tab-content-11">
                  
                   <dt class="tab-name-12 "><a href="javascript:fn_tab('act');">학습활동서</a></dt>
                  <dd id="tab-content-12">
                  
                  <dt class="tab-name-13 on" ><a href="#!">평가결과</a></dt>
                  <dd id="tab-content-13"  style="display:block">
							...
							<div class="search-box-1 mb-020">
								...
								<select name="searchTraningProcess" id="searchTraningProcess" onchange="fn_search(1);">
									...
									<c:forEach var="result" items="${listTraningStatusCcn}" varStatus="status">
										<c:set var="searchTraningProcess" value="${result.traningProcessId}|${result.companyId}"/>
										<option value="${result.traningProcessId}|${result.companyId}" <c:if test="${searchTraningProcess eq param.searchTraningProcess }" > selected="selected"  </c:if>>${result.traningProcessName}-${result.traningProcessPeriod}회차</option>
									</c:forEach>
								</select>
								
								<label for="searchSubject" class="hidden">교과목</label>
								<select name="searchSubject" id="searchSubject" onchange="fn_search(1);">
									<option value="">교과목</option>
									<c:forEach var="result" items="${listTraningSubjectStatusCcn}" varStatus="status" >
										<c:set var="searchSubject" value="${result.yyyy}|${result.term}|${result.subjectCode}|${result.subjectClass}"/>
										<option value="${result.yyyy}|${result.term}|${result.subjectCode}|${result.subjectClass}" <c:if test="${searchSubject eq param.searchSubject }" > selected="selected"  </c:if>>${result.subjectName}</option>
									</c:forEach>
								</select> 
							...
							<table class="type-2">
								<caption>학번 학습근로자명 진행학기 교과목명능력단위명 이수여부에 대한 정보 제공</caption>
								<colgroup>
									<col style="width:15%" />
									<col style="width:15%" />
									<col style="width:15%" />
