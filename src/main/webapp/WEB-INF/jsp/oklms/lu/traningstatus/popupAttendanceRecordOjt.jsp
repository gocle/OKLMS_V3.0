...
<script type="text/javascript" src="/js/oklms/iscroll.js"></script>
		...
		<div id="pop-wrapper" class="min-w650">
			...
			<ul id="learner-wrap" class="mt-010 mb-010">
				<li id="prev" onclick="myScroll.scrollToPage('prev', 0);return false">prev</li>
				<li id="wrap">
					<!-- 학습자수 * 128px의 값을 아래 style width에 넣어줘야함 -->
					<div id="scroller" style="width:1280px;">
						<ul id="thelist" class="name-tab-btn">
									<c:forEach var="result" items="${subjectNameList}" varStatus="status">
										<li <c:if test="${result.classId eq traningStatusVO.classId }">  class="current" </c:if> >
											<a href="#!" onclick="javascript:fn_search('${result.classId}')">${result.classId}분반_${result.companyName }</a>
										</li>										
									</c:forEach>
						</ul>
					</div>
			...
			<h4 class="mb-010"><span>${currProcVO.subjectName } / ${currProcVO.subjectCode } / ${currProcVO.subClass }반</span> ㅣ ${currProcVO.yyyy}학년도 / ${currProcVO.termName}</h4>
