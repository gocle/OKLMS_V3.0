...
function showRegular(index){
	...
	var html = "";  
	 ...
	 html +="						<div class='learner-training  mt-010 mb-010'>";
	 html +="							<div class='sub_text blue_text'><i class='xi-label'></i> 보강훈련</div>";
	 html +="							<div class='search_box3 float-left mt-010 mb-010''>";
	 html +="								<div><input type='text' name='studyDate' id='startDate"+index+"' style='width:85%;'/></div>";
	 html +="								<div>";
	 html +="									<label for=''>시작</label>";
	 html +="									<select name='traningStHour'>";
	 ...
	 html +="        						</div>";	
	 html +="								<div>";
	 html +="									<label for=''>종료</label>";
	 html +="									<select name='traningEdHour'>";
	 html +="											<option value='09' >오전 9시</option>";
	 html +="											<option value='10' >오전 10시</option>";
	 html +="											<option value='11' >오전 11시</option>";
	 html +="											<option value='12' >오전 12시</option>";
	 ...
	 html +="								</table>";
	 html +="							</div>";
	 html +="							<div class='btn-area mt-010'>";
...
function fn_insertEnrichment(index){
	...
	if( !$( "#traningNoteMemberIds"+index ).val()){
		alert("학습근로자를 먼저추가해주세요.");
		return;
	}
...
<fmt:parseNumber var = "offTimeHour" integerOnly = "true" value = "${offTimeHour}" />


					<h4 class="mb-010"><span>${TraningNoteVO.subjectName} / ${TraningNoteVO.subjectCode } / ${TraningNoteVO.classId }반</span> ${TraningNoteVO.yyyy}학년도 ${TraningNoteVO.termName} </h4>
					<form name="frmTraning" id="frmTraning" method="post">
						...
						<form name="frmTraning${status.index}" id="frmTraning${status.index}" method="post">
							...
							<input type="hidden" id="planTime" name="planTime" value="${offTimeHour}" />
							...
							<div class="sub_text blue_text mt-040">
 								 ...
 								 ${resultSum[status.index].traningDate}(${resultSum[status.index].dayOfWeek}) ${resultSum[status.index].traningSt}~${resultSum[status.index].traningEd} (${offTimeHour}시간)  ${result.traniningNoteMasterId }
 							</div>
							
							<div class="table-responsive mt-010">
								<table class="type-2">
									<caption>기업명 학번 성명 훈련시간 성취도 온라인 등 정보제공</caption>
									<colgroup>
										<!-- col width="5%" /-->
										<col width="15%" />
										<col width="15%" />
									...
									<thead>
										<tr>
											...
											<c:if test="${TraningNoteVO.onlineType ne 'NONE'}"> 
												<th scope="col">온라인</th>
											</c:if>
									...
									<tbody>
										...
										<tr>
											...
											<c:choose>
												<c:when test="${resultlist.outYn eq 'Y'}">
													<font color="red">${resultlist.memName}</font> <input type="hidden" name="memNmArr" id="memNmArr" value="${resultlist.memName}">
												</c:when>
												<c:otherwise>
													${resultlist.memName} <input type="hidden" name="memNmArr" id="memNmArr" value="${resultlist.memName}">
												</c:otherwise>
											</c:choose>
											  
												<c:if test="${!empty resultlist.studyTime }" >
												<input type="hidden" id="studyTimeArr" name="studyTimeArr" readonly="readonly"  maxlength="2"  style="width:95%;" onchange="javascript:if(this.value > ${offTimeHour} ){alert('훈련시간은 주간훈련시간보다 적어야합니다.');this.value='${offTimeHour}';return;};studyTimeArrZeor(this.value,'${offTimeHour}','${resultlist.memId}','${status.index+1}');" value="${resultlist.studyTime}"/> 
												</c:if>
												<c:if test="${empty resultlist.studyTime }" >
												<input type="hidden" id="studyTimeArr" name="studyTimeArr" readonly="readonly"  maxlength="2"   style="width:95%;" onchange="javascript:if(this.value > ${offTimeHour} ){alert('훈련시간은 주간훈련시간보다 적어야합니다.');this.value='${offTimeHour}';return;}studyTimeArrZeor(this.value,'${offTimeHour}','${resultlist.memId}','${status.index+1}');"  value="${offTimeHour}"/> 
												</c:if>
											</td>
											<td>
											<c:choose>
												<c:when test="${resultlist.attend eq '103'}">출석</c:when>
												<c:when test="${resultlist.attend eq '102'}">지각</c:when>
												<c:when test="${resultlist.attend eq '101'}">결석</c:when>
								...
								//--> 
											...
											<td class="left"><label for="bigoArr" class="hidden">비고</label><input type="text" style="width:95%;" id="bigoArr" name="bigoArr" value="${resultlist.bigo}"/></td>
										</tr>
										</c:forEach>
										<tr>
											<th scope="row"><label for="review${status.index}"></label>총평</th>
											<td colspan="6" class="left">
											
											<c:choose>
												<c:when test="${empty resultSum[status.index].review}">
													<textarea name="review" id="review${status.index}" rows="5" >${TraningNoteVO.lessonCnMst}</textarea>
												</c:when>
												<c:otherwise>
													<textarea name="review" id="review${status.index}" rows="5" >${resultSum[status.index].review}</textarea>
												</c:otherwise>
											</c:choose>
							...
							<div class="btn-area align-center mt-010">
								<a href="#!" class="black" onclick="fn_insert('${status.index}');">저장</a>
							</div>
							<!-- E : 정규수업 -->
							</form>
							</c:if>
...
<form name="frmTraningEnrichment${statustop.index}" id="frmTraningEnrichment${statustop.index}" method="post" >
								...
								<dd style="display:block;" class="mt-010">
								<div class="search-box-1  mb-020 align-left">
									<input type="text" name="studyDate" id="startDate" value="${TraningEnrichmentTimeVO.traningDate}"/> 
									<span class="respon-block">
									<label for="traningStHour" >시작 :</label> <select name="traningStHour"  class="respon-select">
									...
									<label for="traningStMin" class="hidden">시작 시간 분</label>
									<select name="traningStMin"  class="respon-select">
								...
								</span> <span class="search-none">~</span>
								<span class="respon-block">
								<label for="traningEdHour">종료 :</label> <select name="traningEdHour"  class="respon-select">
									...
									<label for="traningEdMin" class="hidden">종료 시간 분</label>
									<select name="traningEdMin"  class="respon-select">
										<option value="00"  <c:if test="${TraningEnrichmentTimeVO.traningStMin eq '00'}"> selected="selected" </c:if> >0 분</option>
										<option value="10" <c:if test="${TraningEnrichmentTimeVO.traningStMin eq '10'}">selected="selected" </c:if> >10 분</option>
										<option value="20" <c:if test="${TraningEnrichmentTimeVO.traningStMin eq '20'}">selected="selected" </c:if>>20 분</option>
										<option value="30" <c:if test="${TraningEnrichmentTimeVO.traningStMin eq '30'}">selected="selected" </c:if>>30 분</option>
									...
									<div class="table-responsive mt-010">
										<table class="type-2">
											<caption>기업명 학번 성명 훈련시간 성취도 보강비고 등 정보제공</caption>
											<colgroup>
												<col width="15%" />
												<col width="15%" />
												<col width="15%" />
	...
	<c:set var="memNames" value="" />
											<tbody  id="regularTr${statustop.index}">
												...
												<tr id="${statustop.index}tr${status.index}">
													...
													<td><label for="studyTimeArr"  class="hidden">훈련시간</label><input type="text" style="width:95%;" id="studyTimeArr" name="studyTimeArr" value="${resultlist.studyTime}"/></td>
										<script type="text/javascript">
										<!--
										$(document).ready(function() {
										...
										//--> 
												...
												<tr>
													<th><label for="totalReview" >보강총평</label></th>
													<td colspan="6" class="left"><textarea name="review" rows="6" id="totalReview">${TraningEnrichmentTimeVO.review}</textarea></td>
												</tr>
											</tbody>
										</table>
									</div>
