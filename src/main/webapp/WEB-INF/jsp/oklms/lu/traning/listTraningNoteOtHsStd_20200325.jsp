							...
							<dl>
								...
								<dd style="display:block;">
									...
									//--> 
												...
												<td>
												<c:choose>
													<c:when test="${resultlist.state eq 'W'}">승인요청</c:when>
													<c:when test="${resultlist.state eq 'R'}">재승인요청</c:when>
													<c:when test="${resultlist.state eq 'X'}">반려</c:when>
													<c:when test="${resultlist.state eq 'C'}">승인</c:when>
												</c:choose>
												</td>
											</tr>
											</c:forEach>
									...
									<div class="btn-area align-center mt-010">
										<c:choose>
											<c:when test="${resultlist.state eq 'C'}"><a href="javascript:alert('승인 이후에는 수정이 불가능합니다.')" class="yellow">저장</a></c:when>
