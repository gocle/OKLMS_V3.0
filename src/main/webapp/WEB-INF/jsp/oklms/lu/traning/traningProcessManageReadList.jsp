...
<script type="text/javascript">
	...
	function fn_search( param1 ){
		...
		var reqUrl = "/lu/traning/getTraningProcessManageList.do";
...
<input type="hidden" id="uiGubun" name="uiGubun" value="traningProcessCompanyReadPop" />
<input type="hidden" id="returnUrl" name="returnUrl" value="/lu/traning/traningProcessManageReadList" />
...
<ul id="student-popup" style="display:none;">
	<li class="company-area" style="margin-left:-350px; margin-top:-119px;">
		...
		<table class="type-2">
				...
				<tbody>
					<c:forEach var="resultList" items="${resultCompanyList}" varStatus="status">
					<tr>
						<td><input type="radio" name="tempCompanyId" id="tempCompanyId" value="${resultList.companyId}||${resultList.companyName}||${resultList.address}||${resultList.choiceDay}||${resultList.employInsManageNo}"></td>
						<td>${resultList.companyName}</td>
						<td>${resultList.address}</td>
						<td>${resultList.choiceDay}</td>
						<td>${resultList.employInsManageNo}</td>
					</tr>
					</c:forEach>
				</tbody>
			</table><!-- E : 기업체검색 -->
	...
	<table id="styleDisplay1" class="type-2 mt-020">
		<colgroup>
			<col style="width:*" />
			<col style="width:*" />
		</colgroup>
	...
	<div class="btn-area align-right mt-010">
		<a href="#fn_goList" class="black" onclick="javascript:fn_goList();">목록</a>
		<a href="#fn_delete" class="black" onclick="javascript:fn_delete();">삭제</a>
		<a href="#fn_goUpdate" class="black" onclick="javascript:fn_goUpdate();">수정</a>
	</div><!-- E : btn-area -->
</div><!-- E : 훈련과정명 -->
