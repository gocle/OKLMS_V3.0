...
%> 
 						<h2>훈련일지${TraningNoteVO.subjectType eq 'HSKILL' ? '(고숙련)' : ''}</h2>
...
function fn_insert(index,memId){
	...
	$("#frmTraning"+index+" [name='state']").val($("#frmTraning"+index+" [name='stateArr']").val());
	$("#frmTraning"+index+" [name='returnComment']").val($("#frmTraning"+index+" [name='returnCommentArr']").val());
	$("#frmTraning"+index+" [name='memId']").val(memId);
	
	$("#frmTraning"+index+" [name='traniningNoteDetailId']").val($("#frmTraning"+index+" [name='traniningNoteDetailIdArr']").val());
	$("#frmTraning"+index+" [name='traniningNoteMasterId']").val($("#frmTraning"+index+" [name='traniningNoteMasterIdArr']").val());
...
function fn_stateChange(obj,num){
	var val = obj.value;
	if(val == "X"){
		$('#returnComment'+num).attr('readonly', false);
	} else {
		$('#returnComment'+num).attr('readonly', true);
	}
...
<form name="frmTraning" id="frmTraning" method="post">
				...
				<input type="hidden" id="companyId" name="companyId" value="${TraningNoteVO.companyId}" />
							...
							<h4 class="mb-010"><span>${TraningNoteVO.subjectName} / ${TraningNoteVO.subjectCode }</span> (${TraningNoteVO.classId }반_${TraningNoteVO.companyName})ㅣ ${TraningNoteVO.yyyy}학년도 ${TraningNoteVO.termName} </h4>
...
<c:forEach var="timeList" items="${timeList}" varStatus="status">
						...
						<form name="frmTraning${status.index}" id="frmTraning${status.index}" method="post">
							...
							<input type="hidden"  id="traniningNoteDetailId"  name="traniningNoteDetailId" value="" />
							<input type="hidden"  id="traniningNoteMasterId"  name="traniningNoteMasterId" value="" />
							...
							<dl>
								...
								<dd style="display:block;">
									...
									//--> 
												...
												<td>
												<c:choose>
													<c:when test="${!empty resultlist.state}">
														<select name="stateArr" id="state${status.index+1}" onchange="fn_stateChange(this,'${status.index}')">
															<option value="W" ${resultlist.state eq 'W' ? 'selected' : ''}>승인요청</option>
															<option value="R" ${resultlist.state eq 'R' ? 'selected' : ''}>재승인요청</option>
															<option value="X" ${resultlist.state eq 'X' ? 'selected' : ''}>반려</option>
															<option value="C" ${resultlist.state eq 'C' ? 'selected' : ''}>승인</option>
														</select>
													</c:when>
													<c:otherwise>
														미작성
												...
												<td>
												<c:if test="${!empty resultlist.state}">
												<a href="#!" class="yellow" onclick="fn_insert('${status.index}')">저장</a>
												</c:if>
												</td>
												<td>
												<c:if test="${!empty resultlist.state}">
												<input type="text" name="returnCommentArr" id="returnComment${status.index}" readonly="readonly" value="${resultlist.returnComment}">
												</c:if>
												</td>
											</tr>
											</c:forEach>
...
<c:set var="memNames" value="" />
									...
									//--> 
											...
											<tr>
												<th>보강총평</th>
												<td colspan="5" class="left">${TraningEnrichmentTimeVO.review}</td>
											</tr>
										</tbody>
									</table>
 									<div class="btn-area align-center mt-010">
