...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
						...
						<div class="group-area">
							...
							<table class="type-write-view mb-010">
								<colgroup>
	...
	if(${(teamprojectVO.submitType eq 'T')? 'true':'false'}){
		$(".isLeader").each(function(idx){
			$(this).change(function(){
				score = this.value;
				var groupId = "group_id_"+$(this).data("groupId");
				$(".group_id_"+$(this).data("groupId")).each(function(idx){
					//if(idx != 0){
						//this.value = score;
						//$(this).change();
						$("input:checkbox[id='"+groupId+"']").prop("checked", true);
					//}
				});
			});
		});
						...
						<table class="type-2"  id="tblGroup">
							...
							<c:forEach var="result" items="${result}" varStatus="status">
								...
								<tr>
									<td>
									<c:if test="${loginAuthGroupId ne '2016AUTH0000006' }">
									<input type="checkbox" name="arrTeamprojectSubmitId" id="group_id_${result.groupId}" value="${result.teamprojectSubmitId }" <c:if test="${empty result.atchFileId }">disabled="disabled"</c:if> />
									</c:if>
									</td>
									<%-- <td class="left ${result.groupId}"><c:if test='${result.leaderYn eq "Y"}' ><fmt:formatNumber type="number" minIntegerDigits="2"  value="${result.groupId}" />팀</c:if></td> --%>
									...
									<td>
										<c:choose>
											<c:when test="${!empty result.atchFileId }">											
												...
												<c:if test="${loginAuthGroupId ne '2016AUTH0000006' }">
												<input  type="number" min="0" max="9" class="arrEvalScore <c:if test='${result.leaderYn eq "Y"}' >isLeader</c:if> group_id_${result.groupId}" data-group-id="${result.groupId}"  name="arrEvalScore_${result.teamprojectSubmitId }" id="arrEvalScore_${result.teamprojectSubmitId }" style="width:60px; text-align:center;" value="${result.evalScore }" />
												</c:if>
											</c:when>
											<c:otherwise>
												${result.evalScore }
