...
function initHtml() {
		...
		for(var i=0; i < listSubmits.length; i++){
			student = listSubmits[i];
			groupNum = com.addToZero(student.groupId,2);
			
			// 팀 그룹생성
			if(i == 0 || preGroupNum != groupNum){
			...
			// 팀장설정.
			if(student.leaderYn == "Y"){
				$("[name='teamGroup_"+groupNum+"_leaderYn']", $("#teamGroupMemId_"+student.memId) ).attr({"checked":"checked"});
			}
		...
		$("[name*='_leaderYn']").change(function() {
			  $("#isChangeLeader").val("Y");
		});
...
function fn_formSave(){
	...
	if( $("#stuSubmitCnt").val() == 0 ){
		...
		if (  confirm("제출건이 있을 경우에는 팀구성 및 제출구분을\n제외한 항목만 수정이 가능합니다.\n저장 하시겠습니까?" )) {
			...
			$("#projectDesc").val(data);
			var reqUrl =  "/lu/teamproject/updateTeamprojectInfo.do";
			$("#frmTeamproject").attr("target", "_self");
			$("#frmTeamproject").attr("action", reqUrl);
			$("#frmTeamproject").submit();
		}
	...
	if( !(teamCnt.length == 0 || !Number(teamCnt)) ){
		...
		for(var i=1; i <= teamCnt; i++){
			...
			if($("[type='radio'][name='teamGroup_"+groupNum+"_leaderYn']").length == 0){
				...
				if( $("[type='radio'][name='teamGroup_"+groupNum+"_leaderYn']").is(":checked") == false ){
					alert("팀장 선택이 필요합니다.");
					result = false;
					break;
				}
...
<input type="hidden" name="isTeamRenew"  id="isTeamRenew" value="N" />
<input type="hidden" name="isChangeLeader"  id="isChangeLeader" value="N" />
					...
					<div class="group-area">
						<table class="type-write">
							...
							<tbody>
								<tr>
									<th>프로젝트 주제</th>
									<td colspan="3"><input type="text" name="projectName" id="projectName" style="width:97%;" value="${teamprojectVO.projectName}" /></td>
								</tr>
								<tr>
									<th>프로젝트 기간</th>
									<td colspan="3">
										<input type="text" name="projectStartDate" id="projectStartDate"  style="width:65px" value="${teamprojectVO.projectStartDate}" />
										  ~ 
										<input type="text" name="projectEndDate" id="projectEndDate"  style="width:65px" value="${teamprojectVO.projectEndDate}" />
									</td>
								</tr>
								<tr>
									<th>과제제출 기간</th>
									<td colspan="3">
										<input type="text" name="submitStartDate" id="submitStartDate" style="width:65px" value="${teamprojectVO.submitStartDate}"  />
										  ~ 
										<input type="text" name="submitEndDate" id="submitEndDate"  style="width:65px" value="${teamprojectVO.submitEndDate}"  />
									</td>
								</tr>
								<tr>
									<th>제출구분</th>
									<td>
										<input type="radio" name="submitType" value="T" class="choice" <c:if test="${teamprojectVO.submitType eq 'T'}">checked="checked"</c:if>/>&nbsp;&nbsp;팀장만 제출
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="submitType" value="I" class="choice" <c:if test="${teamprojectVO.submitType eq 'I'}">checked="checked"</c:if> />&nbsp;&nbsp;개별 제출
									</td>
									<th>지각제출</th>
									<td>
										<input type="radio" name="submitLateYn" value="Y" class="choice" <c:if test="${teamprojectVO.submitLateYn eq 'Y'}">checked="checked"</c:if> />&nbsp;&nbsp;허용
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="submitLateYn" value="N" class="choice" <c:if test="${teamprojectVO.submitLateYn eq 'N'}">checked="checked"</c:if> />&nbsp;&nbsp;불허
									</td>
								</tr>
								<tr>
									<th>평가</th>
									<td>
										<input type="radio" name="evalYn" value="Y" class="choice" <c:if test="${teamprojectVO.evalYn eq 'Y'}">checked="checked"</c:if> />&nbsp;&nbsp;Y
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="evalYn" value="N" class="choice" <c:if test="${teamprojectVO.evalYn eq 'N'}">checked="checked"</c:if> />&nbsp;&nbsp;N
									</td>
									<th>배점</th>
									<td><input type="number" name="score" id="score"  min="0" max="9" style="width:30px; text-align:right;" value="${teamprojectVO.score}"   /> 점
									</td>
								</tr>
								<tr>
									<th>내용</th>
									<td colspan="3"><textarea name="projectDesc" id="projectDesc" rows="5">${teamprojectVO.projectDesc}</textarea></td>
								</tr>
								<tr>
									<th>첨부파일</th>
									<td colspan="3">
								...
								<tr>
									<th>팀구성</th>
									<td colspan="3">
										<input type="radio" name="compositionType" value="N" class="choice" <c:if test="${teamprojectVO.compositionType eq 'N'}">checked="checked"</c:if> />&nbsp;&nbsp;학번
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="compositionType" value="C" class="choice" <c:if test="${teamprojectVO.compositionType eq 'C'}">checked="checked"</c:if> />&nbsp;&nbsp;기업
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="radio" name="compositionType" value="M" class="choice" <c:if test="${teamprojectVO.compositionType eq 'M'}">checked="checked"</c:if> />&nbsp;&nbsp;수동
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										(총인원:${currProcVO.count })
										<input type="text" name="teamCnt" id="teamCnt"  style="width:30px" value="${teamprojectVO.teamCnt}" /> 팀 
										<span id="custTeamArea" style="width: auto; display:none;" >
										(<span id="numberOfTeamMembers" style="width: auto;" ></span>명/1팀)
										</span>
										<span id="createBtn" style="width: auto;" >
										&nbsp;&nbsp;<a href="#!" class="btn-full-blue" onclick="generateTeam();">생성</a>
										</span>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="checkbox" name="teamLeaderAutoYn" id="teamLeaderAutoYn" value="Y" class="choice" <c:if test="${teamprojectVO.teamLeaderAutoYn eq 'Y'}">checked="checked"</c:if> /> 팀장 자동선정
									</td>
								</tr>
							</tbody>
						</table>
						...
						<div class="btn-area align-right mt-010">
							<a href="/lu/teamproject/listTeamproject.do" class="gray-1 float-left">취소</a><a href="#" onclick="javascript:fn_formSave();" class="orange">수정</a>
						</div><!-- E : btn-area -->
