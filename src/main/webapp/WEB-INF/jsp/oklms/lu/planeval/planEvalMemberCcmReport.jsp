...
<input type="hidden" id="reportGubun" name="reportGubun" value="MEMBER" />
	...
	<dl id="tab-type">
		...
		<dd id="tab-content-12" style="display:block;">
			...
			<table class="type-2 mt-010">
				...
				<tbody>
					<c:forEach var="result1" items="${resultMemberRebortList}" varStatus="status">
					<tr>
						<td><input type="radio" id="tmpSubjectCode" name="tmpSubjectCode" value="${result1.yyyy}||${result1.term}||${result1.subjectCode}||${result1.subClass}||${result1.memId}"></td>
						<td>${result1.subjectTraningType}</td>
						<td>${result1.yyyy}</td>
						<td>${result1.termName}</td>
						<td class="left">${result1.subjectName}</td>
						<td>${result1.subClass}</td>
						<td>${result1.memId}</td>
						<td>${result1.memName}</td>
					</tr>
					</c:forEach>
					<c:if test="${fn:length(resultMemberRebortList) == 0}">
					<tr>
			...
			<div class="btn-area align-right mt-010">
				<!-- 최선호과장요청으로 주석처리함. -->
				<!-- <a href="#!" onclick="javascript:fn_report(); return false" class="gray-3">직무수행능력평가결과 (수강자별) 출력</a> -->
			</div><!-- E : btn-area -->
		</dd>
	</dl>
