...
<div id="">
	...
	<div class="search-box-1 mt-020">
		...
		<select id="searchTerm" name="searchTerm" onchange="">
			...
			<option value="3" <c:if test="${planEvalVO.searchTerm eq '3' }" > selected="selected"  </c:if>>여름학기</option>
			<option value="4" <c:if test="${planEvalVO.searchTerm eq '4' }" > selected="selected"  </c:if>>겨울학기</option>
		</select>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<c:choose>
					<c:when test="${result.subjectTraningType eq 'OFF'}">
						Off-JT
					</c:when>
					<c:otherwise>
						OJT
