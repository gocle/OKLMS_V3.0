					...
					<div id="scroller" style="width:${fn:length(listOjtClassName)*128}px;">
						<ul id="thelist" class="name-tab-btn">
							...
							<li <c:if test="${result.stdSeq eq planEvalVO.memSeq and result.subjectClass eq planEvalVO.subClass}"> class="current" </c:if>>
								<a href="#!" onclick="javascript:fn_goMember('${result.subjectClass}','${result.stdSeq }','${status.count }'); return false">${result.subjectClass}분반_${result.stdName }</a>
							</li>
							</c:forEach>
						</ul>
					</div>
...
<c:if test="${subjectType != 'HSKILL'}">
	<dl id="tab-type-2">
		...
		<div class="group-area-title">
			<h4 class="mb-010"><span>${subjectVO.subjectName} / ${subjectVO.subjectCode}</span> (${subjectVO.subjectClass}반) ㅣ ${subjectVO.yyyy}학년도 ${subjectVO.termName}</h4>
			<p>
			<a href="#learner-wrap_area" rel="modal:open" class="btn btn-black form-control mmd btn-radius50">개설교과 분반 검색 <i class="xi-search"></i></a>
			<a href="#learner-wrap_area1" rel="modal:open" class="btn btn-black form-control mmd btn-radius50">학습자 검색 <i class="xi-search"></i></a>
			</p>
		</div>
		<table class="type-1 mt-010">
			<colgroup>
