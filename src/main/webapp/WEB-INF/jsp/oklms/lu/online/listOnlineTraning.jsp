...
<script type="text/javascript">
	...
	var attCnt = '${onlineTraningVO.attCnt}'; // 학습진행카운트
	//var attCnt = '0'; // 학습진행카운트임시 무조건 학습자없음
	...
	function fn_opener_add_schedule( data, weekCnt, courseContentId, divisionNum, contentName, code, minute){
		...
		if(divisionNum > 0){
			...
			for (var i = 0; i < jsonData.body.list.length; i++) {		
				...
				if(registered_from_device_type_code == "1"){
			    	registered_from_device_type_code_name = "PC+M";
			    ...
			    for (var j = 0; j < list.lesson.item_list.length; j++) {
			    	...
			    	for (var z = 0; z < item_list.subitem_list.length; z++) {
			    		...
			    		// 클립정보를 담는다
			    		var data = new Object();
			    		data.lesson_id =  list.lesson_id;
			    		data.lesson_item_id =  subitem_list.lesson_item_id;
			    		data.lesson_sub_item_id =  subitem_list.id;
			    		data.required_learning_time_in_seconds =  subitem_list.required_learning_time_in_seconds;
			    		
			    		lessonList.push(data);
			    		
			    		//console.log("lesson_id : "+list.lesson_id);
			    		//console.log("lesson_item_id : "+subitem_list.lesson_item_id);
			    		//console.log("lesson_sub_item_id : "+subitem_list.id);
			    		//console.log("required_learning_time_in_seconds : "+subitem_list.required_learning_time_in_seconds);
			    		minute += parseInt(subitem_list.required_learning_time_in_seconds);
			    	}
			    }
			    
   				var jsonLessonData = JSON.stringify(lessonList) ;
			    
			    //console.log("============== jsonLessonData : "+jsonLessonData);
			    
			    
			    minute = Math.floor(minute / 60); 
			    if(minute == "undefined") minute = 0;
			    
			    var weekStartDate = $("#weekStDate"+weekNum).val();
			    var weekEndDate = $("#weekEdDate"+weekNum).val();
				...
				schStr+="	<input type='text' name='subjTitles' value='"+list.title+"' />";
				...
				schStr+="	<input type='hidden' name='jsonLessonDatas' value='"+jsonLessonData+"' />";
			...
			for (var i = 0; i < jsonData.body.list.length; i++) {		
			    ...
			    var weekStartDate = $("#weekStDate"+weekCnt).val();
			    var weekEndDate = $("#weekEdDate"+weekCnt).val();
				...
				schStr += "<td class='left'>";
				//schStr += "<input type='text' name='weekSchStDates' id='week_sch_st_date_"+(rowLen + (i+1))+"' value=\""+weekStartDate+"\"  readonly style='width:65px' />~<br />";
				//schStr += "<input type='text' name='weekSchEdDates' id='week_sch_ed_date_"+(rowLen + (i+1))+" value=\""+weekEndDate+"\"  readonly style='width:65px' />";
				
				schStr += "<input type='text' name='weekSchStDates' id='week_sch_st_date_"+(rowLen + (i+1))+"' value=\""+weekStartDate+"\" readonly style='width:65px' />~<br />";
				schStr += "<input type='text' name='weekSchEdDates' id='week_sch_ed_date_"+(rowLen + (i+1))+"' value=\""+weekEndDate+"\"  readonly style='width:65px' />";
				
				schStr += "</td>";
				schStr += "<td><input type='text' name='weekSchMins'  id='week_sch_min_"+(rowLen + (i+1))+"'  class='week_min_"+weekCnt+"' value='"+minute+"'  maxlength='3' value='' style='width:25px; text-align:center;' /></td>";
				schStr += "<td><a href='alert(1)'  class='btn-search-gray '></a></td>";
				//schStr += "<td><a href='javascript:goPreview(\"Y\",\""+weekCnt+"\",\"\",\""+schNum+"\",\""+list.title+"\",\""+list.lesson_id+"\",\"\",\"\",'\"\",\"\");'  class='btn-search-gray '></a></td>";
				schStr += "<td><a href='#none' onclick='removeRow(this);' class='btn-line-gray'>삭제</a></td>";
				//schStr += "<td><a href='#!' class='btn-line-yellow'>편집</a></td>";
	...
	function fn_content_list_pop(weekCnt) { 
		...
		$("#frmLesson").attr("target", "popSearch");
		$("#frmLesson").submit();
	} 
	
	function fn_content_edit_pop(rowId) { 
		popOpenWindow("", "popSearch", 1000, 750);
		var reqUrl = "/lu/online/popupOnlineTraning.do";
		$("#rowId").val(rowId);
		$("#frmLesson").attr("action", reqUrl);
	...
	function fn_content_data_pop(weekCnt) { 
		popOpenWindow("", "popSearch", 1000, 750);
		var reqUrl = "/lu/online/popupOnlineTraningData.do";
		$("#weekCnt").val(weekCnt);
		$("#frmLesson").attr("action", reqUrl);
		$("#frmLesson").attr("target", "popSearch");
		$("#frmLesson").submit();
			...
			com.ajax(reqUrl, param, function(obj, data, textStatus, jqXHR){	
				if (200 == jqXHR.status ) {
					...
					if(retCd == 10000 ){
						...
						if($("#subjOnStandSeq").val() == null || $("#subjOnStandSeq").val() == ""){
							alert("등록에 실패했습니다.")
						}
	...
	function fn_saveOnSchedule(){
		...
		if(weekLen > 0){
			for( var i=0; i < weekLen; i++){
				...
				if($('input:text[name=weekMins]').eq(i).val() == ""){
					...
					var weekMins = parseInt($('input:text[name=weekMins]').eq(i).val());
					var subjPoint = parseInt($("#subjPoint").val());
					if(subjPoint > 0){
						var weekCheckMin = 25 * subjPoint;
						if(weekMins < weekCheckMin){
							weekMsg += (i+1)+"주차,";
						}
					}
				}
				...
				weekSchDelim += $(".add_sch_"+(i+1)).find("input:text[name='subjTitles']").length+"|";
			}
		}
		...
		// 학습 진행중인 학습자가 없을때는 새로등록
		if(attCnt == 0){
			if(weekMsg != ""){
				var subjPoint = parseInt($("#subjPoint").val());
				var weekCheckMin = 25 * subjPoint;
				weekCheckMsg = weekMsg.substring(0,(weekMsg.length-1))+"의 학습시간이 "+weekCheckMin+"분 미만입니다.\n\n이대로 저장하시겠습니까?\n\n학습 근로자 학습 시작 후에는 기본정보만 수정이 가능합니다.";
			} else {
				weekCheckMsg = "온라인 훈련 계획서를 등록 하시겠습니까?\n학습 근로자 학습 시작 후에는 기본정보만 수정이 가능합니다.";
			}
			
			
			
			if(confirm(weekCheckMsg)){
				var reqUrl = CONTEXT_ROOT + "/lu/online/insertOnlineTraningWeekSchedule.json";
			 	var param = $("#frmWeekSch").serializeArray();
				com.ajax(reqUrl, param, function(obj, data, textStatus, jqXHR){	
					if (200 == jqXHR.status ) {
						var retData 	= jqXHR.responseJSON.retData;
						var retCd 	= jqXHR.responseJSON.retCd;
			...
			var reqUrl = CONTEXT_ROOT + "/lu/online/updateOnlineTraningWeekSchedule.json";
		 	var param = $("#frmWeekSch").serializeArray();
			com.ajax(reqUrl, param, function(obj, data, textStatus, jqXHR){	
				if (200 == jqXHR.status ) {
					var retData 	= jqXHR.responseJSON.retData;
	...
	function fn_subjectList(){
		var reqUrl = "/lu/currproc/listCurrProc.do";
		$("#formSubject").attr("action", reqUrl);
		$("#formSubject").submit();
	}
	...
	function fn_copy(){
		...
		$("#frmWeekSch").attr("target", "_self");
		$("#frmWeekSch").submit();
	}
	
	function fn_file_pop(weekId) { 
		popOpenWindow("", "popFile", 650, 450);
		var reqUrl = "/lu/online/popupOnlineTraningFile.do";
		$("#file_weekId").val(weekId);
		//$("#rowId").val(rowId);
		$("#frmFile").attr("action", reqUrl);
		$("#frmFile").attr("target", "popFile");
		$("#frmFile").submit();
	} 
	...
	<form id="formSubject" name="formSubject" method="post">
	...
	<form id="frmTraning" name="frmTraning" method="post">
		<input type="hidden" id="c_yyyy" name="c_yyyy" value=""/>
		<input type="hidden" id="c_term" name="c_term" value=""/>
		<input type="hidden" id="c_subjectCode" name="c_subjectCode" value=""/>
		<input type="hidden" id="c_subjectClass" name="c_subjectClass" value=""/>
	</form>
	 -->
			...
			<li class="preview-area" id="preview-area">
				...
				<div id="preview-container">
					...
					<div id="preview-movie-zone">
						...
						<iframe id="contentFrame" width="1000px" height="660px" frameborder="0" allowfullscreen title="preview-movie-zone"></iframe>
	...
	<div class="group-area">
		<table class="type-1 mb-005  responsive-tr">
			<caption>교과형태 과정구분 훈련시간 담당교수 온라인교육형태 선수여부에 대한 정보 제공</caption>
			<tbody>
				<tr>
		...
		<form id="frmStand" name="frmStand" method="post">
			...
			<table class="type-1 " style="border-top:1px solid #DDD;">
				<caption>학습기간 출석기준 학점기준 진도율 계산방식에 대한 정보 제공</caption>
				<colgroup>
				...
				<tbody>
					<tr>
						...
						<td>
							- 전체 학습기간 : 
							<label for="startDate" class="hidden">전체 학습기간 시작일 </label>
							<input type="text" name="startDate" id="startDate" value="${onlineTraningVO.startDate}" readonly="readonly"  /> ~
							<label for="endDate" class="hidden">전체 학습기간 종료일 </label>
							<input type="text"name="endDate" id="endDate" value="${onlineTraningVO.endDate}" readonly="readonly" />
							
							<div class="mt-005 display-block">
							- <label for="weekStudyType1">주차 학습기간 :  </label>
							...
							<label for="weekStudy" class="hidden">주차선택</label>
							...
							&nbsp;&nbsp;<label for="weekStudyType2"><input type="radio" name="weekStudyType"  id="weekStudyType2" ${onlineTraningVO.weekStudyType eq 'FREE' ? 'checked' : ''} class="choice" value="FREE"/> FREE</label>
							</div>
						</td>
					</tr>
					<tr>
						<th scope="row">출석기준</th>
						<td>
							<div class="display-block">
							- <label for="attendProgress">출석 : 진도율&nbsp;&nbsp;</label>
							...
							<div class="mt-005 display-block">
							- <label for="cutProgress">결석 : 진도율&nbsp;&nbsp;</label> 
					...
					<tr>
						<th scope="row"><label for="scheduleTime">학점기준</label></th>
						<td>
					...
					<tr>
						<th scope="row">진도율 계산방식</th>
						<td>
							<label for="progressAttendTypeCode1"><input type="radio" name="progressAttendTypeCode" id="progressAttendTypeCode1" value="H" ${onlineTraningVO.progressAttendTypeCode eq 'H' ? 'checked' : ''}  class="choice" /> 시간+페이지</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<label for="progressAttendTypeCode2"><input type="radio" name="progressAttendTypeCode" id="progressAttendTypeCode2" value="T" ${onlineTraningVO.progressAttendTypeCode eq 'T' ? 'checked' : ''}  class="choice" /> 시간</label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<label for="progressAttendTypeCode3"><input type="radio" name="progressAttendTypeCode" id="progressAttendTypeCode3" value="P" ${onlineTraningVO.progressAttendTypeCode eq 'P' ? 'checked' : ''}  class="choice" /> 페이지</label>
						</td>
					</tr>
				<tbody>
			</table>
		...
		<div class="btn-area mt-010">
			<div class=" float-right">
				<a href="#none" onclick="fn_saveOnStand();" class="orange">설정 저장</a>
			</div>
		</div>
	</div>
	...
	<form id="frmWeekSch" name="frmWeekSch" method="post">
		...
		<input type="hidden" id="frmWeekSch_yyyy" name="yyyy" value="${subjectVO.yyyy}" /> 
		<input type="hidden" id="frmWeekSch_term" name="term" value="${subjectVO.term}"/>
		<input type="hidden" id="frmWeekSch_subjectCode" name="subjectCode" value="${subjectVO.subjectCode}"/>
		<input type="hidden" id="frmWeekSch_subjectClass" name="subjectClass" value="${subjectVO.subjectClass}"/>
		...
		<div class="table-responsive mt-020">
		<table class="type-2" id="weekTable">
			<caption>주차 차시 콘텐츠명 학습기간 학습시간 미리보기 보조자료등의 정보 제공</caption>
			<colgroup>
				<col style="width:3%" />
				<col style="width:5%" />
				<col style="width:5%" />
			...
			<tr id="week_first">
				...
				<th scope="col">삭제</th>
				<th scope="col">콘텐츠</th>
				<th scope="col">보조자료</th>
				<th scope="col">지원기기</th>
				<th scope="col">이동</th>
			</tr>
		
			<c:forEach var="result" items="${resultList}" varStatus="status">
				<tr name="week" class="week_${result.weekCnt}">
					<td><label for="choice" class="hidden">선택</label><input type="checkbox" name="" id="choice" value="" class="choice" /></td>
					<td name="weekText">${result.weekCnt}주차</td>
					<td></td>
					<td class="left">
					<label for="contentName${result.weekCnt}" class="hidden">콘텐츠명</label>
					<input type="text" name="contentNames" id="contentName${result.weekCnt}" value="${result.contentName}" style="width:100%;" />
					...
					<td>
						<label for="weekStDate${result.weekCnt}" class="hidden">학습기간 시작일</label>
						<input type="text" name="weekStDates" id="weekStDate${result.weekCnt}" value="${result.weekStDate}"  style="width:95px" readonly />~
						<label for="weekEdDate${result.weekCnt}" class="hidden">학습기간 종료일</label>
						<input type="text" name="weekEdDates" id="weekEdDate${result.weekCnt}" value="${result.weekEdDate}" style="width:95px" readonly />
					</td>
					<td>
						<label for="week_min_${result.weekCnt}" class="hidden">학습시간</label>
						<input type="text" name='weekMins'  id="week_min_${result.weekCnt}" value="${result.weekMin}" maxlength="3" style="width:100%; text-align:center;" />
					</td>
					...
					<td><a class="btn-line-blue" href="#!" onclick="fn_file_pop('${result.weekId}');" >등록</a></td>
					<td></td>
				...
				<c:forEach var="sch" items="${scheduleList}" varStatus="sch_status">
					...
					<c:if test="${result.weekCnt eq sch.weekCnt}">
						<tr class="add_sch_${status.count}" id="row_${sch_status.count}" name="add_sch">
						<td></td>
						<td>ㄴ</td>
						<td name="week_sch_text">${sch.subjStep}차시</td>
						<td class="left">
						<label for="subjTitles${sch_status.count}" class="hidden">콘텐츠명</label>
						<input name="subjTitles" id="subjTitles${sch_status.count}" type="text" value="${sch.subjTitle}" style="width:60%;">	
						
						<input name="linkContentTypes" id="linkContentTypes${sch_status.count}" type="hidden" value="${sch.linkContentYn eq 'Y' ? 'CMS' : 'LMS'}">	
						<input name="cmsCourseContentIds" id="cmsCourseContentIds${sch_status.count}" type="hidden" value="${sch.cmsCourseContentId}">	
						<input name="cmsLessonIds" id="cmsLessonIds${sch_status.count}" type="hidden" value="${sch.cmsLessonId}">	
						<input name="cmsIds" id="cmsIds${sch_status.count}" type="hidden" value="${sch.cmsId}">	
						<input name="contentTypes" id="contentTypes${sch_status.count}"  type="hidden" value="${sch.contentType}">	
						<input name="contentsDirs" id="contentsDirs${sch_status.count}" type="hidden" value="${sch.contentsDir}">	
						<input name="contentsIdxFiles" id="contentsIdxFiles${sch_status.count}" type="hidden" value="${sch.contentsIdxFile}">	
						<input name="contentsRealFiles" id="contentsRealFiles${sch_status.count}" type="hidden" value="${sch.contentsRealFile}">	
						<input name="urls" id="urls${sch_status.count}" type="hidden" value="${sch.url}">	
						<input name="deviceTypeCodes" id="deviceTypeCodes${sch_status.count}" type="hidden" value="${sch.deviceTypeCode}">
						<input name="jsonLessonDatas" id="jsonLessonDatas"${sch_status.count}  type="hidden" value='${sch.jsonLessonData}'>
						<input name="subjSchIds" id="subjSchIds"${sch_status.count}  type="hidden" value='${sch.subjSchId}'>
						</td>
						<td>
						<!-- 과정복사 파라메터가 있을경우 차시별 시작시간은 주차기주능로 세팅 -->
						<c:choose>
							<c:when test="${!empty param.cYyyy and !empty param.cTerm and !empty param.cSubjectCode and !empty param.cSubjectClass}">
								<label for="week_sch_st_date_${sch_status.count}" class="hidden">학습기간 시작일</label>
								<input name="weekSchStDates"  id="week_sch_st_date_${sch_status.count}" value="${result.weekStDate}"  style="width: 95px" type="text" readonly>~
								
								<label for="week_sch_ed_date_${sch_status.count}" class="hidden">학습기간 종료일</label>
								<input name="weekSchEdDates" id="week_sch_ed_date_${sch_status.count}" value="${result.weekEdDate}"  style="width: 95px" type="text" readonly>
							</c:when>
							<c:otherwise>
								<label for="week_sch_st_date_${sch_status.count}" class="hidden">학습기간 시작일</label>
								<input name="weekSchStDates"  id="week_sch_st_date_${sch_status.count}" value="${sch.startDate}"  style="width: 95px" type="text" readonly>~
								
								<label for="week_sch_ed_date_${sch_status.count}" class="hidden">학습기간 종료일</label>
								<input name="weekSchEdDates" id="week_sch_ed_date_${sch_status.count}" value="${sch.endDate}"  style="width: 95px" type="text" readonly> 
							</c:otherwise>
						</c:choose>
						</td>
						<td>
							<label for="week_sch_min_${sch_status.count}" class="hidden">학습시간</label>
							<input name="weekSchMins" class="week_min_1" id="week_sch_min_${sch_status.count}" value="${sch.studyTime}" style="width: 100%; text-align: center;" type="text" maxlength="3">
						</td>
						<td><a class="btn-search-gray" href="javascript:goPreview('${sch.linkContentYn}','${result.weekCnt}','${result.contentName}','${sch.subjStep}','${sch.subjTitle}','${sch.cmsLessonId}','${sch.contentType}','${sch.contentsDir}','${sch.contentsIdxFile}','${sch.url}');"><span class="hidden">미리보기</span></a></td>
						<td><a class="btn-line-gray" onclick="removeRow(this);" href="#none">삭제</a></td>
						<%-- <td><a class="btn-line-yellow" href="#none" onclick="fn_content_edit_pop('${sch_status.count}');">편집</a></td> --%>
						 <td><!-- <a class="btn-line-yellow" href="#none" >편집</a> --></td>
						<td></td>
						<td>${sch.deviceTypeCodeName}</td>
						<td><a onclick="moveSchUp(this);" href="#none"><img src="/images/oklms/std/inc/arrow_top.png" alt="위로"></a><br><a onclick="moveSchDown(this);" href="#none"><img src="/images/oklms/std/inc/arrow_down.png" alt="아래로"></a>
						</td>
						</tr>
					</c:if>
				</c:forEach>
	...
	<div class="btn-area mt-010">
		<div class="float-left">
			<a href="#fn_subjectList" onclick="fn_subjectList();" class="gray-1">교과목 정보</a>
		</div>
		<div class="float-right">
