...
<script type="text/javascript" src='/js/oklms/cmsPreview.js'></script>
<script type="text/javascript">
	...
	var tabNum = '${param.tabNum}'; //교과 학점정보
	...
	$(document).ready(function() {
		...
		//초기 셋팅
		$(document).on("click",".list-zone dt",function(){  
			$(this).parent().siblings().find('dd').slideUp();
			$(this).siblings('dd').slideToggle();
		});

		$(document).on("click",".list-zone dd a",function(){  	
	...
	function fn_add_schedule(type){
		...
		var openerWeekStartDate = $(opener.document).find("#weekStDate"+weekCnt).val(); 
		var openerWeekEndDate = $(opener.document).find("#weekEdDate"+weekCnt).val(); 
		if(type == "CMS"){
			...
			if( contentCnt == 0 && schCnt == 0  ){
				...
				if(contentCnt > 0){
					...
					$('input:checkbox[name="sch"]:checked').each(function (index) {  
						...
						var registered_from_device_type_code = chkVal.split('|')[5];
						...
						schStr+="<input type='hidden' name='jsonLessonDatas' value='"+chkVal.split('|')[7]+"' />";
						
						schStr += "</td>";
						
						schStr += "<td class='left'>";
						schStr += "<input type='text' name='weekSchStDates' id='week_sch_st_date_"+(rowLen + (index+1))+"' value='"+openerWeekStartDate+"' readonly style='width:65px' />~<br />";
						schStr += "<input type='text' name='weekSchEdDates' id='week_sch_ed_date_"+(rowLen + (index+1))+"' value='"+openerWeekEndDate+"'  readonly style='width:65px' />";
						schStr += "</td>";
						schStr += "<td><input type='text' name='weekSchMins'  id='week_sch_min_"+(rowLen + (index+1))+"' class='week_min_"+weekCnt+"'  maxlength='3' value='"+chkVal.split('|')[6]+"' style='width:25px; text-align:center;' /></td>";
						//schStr += "<td><a href='#none'  class='btn-search-gray '></a></td>";
					    schStr += "<td><a href='javascript:goPreview(\"Y\",\""+weekCnt+"\",\"\",\""+(rowLen + (index+1))+"\",\""+chkVal.split('|')[1]+"\",\""+chkVal.split('|')[3]+"\",\"\",\"\",\"\",\"\");'  class='btn-search-gray '></a></td>";
			...
			if($("#movieType").val() ==  "MOVIE"){
				...
				schStr+="		<input type='hidden' name='jsonLessonDatas' />";
				
				schStr +="</td>";
				
				schStr += "<td class='left'>";
				schStr += "<input type='text' name='weekSchStDates' id='week_sch_st_date_"+(rowLen +1)+"' value='"+openerWeekStartDate+"'  readonly style='width:65px' />~<br />";
				schStr += "<input type='text' name='weekSchEdDates' id='week_sch_ed_date_"+(rowLen +1)+"' value='"+openerWeekEndDate+"' readonly style='width:65px' />";
				schStr += "</td>";
				...
				//schStr += "<td><a href='#none'  class='btn-search-gray '></a></td>";
				schStr += "<td><a href='javascript:goPreview(\"N\",\""+weekCnt+"\",\"\",\""+(rowLen +1)+"\",\""+$("#movieSchTitle").val()+"\",\"\",\"MP4\",\""+$("#contentsDir").val()+"\",\""+$("#contentsIdxFile").val()+"\",\"\");'  class='btn-search-gray '></a></td>";
				schStr += "<td><a href='#none' onclick='removeRow(this);' class='btn-line-gray'>삭제</a></td>";
				//schStr += "<td><a href='#!' class='btn-line-yellow'>편집</a></td>";
				schStr += "<td></td>";
				//schStr += "<td><a href='#!' onclick='#' class='btn-line-blue'>등록</a></td>";
				...
				if($("#url").val().length >= 7 && $("#url").val().length < 20){
					alert("잘못 된 URL 입니다.");
					$("#url").focus();
					return;
				}
				...
				schStr += "<td><input type='text' name='weekSchMins'  id='week_sch_min_"+(rowLen +1)+"' class='week_min_"+weekCnt+"'  maxlength='3' value='' style='width:25px; text-align:center;' /></td>";
				//schStr += "<td><a href='#none'  class='btn-search-gray '></a></td>";
				schStr += "<td><a href='javascript:goPreview(\"N\",\""+weekCnt+"\",\"\",\""+(rowLen +1)+"\",\""+$("#urlSchTitle").val()+"\",\"\",\"URL\",\"\",\"\",\""+$("#url").val()+"\");'  class='btn-search-gray '></a></td>";
	...
	function addTree(data, rowId){
		...
		for (var i = 0; i < jsonData.body.list.length; i++) {
			...
			var required_learning_time_in_seconds = 0;
		    for (var j = 0; j < list.lesson.item_list.length; j++) {
		    	...
		    	for (var z = 0; z < item_list.subitem_list.length; z++) {
		    		...
		    		required_learning_time_in_seconds += parseInt(subitem_list.required_learning_time_in_seconds);
		    	}
		    }
		    ...
		    required_learning_time_in_seconds = Math.floor(required_learning_time_in_seconds / 60); 
		    
		    
		    if(required_learning_time_in_seconds == "undefined") minute = 0;
		    
		    var chkVal = "CMS|"+ cleanQueryTerm(list.title) +"|"+rowId+"|"+list.lesson_id+"|"+list.id+"|"+registered_from_device_type_code+"|"+required_learning_time_in_seconds+"|"+jsonLessonData;
		    ...
		    treeStr+="	<input type='hidden' name='jsonLessonDatas' value='"+jsonLessonData+"' />";
	...
	function fn_search( param1 ){
		...
		$("#frmPop").attr("action", reqUrl);
		$("#frmPop").attr("target", "_self");
		$("#frmPop").submit();
	}
	
	function fn_cmsInit( param1 ){
		$("#pageIndex").val( param1 );
		
		if(confirm("최신 버전으로 동기화 하시겠습니까?")){
			var reqUrl = CONTEXT_ROOT + targetUrl + "popupOnlineTraningCmsInit.do";
	...
	function fn_search_tab(tabNum){
		$("#tabNum").val(tabNum);
	}
	
	function fn_search_subject( ){
		var reqUrl = CONTEXT_ROOT + targetUrl + "popupOnlineTraning.do";
	...
	function fn_subject_copy(c_yyyy,c_term,c_subjectCode,c_subjectClass){
		
		$(opener.document).find("#c_yyyy").val( c_yyyy );
		$(opener.document).find("#c_term").val( c_term );
		$(opener.document).find("#c_subjectCode").val( c_subjectCode );
		$(opener.document).find("#c_subjectClass").val( c_subjectClass );
		
		$(opener.location).attr("href", "javascript:fn_copy();"); //jquery 이용
	}
...
<form id="frmPop" name="frmPop" method="post">
	...
	<input type="hidden" id="subjPoint" name="subjPoint" value="${param.subjPoint}" />
			...
			<dl id="tab-type">
				...
				<dd id="tab-content-11" style="display:block;">
					<div class="search-box-1 mb-010">
						...
						<input type="text" name="searchStDate" id="searchStDate" value="${cmsCourseBaseVO.searchStDate}" readonly="readonly" style="width:100px" />  ~<input type="text" name="searchEdDate" id="searchEdDate" value="${cmsCourseBaseVO.searchEdDate}" readonly="readonly" style="width:100px" /> 
						<input type="text" name="searchWrd" id="searchWrd" value="${cmsCourseBaseVO.searchWrd}" onkeypress="press(event);"" style="width:150px"/>
						<a href="#" onclick="fn_search('1');" style="width:60px">검색</a>&nbsp;&nbsp;<a href="#" onclick="fn_cmsInit('1');">CMS 동기화</a>
					</div><!-- E : search-box-1 -->
				...
				<dd id="tab-content-12">
					...
					<table class="type-write">
						<tbody>
							...
							<tr id="movie2">
								...
								<td>
									<input type="text" name="movieSchTitle" id="movieSchTitle">
								</td>
							</tr>
							<tr id="movie3">
								...
								<td>
									...
									</span> <span id="movieName">0KB / 200M</span>
								</td>
							</tr>
					...
					<div class="btn-area mt-010">
						...
						<dt class="tab-name-13"><a href="javascript:showTabbtn3();fn_search_tab('3');">기존 과정</a></dt>
						<dd id="tab-content-13">
							
								<form id="frmSubject" name="frmSubject" method="post">
				<input type="hidden" id="tabNum" name="tabNum" value="${param.tabNum}" />
							
							<div class="search-box-1 mb-010">
							<%--
								<select name="yyyy" id="yyyy"> 
									<option value="">학년도</option>
										<option value="2018" ${param.yyyy eq '2018' ? 'selected' : ''} >2018학년도</option>
										<option value="2017" ${param.yyyy eq '2017' ? 'selected' : ''} >2017학년도</option>
										<option value="2016" ${param.yyyy eq '2016' ? 'selected' : ''}>2016학년도</option>
										<option value="2015" ${param.yyyy eq '2015' ? 'selected' : ''}>2015학년도</option>
								</select>
		 					--%>
								<select name="yyyy" id="yyyy">  
									<option value="" >학년도</option>
								...
								<select name="term" id="term">
										...
										<option value="1" ${param.term eq '1' ? 'selected' : ''}>1학기</option>
										<option value="2" ${param.term eq '2' ? 'selected' : ''}>2학기</option>
										<option value="3" ${param.term eq '3' ? 'selected' : ''}>여름학기</option>
										<option value="4" ${param.term eq '4' ? 'selected' : ''}>겨울학기</option>
									</select>
								<select name="subjectGrade" id="subjectGrade">
											<option value="">대상학년</option>
											<option value="1" ${param.subjectGrade eq '1' ? 'selected' : ''}>1학년</option>
											<option value="2" ${param.subjectGrade eq '2' ? 'selected' : ''}>2학년</option>
											<option value="3" ${param.subjectGrade eq '3' ? 'selected' : ''}>3학년</option>
											<option value="4" ${param.subjectGrade eq '4' ? 'selected' : ''}>4학년</option>
										</select>
								<select name="point" id="point">
											<option value="">학점</option>
											<option value="1" ${param.point eq '1' ? 'selected' : ''}>1학점</option>
											<option value="2" ${param.point eq '2' ? 'selected' : ''}>2학점</option>
											<option value="3" ${param.point eq '3' ? 'selected' : ''}>3학점</option>
											<option value="4" ${param.point eq '4' ? 'selected' : ''}>4학점</option>
										</select>
								<input type="text" name="searchKeyword" value="${param.searchKeyword}" placeholder="교과명 검색" maxlength="15" style="width:20%"/>
								<a href="#!" onclick="fn_search_subject();">검색</a>
							</div><!-- E : search-box-1 -->
							...
							<div class="table-responsive">
								...
								<c:forEach var="result" items="${onlineList}" varStatus="status">						
									<tr>
											<td>${result.yyyy}학년도</td>
											<td>${result.termName}</td>
											<td>${result.subjectGrade}학년</td>
											<td>${result.point}학점</td>
											<%-- <td class="left"><a href="#!" class="text">+ ${result.subjectName}</a></td> --%>
											<td class="left"> ${result.subjectName}</td>
											<td><a href="#!" onclick="fn_subject_copy('${result.yyyy}','${result.term}','${result.subjectCode}','${result.subjectClass}');"  class="btn-full-blue">과정복사</a></td>
										</tr>
								</c:forEach>	
							...
							<div class="btn-area mt-010">
								<div class="float-right">
									<!-- <a href="javascript:alert('추가할 콘텐츠가 없습니다.')" class="orange">목록에 추가</a> -->
									<a href="#" onclick="window.self.close();" class="gray-3">닫기</a>
								</div>
							</div><!-- E : page-num-btn -->
							</form>
