...
<script type="text/javascript">
	...
	function fn_formSave(){
		...
		var checkedVals = com.getCheckedVal('check0','timeIds');
		if(checkedVals == ''){
			alert("변경 할 시간표에 체크 해주시길 바랍니다.");
			return;
		}
		
		$("#checkedVals").val(checkedVals);
		...
		if (confirm("변경신청한 입력값대로 변경 하시겠습니까?")) {
			...
			<c:forEach var="result" items="${resultTraningChangeReadList}" varStatus="scriptStatus2">
				...
				if($("#changeLeadTime-"+${scriptStatus2.count}).val() < 0 ){
					$("#changeLeadTime-"+${scriptStatus2.count}).focus();
					alert("신규 변경신청 목록에 변경 후 (신청내용) 소요시간이 잘못되어 있습니다.");
					return false;
				}
			</c:forEach>
	...
	function fn_formChoiceSave(){
		...
		if (confirm("변경신청 하시겠습니까?")) {
			var reqUrl = CONTEXT_ROOT + "/lu/traningChange/updateTraningChangeScheduleApplicationChoice.do";
	...
	//변경 신청건 저장
	function fn_formTimeSave(paramIdx){
		...
		if (confirm("변경신청한 입력값대로 변경 하시겠습니까?")) {
			...
			$("#paramIdx").val(paramIdx); 
			var reqUrl = CONTEXT_ROOT + "/lu/traningChange/updateTraningChangeScheduleApplicationTime.do";
			$("#frmTraning").attr("action", reqUrl);
			$("#frmTraning").submit();
		}
	}
	...
	function fn_time_add(index){
		var stHour = $("#changeTraningStHour-"+index).val();
		var edHour = $("#changeTraningEdHour-"+index).val();
		if(stHour != "" && edHour != ""){
			$("#changeLeadTime-"+index).val(Number(edHour-stHour))
		}
	}
...
<input type="hidden" id="paramIdx" name="paramIdx" value="" />
<input type="hidden" id="checkedVals" name="checkedVals" value="" />

<div id="">
<h2>훈련시간표 변경신청</h2>

	<h4 class="mt-010"><span>${currProcReadVO.subjectName}</span> (${currProcReadVO.subClass}반) ㅣ ${currProcReadVO.yyyy}학년도 ${currProcReadVO.termName}</h4>
	...
	<div class="group-area mt-010">
		...
		<table class="type-2">
			...
			<thead>
				<tr>
					<th rowspan="2">선택<br/>
					<input type="checkbox" name="checkbox" id="check0" name="check0" class="choice" onclick="javascript:com.checkAll('check0','timeIds');" />
					</th>
					<th rowspan="2">주차</th>
			...
			<tbody>
				...
				<tr>
					<td><input type="checkbox"  id="timeIds-${status.count}" name="timeIds" value="${status.count}" class="choice" /></td>
					...
					<td><input type="text" id="changeTraningDate-${status.count}" name="changeTraningDate-${status.count}"  value="${result.traningDate}" style="width:85px;" readonly="readonly"/></td>
					<td>
						<select id="changeTraningStHour-${status.count}" name="changeTraningStHour-${status.count}" onchange="fn_time_add(${status.count});" style="margin-bottom:3px;">
							<!-- <option value="" selected="selected">시작시간</option> -->
							<c:forEach var="timeHourCd" items="${timeHourCode}" varStatus="statusCode">
								<option value="${timeHourCd.codeId}"<c:if test="${timeHourCd.codeId eq result.traningStHour}">selected="selected" </c:if>>${timeHourCd.codeName}:00분</option>
							</c:forEach>
						</select>
						<select id="changeTraningEdHour-${status.count}" name="changeTraningEdHour-${status.count}" onchange="fn_time_add(${status.count});" style="margin-bottom:3px;">
							<!-- <option value="" selected="selected">종료시간</option> -->
							<c:forEach var="timeHourCd" items="${timeHourCode}" varStatus="statusCode">
								<option value="${timeHourCd.codeId}" <c:if test="${timeHourCd.codeId eq result.traningEdHour}">selected="selected" </c:if>>${timeHourCd.codeName}:00분</option>
							</c:forEach>
						</select>
					</td>
					<td>
						<input type="number" min="0" max="10" name="changeLeadTime-${status.count}" id="changeLeadTime-${status.count}" class="arrEvalScore"  value="${result.leadTime}" onchange="javascript:com.checkNumber(document.getElementById('changeLeadTime-${status.count}'));" style="width:40px; text-align:center;" />
						<input type="hidden" id="timeId-${status.count}" name="timeId-${status.count}" value="${result.timeId}" />
					</td>
					
					<%-- <td><a href="javascript:fn_formTimeSave('${status.count}');" class="btn-line-blue" style="width:40px;">변경신청</a></td> --%>
		...
		<!-- 	<a href="#fn_formChoiceSave" onclick="javascript:fn_formChoiceSave();" class="orange" >변경신청</a> -->
