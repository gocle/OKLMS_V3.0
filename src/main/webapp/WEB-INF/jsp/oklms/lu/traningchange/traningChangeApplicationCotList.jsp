	...
	<div class="group-area mt-020">
		...
		<table class="type-2">
			...
			<tbody>
				...
				<tr>
					<td><input type="radio" id="traningProcessIdSelect" name="traningProcessIdSelect" value="${result.yyyy},${result.term},${result.subjectCode},${result.subClass}" class="choice"  checked /></td>
					<td>${status.count}</td>
					<td>${result.updateDate}</td>
					<td>${result.memName}</td>
					<td class="left">${result.changeReason}</td>
					<td class="left">${result.retunReason}</td>
		...
		<div class="btn-area mt-010">
			<div class="float-left">
				<c:if test="${fn:length(resultList1) == 0}">
				<a href="#fn_cret" onclick="javascript:fn_cret(); return false" class="yellow" onkeypress="this.onclick();">신규 변경신청</a>
				</c:if>	
			</div>		
