...
<input type="hidden" name="state"  value="${activityVO.state}" >
								 ...
								 <div class="training-info">
										 <div class="txt-box">
											<dl>
												<dt><label for="weekCnt"></label>주차</dt>
												<dd>
								...
								<div class="search-box-1 mt-020">
									<label for="searchName">성명:</label>
									<input type="text" name="searchName" id="searchName" value="${activityVO.searchName}"/>
									<a href="#!" onclick="javascript:fn_search_search();">검색</a>
								</div><!-- E : search-box-1 -->
								<div class="table-responsive mt-030">
									<table class="type-2">
										<caption>성명 계획훈련시간 길제훈련시간 과제물 정보제공</caption>
										<colgroup>
											<col style="width:10%" />
										...
										<tr>
											...
											<c:choose>
												<c:when test="${result.outYn eq 'Y'}">
													<a href="#!" id="${result.memId}" onclick="javascript:showTabbtn2();$('#current_${status1.count}').click();" class="text"><font color="red">${result.memName }</font> </a>
												</c:when>
												<c:otherwise>
													<a href="#!" id="${result.memId}" onclick="javascript:showTabbtn2();$('#current_${status1.count}').click();" class="text">${result.memName } </a>
												</c:otherwise>
											</c:choose>
											...
											<td>
												...
												<c:if test="${!empty result.content }">
													<a href="javascript:showTabbtn2();$('#current_${status1.count}').click();" class="btn-line-gray">작성</a>
												</c:if>
											</td>
											<td>
							...
							<c:set var="sumNumber" value="0" />
								<c:forEach var="result" items="${memberVO.arrActivityVO}" varStatus="status">
									...
									<table class="type-write mt-005" style="border-top:1px solid #DDD;">
										<caption>교육내용 요청사항 과제물첨부에 대한 정보 제공</caption>
										<colgroup>
											<col width="20%" />
											<col width="10%" />
											<col width="*" />
