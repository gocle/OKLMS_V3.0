...
<form name="frmActivity${status.count}" id = "frmActivity${status.count}" method="post" enctype="multipart/form-data" >
...
<input type="hidden" name="state"  value="${activityVO.state}" >
								...
								<tr>
									...
									<c:choose>
										<c:when test="${currProcVO.subjectTraningType eq 'OJT' }">
											<c:choose>
												<c:when test="${result.status eq '1'}">승인대기</c:when>
												<c:when test="${result.status eq '2'}">승인</c:when>
												<c:when test="${result.status eq '3'}">반려</c:when>
											</c:choose>
										</c:when>
										<c:otherwise>
						...
						<div class="btn-area align-right mt-010">
							<c:if test="${result.activityNoteId eq null}">
								<a href="#!" onclick="javascript:fn_formSave('frmActivity${status.count}');" class="orange">등록</a>
							</c:if>
							<c:if test="${result.activityNoteId ne null}">
								<a href="#!" onclick="javascript:fn_formSave('frmActivity${status.count}');" class="orange">수정</a>
							</c:if>							
						</div><!-- E : btn-area -->
