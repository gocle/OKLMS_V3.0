...
<c:set var="interviewNoteDateMmTemp" value="${result.interviewNoteDateMm }"/> 
						<table class="type-write  mb-020">
							...
							<tr>
								<th rowspan="9" class="sub-name">
								${result.interviewNoteDateMm } 월
								</th>
 ...
 <form:form modelAttribute="frmInterview" name="frmInterview" method="post">
	...
	<input type="hidden" name="yyyy" id="yyyy"  value="${interviewVO.yyyy }" />
	<%-- <input type="hidden" name="mm" id="mm" value="${interviewVO.mm }" /> --%>
...
function fn_move(month){
	var reqUrl =  "/lu/interview/getInterviewCenter.do";
	$("#interviewNoteDateMm").val($("#yyyy").val()+"."+month);
	$("#frmInterview").attr("target", "_self");
	$("#frmInterview").attr("action", reqUrl);
	$("#frmInterview").submit();		
}
...
//--> 
						...
						<div class="btn-area align-right mt-010">
							...
							<c:if test="${!empty preMonth }">
								<a href="#!" onclick="javascript: fn_move('${preMonth }');" class="gray-1 float-center">이전</a>
							</c:if>
							<c:if test="${!empty nxtMonth }">
								<a href="#!" onclick="javascript: fn_move('${nxtMonth }');" class="gray-1 float-center">다음</a>
							</c:if>
