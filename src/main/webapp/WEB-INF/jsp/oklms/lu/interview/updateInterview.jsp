							...
							<tbody>
								...
								<tr>
									...
									<c:if test="${!empty resultFile2}">
											...
											<a href="javascript:com.deleteFile('${resultFile2.atchFileId}|${resultFile2.fileSn}', '/lu/interview/goUpdateInterview.do?interviewNoteId=${result.interviewNoteId}' )">											 
								...
								<tr>
									...
									<c:if test="${!empty resultFile1}">
											...
											<a href="javascript:com.deleteFile('${resultFile1.atchFileId}|${resultFile1.fileSn}', '/lu/interview/goUpdateInterview.do?interviewNoteId=${result.interviewNoteId}' )">
											<img src="/images/oklms/std/inc/icon_del.png" />
											</a>
									</c:if>	
									<c:if test="${empty resultFile1}">
...
$(document).ready(function() {
	...
	$.ajax({
		  ...
		  data: { "interviewMemberSeqs":listSeq,"companyId": "${result.companyId }","pageIndex":"1"},
...
function fn_memberLList( pageIndex){
	...
	$.ajax({
		  type: "POST",
		  url: "/lu/interview/ajaxInterviewMember.do",
		  data: {"interviewMemberSeqs":listSeq, "companyId": "${result.companyId }","pageIndex": pageIndex},
		  success:function( html ) {
			$( "#interviewMemberList" ).html( html );
			showLearningpop();
		  }
...
function fn_formSave(){ 
	if (  confirm("수정 하시겠습니까?")) {
		var reqUrl =  "/lu/interview/updateInterview.do";
