...
<c:set var="rowspanNum" value="0"/>
<c:forEach var="result" items="${resultlist}" varStatus="status">

<c:if test="${!empty result.traningProcessName }">
<c:set var="rowspanNum" value="${rowspanNum+1}"/>
</c:if>
<c:if test="${!empty result.interviewMemName}">
<c:set var="rowspanNum" value="${rowspanNum+1}"/>
</c:if>
<c:if test="${!empty result.interviewMemberNames}">
<c:set var="rowspanNum" value="${rowspanNum+1}"/>
</c:if>
...
<c:if test="${!empty result.totalTalk}">
<c:set var="rowspanNum" value="${rowspanNum+1}"/>
</c:if>
...
<c:if test="${status.first}">							
								<th rowspan="${rowspanNum}" class="sub-name">${result.interviewNoteDateMm } 월</th>
</c:if>								
...
<c:if test="${!empty result.totalTalk}">
							<tr>
								...
								<td><textarea name="textarea"  >${result.totalTalk }</textarea></td>
							</tr>
</c:if>							
<c:if test="${!empty result.tempSj1}">
							<tr>
