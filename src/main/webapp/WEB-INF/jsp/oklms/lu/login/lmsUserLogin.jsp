...
<%
	...
	session1.setAttribute("NOT_USER_YN","N");
...
// 		});
	...
	function fn_board_pop(param1, param2){
		...
		$("#bbsProcBodyForm").attr("action", reqUrl);
		$("#bbsProcBodyForm").attr("target", "popSearch");
		$("#bbsProcBodyForm").submit();
	}
	...
	*/
	function fn_formValidate() {
		...
		var chk2Cnt = $("input[name=check-2]:checkbox:checked").length;
		...
		var chk3Cnt = $("input[name=check-3]:checkbox:checked").length;
	...
	function fn_formSave(){
		if (fn_formValidate()) {
			...
			if(confirm("회원가입 하시겠습니까?\n센터전담자 승인 후 로그인이 가능합니다.")){
				
				var reqUrl = "/commbiz/outmember/inesertTempOutMember.json";
	...
	function fn_showId(){
		...
		$("#id-find").css('display', 'inline-block');
	}
	
	function memIdSearch(){
		if( $("#frmId_memName").val() == "" ){
			alert("이름을 입력하세요.");
			$("#frmId_memName").focus();
			return;
		}
		...
		reqUrl = "/commbiz/member/listSearchMemberId.json";  
		var html = "";
		$.ajax({
			url:reqUrl,
			type:"post",
			...
			success:function(data){
				...
				html += "<table class='type-2 mt-020 wp100'>";
				html += "<caption>아이디 찾기 아이디 가입일을 입력해주세요</colgroup>";
				html += "<colgroup>";
				html += "<col style='*'>";
				html += "<col style='*'>";
				html += "<colgroup>";
				...
				if(data.resultList.length > 0){
					for(var i=0; i<data.resultList.length; i++){
						html += "<tr>";
						html += "<td><label for=''>"+data.resultList[i].memId+"</label></td>";
						html += "<td>"+data.resultList[i].insertDate+"</td>";
						html += "</tr>";
					}
					//fn_modalClose();
					
					//$('.jquery-modal').css('display','block');
					//$("#id-find2").css('display', 'inline-block');
				} else {
					html += "<tr>";
					html += "<td><td colspan='2'>일치하는 회원정보가 없습니다.</td></td>";
					html += "</tr>";
					//fn_modalClose();
	...
	function fn_initMemPassword(){
		...
		reqUrl = "/commbiz/member/initMemPassword.json";  
		$.ajax({
			url:reqUrl,
			type:"post",
			...
			success:function(data){
				if(data.retCd == "SUCCESS"){
					//alert("회원님의 비밀번호를 아이디와 같이 초기화 하였습니다.");
					alert(data.retEmail+"으로 비밀번호를 전송했습니다.");
				} else if(data.retCd == "FALE"){
					alert("회원님의 비밀번호를 초기화 하는대 실패 하였습니다.");
				} else {
    ...
    function getInitialConsonant(char) {
        ...
        return INITIAL_CONSONANTS[cho];
    }

    // 초성 문자열로 변환
    function getInitials(str) {
        return [...str].map(ch => getInitialConsonant(ch)).join('');
    }
    ...
    function filterSelect() {
        ...
        for (let i = 0; i < select.options.length; i++) {
            ...
            const matchInitial = initials.includes(keyword);
            const matchText = text.includes(keyword);

            // 초성 또는 일반 텍스트가 포함될 경우 표시
            option.style.display = (matchInitial || matchText) ? "" : "none";
        }
    }
	...
	<form id="loginProcBodyForm" method="post" action="/commbiz/login/loginProc.do" class="gLogin">
	    ...
	    <header id="header">
	        ...
	        <div class="main-header">
	            <div class="container">
	               <div class="bookmark">
	                   <a href="" onclick="javascript:bookmarksite('oklms','http://oklms.koreatech.ac.kr/');"><i class="xi-star"></i> 즐겨찾기추가</a>
	               </div>
	                ...
	                <div class="logo">
	                    <h1>
	                        <a href="">
	                            <span class="mobile-hide"><img class="" src="/images/oklms/std/main_2020/logo.png" alt="" /></span>
	                            <!-- <span class="pc-hide"><img class="logo-color" src="/renewalb/assets/images/common/logo-color.svg" alt="" /></span> -->
	                        </a>
	                    </h1>
	                </div>
	                <!-- /Logo -->
    ...
    <form id="bbsProcBodyForm" method="post" class="gLogin">
		<input id="bbsProcBodyForm_reqUri" name="reqUri" type="hidden" value="${reqUri }">
		<input id="bbsProcBodyForm_menuArea" name="menuArea" type="hidden" value="LMS">
    </form>
    ...
    <div id="content" class="clearfix">
		<div id="main">
			<div class="container">
				<!-- 메인 1 -->
				<div class="row clearfix" id="main1">
					<div class="box-container">
						<!-- slogan -->
						<div class="box-cell slogan col-lg-">
							<div><img class="" src="/images/oklms/std/main_2020/slogan-text.png" alt="" /></div>
							<div class="h1">일하고, 배우며, 함께 성장하는 <span>일학습병행교육</span></div>
							<div class="slogan-text">
								<p>일과 학습을 동시에!<br />취업난도 해결하고 기업이 원하는 맞춤형 인력을 양성할 수 있는 제도</p>
								<br />
								<a href="" class="btn btn-xl btn-outline" onfocus="">
									이용자매뉴얼 <i class="xi-angle-right-min"></i>
								</a>
							</div>
						</div>
						<!-- //slogan -->
						
						<div class="box-cell col-lg-"></div>
						
						<!-- login-area -->
						<div class="box-cell col-lg-" id="main-login">
							<form action="login-form-area" id="gLogin" class="gLogin">
								<div id="login" class="login-area gLogin">
									<h2>MEMBER LOGIN</h2>
									<div class="form-group">
										<label for="bodyMemId" class="iLabel sound-only">아이디</label><input type="text" name="memId" type="text" id="bodyMemId" placeholder="아이디" class="iText uid form-control"/>
									</div>
									
									<div class="form-group">
										<label for="bodyMemPasswordEncript" class="iLabel sound-only">비밀번호</label><input type="password" name="memPasswordEncript" type="password" id="bodyMemPasswordEncript" placeholder="비밀번호" class="ipass upw form-control"/>
									</div>
									<div class="text-center">
                                      	<input name="login-btn" type="button" value="로그인" onclick="javascript:fnAfterCheckLogin();return false;"  class="btn btn-lg"/>
                                    </div>

									
									<div class="text-center search-btn-box">
										<div class="btn-box1">
				...
				<!--  아이디 찾기 팝업 -->
				<div class="modal hauto" id="id-find">
					<div class="modal-title">아이디 찾기</div>
					<form method="post" name="frmId" id="frmId">         
					<div class="modal-body hauto" id="id_body">
							<ul class="horizontal_form type2 mt-010">
								<li>
									<div>
										<label class="jt_label" for="frmId_memName">이름</label>
									</div>
									<div>
										<input type="text" class="form_field jt_form_full_field" id="frmId_memName" name="memName" maxlength="10" value="" placeholder="">
									</div>
								</li>
								<li>
									<div>
										<label class="jt_label" for="frmId_email">이메일</label>
									</div>
									<div>
										<input type="text" class="form_field jt_form_full_field" id="frmId_email" name="email" value="" maxlength="30" >
									</div>
								</li>
							</ul>
							...
							<div class="btn-area  align-center mt-020">
								<!-- <a href="#id-find2" rel="modal:open"   class="btn btn-black">확인</a> -->
								<a href="javascript:memIdSearch();"  class="btn btn-black">확인</a>
							</div>
							<!-- //버튼 -->
				...
				<div class="modal hauto" id="id-find2" style="display:none;">
					...
					<div class="modal-body hauto">
						...
						<table class="type-2 mt-020 wp100">
							<caption>아이디 찾기 아이디와 가입일을 입력해주세요</caption>
							<colgroup>
								<col style="*">
								<col style="*">
							</colgroup>
					...
					<div class="modal-body hauto">
						<form method="post" name="frmPw" id="frmPw">            
							<ul class="horizontal_form type2 mt-010">
								<li>
									<div>
										<label class="jt_label" for="frmPw_memId">아이디</label>
									</div>
									<div>
										<input type="text" class="form_field jt_form_full_field" id="frmPw_memId" name="memId" maxlength="20" placeholder="">
									</div>
								</li>
								<li>
									<div>
										<label class="jt_label" for="frmPw_memName">이름</label>
									</div>
									<div>
										<input type="text" class="form_field jt_form_full_field" id="frmPw_memName" name="memName" maxlength="20"  value="" placeholder="">
									</div>
								</li>
								<li>
									<div>
										<label class="jt_label" for="frmPw_email">이메일</label>
									</div>
									<div>
										<input type="text" class="form_field jt_form_full_field" id="frmPw_email" name="email" maxlength="30" placeholder="">
									</div>
								</li>
							</ul>
							...
							<!-- 버튼 -->
							<div class="btn-area  align-center mt-020">
								<a href="javascript:fn_initMemPassword();"  class="btn btn-black">확인</a>
							</div>
							<!-- //버튼 -->
						</form>
					</div>
							...
							<ul class="horizontal_form type2 mt-010">
								<li class="fs">
									<div style="padding-bottom:12px;">
										<label for="frmMobile">
											<input type="radio" id="frmMobile" name="frmMobile" value=""> 회원정보에 등록한 휴대전화로 인증<br /><span class="pl-015">(+82-01-6***-4***)</span>
										</label>
									</div>
								</li>
								<li class="fs">
									<div style="padding-bottom:12px;">
										<label for="frmEmail">
											<input type="radio" id="frmEmail" name="frmEmail" value=""> 회원정보에 등록한 이메일로 인증<br /><span class="pl-015">(asddfdf@g****.com)</span>
										</label>
									</labe>
								</li>
							</ul>
					...
					<div class="modal-body hauto">
						...
						<form method="post" action="" enctype="" onsubmit="">            
							<ul class="horizontal_form type3 mt-010">
								...
								<li class="fs">
									<div><label class="jt_label" for="newPw">새 비밀번호</label></div>
									<div>
										<div class="form_control_wrap">
											<input type="password" class="form_field jt_form_full_field" id="newPw" name="" value="" placeholder="8자 이상">
										</div>
										<span class="explain"> 영문, 숫자, 특수문자를 함께 사용하면 보다 안전합니다.</span>
									</labe>
								</li>
								<li class="fs">
									<div><label class="jt_label" for="newPw2">새 비밀번호 확인</label></div>
									<div>
										<div class="form_control_wrap">
											<input type="password" class="form_field jt_form_full_field" id="newPw2" name="" value="" placeholder="">
										</div>
									</labe>
								</li>
							</ul>
				...
				<div id="join-wrap" class="modal large">
					...
					<div class="modal-body hauto">
						<form method="post"  id="frmJoin" name="frmJoin" action ="" enctype="multipart/form-data">      
							...
							<ul class="horizontal_form">
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="companyId">기업</label>
									</div>
									<div>
										<div class="form_control_wrap">
											...
											<input style="margin-top: 5px;" class="form_field jt_form_full_field" type="text" id="searchInput" maxlength="1" onkeyup="filterSelect()" placeholder="예: ㄱ / 가나 / 주식회사"/>
										</div>
										<span class="explain">초성이나 검색어로 선택 가능합니다.</span>
									</div>
								</li>
								...
								<li class="contact_row_w350x fs">
									<div>
										<label class="jt_label" for="">이수교육</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<label for="compLicenceCd1"><input type="radio" name="compLicenceCd" id="compLicenceCd1" value="1">단기(온라인)</label>
											<label for="compLicenceCd2"><input type="radio" name="compLicenceCd" id="compLicenceCd2" value="2">기본</label>
											<label for="compLicenceCd3"><input type="radio" name="compLicenceCd" id="compLicenceCd3" value="3">심화</label>
										</div>
									</div>	
								</li>
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="loginId">아이디</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="koreanNm">이름</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="loginPwd1">비밀번호</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="loginPwd2">비밀번호확인</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="mbtlnum">휴대폰번호</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" id="mbtlnum" name="mbtlnum" value="" placeholder="" onkeydown="onlyNumber(this);" maxlength="11">
										</div>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="memBirth">생년월일</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" name="memBirth" id="memBirth"  maxlength="8" value="" onkeydown="onlyNumber(this);" placeholder="">
										</div>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="compJoinDay">입사일</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" name="compJoinDay" id="compJoinDay"  maxlength="8" onkeydown="onlyNumber(this);" value="" placeholder="">
										</div>
										<span class="explain"> 8자리 입력(예:19821024)</span>
									</div>
								</li>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="email">이메일</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="compTelNo">사무실번호</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" name="compTelNo" id="compTelNo" value="" maxlength="11" onkeydown="onlyNumber(this);"  placeholder="">
											<span class="explain">'-' 없이 입력</span>
										</div>
									</div>	
								</li>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="dutyNm">직위</label>
									</div>
									<div>
										<div class="form_control_wrap">
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="compCareerYear">경력</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" name="compCareerYear" id="compCareerYear"  maxlength="2" onkeydown="onlyNumber(this);" value="" placeholder="">
										</div>
									</div>
								</li>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="compLicenceNm">자격명</label>
									</div>
									<div>
										<div class="form_control_wrap">
											<input type="text" class="form_field jt_form_full_field" name="compLicenceNm" id="compLicenceNm"  maxlength="10" value="" placeholder="">
										</div>
									</div>
								</li>
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="compEduLevelCd">학력</label>
									</div>
									<div>
										<div class="form_control_wrap">
											
											<select id="compEduLevelCd" name="compEduLevelCd" class="form_field jt_form_full_field" >
								...
								<li class="contact_row_w350x">
									<div>
										<label class="jt_label" for="licenceFile">첨부파일</label>
									</div>
									<div>
										<div class="form_control_wrap">
							...
							<div class="agree_check box07  mt-020">
								...
								<div class="mt-020">
									<label class="check-2" for="check-2">
									...
									<br />
									<label class="check-3" row="check-3">
				...
				<div class="clearfix" id="main2">
					<div class="box-container2">
						<!--  공지사항 -->
						<div class="box-cell-left bg-white" id="main-news">
							<h2>공지사항 <div class="btn-more"><a href="#!" onclick="fn_board_list_pop('BBSMSTR_000000000000');" title="더 보기"><i class="xi-align-justify"></i><span class="hidden">공지사항 더보기</span></a></div></h2>
							<ul class="list-group">
								<c:if test="${fn:length(noticeResultList) == 0}">
									<li class="list-group-item">등록된 공지사항이 없습니다.</li>
								</c:if>
						...
						<!--  주요학사일정  -->
						<div class="box-cell-left bg-blue" id="schedule">
							<h2>주요학사일정 <div class="btn-more fw"><a href="#!" onclick="fn_board_list_pop('BBSMSTR_000000000004');" title="더 보기"><i class="xi-align-justify"></i><span class="hidden">주요학사일정 더보기</span></a></div></h2>
							<ul class="list-group">
								<c:if test="${fn:length(academicCalendarResultList) == 0}">
									<li class="list-group-item">등록된 주요학사일정이 없습니다.</li>
								</c:if>
						...
						<!-- popup-area -->
						<div class="box-cell-left col-lg- bg-mint"  id="popupzone">
		...
		<div class="clearfix" id="main-icon">
			<div class="container">
				<!--  바로가기 메뉴 -->
				<div class="quick-area">
					<div class="item">
						<a href="#!" onclick="fn_board_list_pop('BBSMSTR_000000000010');">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon01.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">자주묻는 질문</p>
						</a>
					</div>
					
					<div class="item">
						<a href="http://email.koreatech.ac.kr" target="_blank" title="새창열림">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon02.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">웹메일</p>
						</a>
					</div>
					
					<div class="item">
						<a href="http://www.koreatech.ac.kr/kor/sub04_04_02.do" target="_blank" title="새창열림">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon03.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">제증명</p>
						</a>
					</div>
					
					<div class="item">
						<a href="http://help.koreatech.ac.kr" target="_blank" title="새창열림">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon04.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">원격지원</p>
						</a>
					</div>
					
					<div class="item">
						<a href="http://dasan.koreatech.ac.kr" target="_blank" title="새창열림">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon05.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">다산정보관</p>
						</a>
					</div>
					
					<div class="item">
						<a href="http://knw.koreatech.ac.kr" target="_blank" title="새창열림">
							<div class="icon-item">
								<img src="/images/oklms/std/main_2020/icon06.png" align="absmiddle" alt="" />
							</div>
							<p class="mt-020">웹디스크</p>
						</a>
					</div>
				</div>
				<!--  //바로가기 메뉴 -->
				...
				<div id="main-link">
					<ul>
						<li>
							<a href="http://www.moel.go.kr" target="_blank" title="새창열림"><img src="/images/oklms/std/main_2020/banner01.jpg" alt="고용노동부" /></a>
						</li>
						<li>
							<a href="http://www.koreatech.ac.kr" target="_blank" title="새창열림"><img src="/images/oklms/std/main_2020/banner02.jpg" alt="한국기술교육대학교" /></a>
						</li>
						<li>
							<a href="http://www.hrd.go.kr" target="_blank" title="새창열림"><img src="/images/oklms/std/main_2020/banner03.jpg" alt="통합 HRD-Net" /></a>
						</li>
						<li>
							<a href="http://www.bizhrd.net" target="_blank" title="새창열림"><img src="/images/oklms/std/main_2020/banner04.jpg" alt="기업일학습" /></a>
						</li>
					</ul>
				</div>
				<!-- // main-link -->
