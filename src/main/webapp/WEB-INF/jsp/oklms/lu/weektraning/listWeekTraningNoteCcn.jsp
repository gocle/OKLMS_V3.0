...
$(document).ready(function() {
	...
	$.ajax({
		  ...
		  url: "/lu/interview/ajaxInterviewCompanyTraning.do",
...
function fn_detail(weekCnt , traningType){ 
	...
	var reqUrl = "/lu/weektraning/getWeekTraningNoteCcn.do";
	$("#frmWeekTraning").attr("action", reqUrl);
	$("#frmWeekTraning").attr("target","_self");
	$("#frmWeekTraning").submit();	 
}
...
function sms(){
	/*
	if(!$('input:radio[name=smsWeekCnt]').is(':checked')){
		alert("선택된 주차가 없습니다.");
		return;
	}
	*/
    var weekCnt = $(":radio[name='smsWeekCnt']:checked").val();
      
	
	var cotArr = $("input:checkbox[name=cotMemId]:checked").val();
	var ccmArr = $("input:checkbox[name=ccmMemId]:checked").val();
	
	var sumArr = "";
	if(cotArr){
		sumArr = cotArr;
		if(!ccmArr){
			sumArr  = sumArr+","+ccmArr;
		}	
	}else if(ccmArr){
		sumArr = ccmArr;
	}else{
		alert("현장교사 또는 HRD담당자가 없습니다.");
		return false;
	}
	fn_send_sms_cot(weekCnt,sumArr,'${workCertVO.endTime}','','');
}
...
function fn_company_search(){
	...
	//$("#rowId").val(rowId);
	$("#frmWeekTraning").attr("action", reqUrl);
	$("#frmWeekTraning").attr("target", "popSearchCompany");
	$("#frmWeekTraning").submit();
}
...
<input type="hidden" name="addyn" id="addyn"  />
						...
						<table class="type-2 mt-010">
							...
							<tbody  id="tableList">
								...
								<tr>
									<td><input type="radio" name="smsWeekCnt"  value="${result.weekCnt}" class="choice" /></td>
...
<c:set var="cotMemId" value="" />
<c:set var="ccmMemId" value="" />
<c:forEach  var="result" items="${cotList}" varStatus="status">
	<c:set var="cotMemId" value="${cotMemId},${result.memId }" />
</c:forEach>
<c:forEach  var="result" items="${ccmList}" varStatus="status">
	<c:set var="ccmMemId" value="${ccmMemId},${result.memId }" />
</c:forEach>

						<div class="btn-area align-right mt-010">
							<p>
								<input type="checkbox" name="cotMemId" value="${cotMemId}" class="choice" /> 기업현장교사&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="checkbox" name="ccmMemId" value="${ccmMemId}" class="choice" /> HRD담당자
							</p>
							
							<a href="#!" onclick="javascript:sms();" class="yellow">SMS 발송</a>					
 							<a href="#!" onclick="javascript:fn_excel();" class="black">엑셀저장</a>	
							<a href="#!" onclick="javascript:fn_print();" class="black">출력</a>
						</div>
