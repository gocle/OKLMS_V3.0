...
<c:set var="tempTraningDate" value="0" />	
<c:set var="minTraningDate" value="0" />
<c:set var="maxTraningDate" value="0" />
<c:set var="minTraningDateStr" value="" />
<c:set var="maxTraningDateStr" value="" />
<c:set var="companyNameStr" value="" />


<c:forEach var="resultList" items="${resultList}" varStatus="status">
 
	<c:set var="tempTraningDate" value="${fn:replace(resultList.traningDate, '.', '')}" />
<c:if test="${empty companyNameStr}">
	<c:set var="companyNameStr" value="${resultList.companyName}" />
</c:if>
	<c:if test="${status.first}">
	<c:set var="minTraningDate" value="${fn:replace(resultList.traningDate, '.', '')}" />
	<c:set var="maxTraningDate" value="${fn:replace(resultList.traningDate, '.', '')}" />
	<c:set var="minTraningDateStr" value="${resultList.traningDate }" />
	<c:set var="maxTraningDateStr" value="${resultList.traningDate }" />	
	</c:if>
	
	<c:if test="${tempTraningDate+0 < minTraningDate+0}" >
	<c:set var="minTraningDate" value="${tempTraningDate}" />
	<c:set var="minTraningDateStr" value="${resultList.traningDate }" />	
	</c:if>
	<c:if test="${tempTraningDate+0 > maxTraningDate+0}" >
	<c:set var="maxTraningDate" value="${tempTraningDate}" />
	<c:set var="maxTraningDateStr" value="${resultList.traningDate }" />	
	</c:if>	
						...
						<table  class="mt-020" style="width:100%">
							...
							<tr>
								<td style="width:60%" >
									<div>No. ${traningNoteVO.yyyy}-${traningNoteVO.term}-${traningNoteVO.weekCnt} </div>
								...
								<td style="width:40%" >
									...
									<c:forEach var="detailTraningNotePrd" items="${detailTraningNotePrd}" varStatus="status">
										...
										<li>(협약)기업명&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;${companyNameStr} </li>
										<li>직무분야&nbsp;&nbsp;: - </li>																														
										<li>기&nbsp;&nbsp;간&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp; ${minTraningDateStr}&nbsp; ~ &nbsp;${maxTraningDateStr}</li>
						...
						<table class="type-2 mt-020">
							...
							<c:forEach var="resultList" items="${resultList}" varStatus="status">
							<c:set var="newtempId" value="${resultList.subjectCode}${resultList.classId}${resultList.traningDate}" />
								<tr>
								...
								<c:set var="allReview" value="${allReview}${resultList.review}<br/>" />
									...
									<td rowspan="${resultList.llCount}"> OJT	</td>
								...
								<c:set var="tempId" value="${resultList.subjectCode}${resultList.classId}${resultList.traningDate}" />
							</c:forEach>
