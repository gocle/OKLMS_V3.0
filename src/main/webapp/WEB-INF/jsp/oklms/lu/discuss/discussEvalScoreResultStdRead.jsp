			...
			<table class="type-2">
				...
				<tbody>
					...
					<tr>
						...
						<td>
						<!-- 학과전담자일 경우 점수는 텍스트로 변경 -->
							<c:choose>
								<c:when test="${discussVO.sessionMemType eq 'CDP'}">
									${result.evalScore }
								</c:when>
								<c:otherwise>
									<input type="text"  class="arrEvalScore"  name="arrEvalScore" id="arrEvalScore" style="width:40px; text-align:center;" value="${result.evalScore }" />
								</c:otherwise>
							</c:choose>
