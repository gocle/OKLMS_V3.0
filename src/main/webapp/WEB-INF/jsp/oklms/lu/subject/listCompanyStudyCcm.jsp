...
//         com.pageNavi( "pageNavi", totalCount , pageSize , pageIndex );
	...
	/* 리스트 조회 */
	function fn_search(){
		var reqUrl = CONTEXT_ROOT + targetUrl + "listCompanyCcm.do";
	...
	/* 엑셀 다운로드 */
	function fn_excel(){ 
		reqUrl = CONTEXT_ROOT + targetUrl + "excelCompanyCcm.do";
		$("#frmCompany").attr("action", reqUrl);
	...
	/* 엑셀 다운로드 */
	function fn_print(){ 
		var reqUrl = CONTEXT_ROOT + targetUrl + "printCompanyCcm.do";
		$("#frmCompany").attr("action", reqUrl);
...
<form id="frmMove" name="frmMove" action="<c:url value='/lu/subject/listCompanyCcm.do'/>" method="post">
	<input type="hidden" id="searchStatusType" name="searchStatusType" value="COM" /> 
</form>					
			

<form id="frmCompany" name="frmCompany" action="<c:url value='/lu/subject/listCompanyCcm.do'/>" method="post">
			...
			<dl id="tab-type">
				...
				<dd id="tab-content-11" style="display:block;">
					...
					<div class="search-box-1 mb-010">
						...
						<input type="text" name="searchKeyword" id="searchKeyword" value="${compVO.searchKeyword}" onkeypress="press(event);" placeholder="훈련과정명, 학과명 검색" />
					...
					<table class="type-2">
						...
						<tbody>
							<c:forEach var="result" items="${resultList}" varStatus="status">
								<tr>
									<td rowspan="2"><input type="checkbox" name="infoNumArr" id="infoNumArr${status.index}" value="${result.infoNum}"  class="choice" /></td>
