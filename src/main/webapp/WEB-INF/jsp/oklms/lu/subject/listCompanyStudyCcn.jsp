...
//         com.pageNavi( "pageNavi", totalCount , pageSize , pageIndex );
	...
	//페이지 이동
	function goOrderBy(orderValue,orderType){
		$("#orderValue").val(orderValue);
		$("#orderType").val(orderType);
		reqUrl = CONTEXT_ROOT + targetUrl + "listCompanyCcn.do";
		$("#frmCompany").attr("action", reqUrl);
		$("#frmCompany").attr("target","_self");
		$("#frmCompany").submit();	
	}
	...
	<input type="hidden" id="orderType" name="orderType" /> 
			...
			<dl id="tab-type">
				...
				<dd id="tab-content-11" style="display:block;">
					...
					<h2 class="mt-040">전체기업현황</h2>
					<div class="search-box-1 mt-010 mb-010">
						...
						<select name="term" id="term">
							<option value="1" <c:if test="${compVO.term eq '1' }" > selected="selected"  </c:if>>1학기</option>
							<option value="2" <c:if test="${compVO.term eq '2' }" > selected="selected"  </c:if>>2학기</option>
							<option value="3" <c:if test="${compVO.term eq '3' }" > selected="selected"  </c:if>>여름학기</option>
							<option value="4" <c:if test="${compVO.term eq '4' }" > selected="selected"  </c:if>>겨울학기</option>
						</select>
						<input type="text" name="searchKeyword" id="searchKeyword" value="${compVO.searchKeyword}" onkeypress="press(event);" placeholder="훈련과정명, 기업명 검색"  />
						...
						<table class="type-2">
							...
							<thead>
								<tr>
									...
									<th>훈련과정명&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.HRD_TRANING_NAME','DESC');">▼</a>&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.HRD_TRANING_NAME','ASC');">▲</a></th>
									<th>기업명&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.COMPANY_NAME','DESC');">▼</a>&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.COMPANY_NAME','ASC');">▲</a></th>
									<!-- <th>시작일자</th>
									<th>종료일자</th> -->
									<th>훈련<br />일지<br /><a href="javascript:goOrderBy('AA.NOTE_SUM_CNT','DESC');">▼</a>&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.NOTE_SUM_CNT','ASC');">▲</a></th>
									<th>내부<br />평가<br /><a href="javascript:goOrderBy('AA.EVAL_PLAN_CNT','DESC');">▼</a>&nbsp;&nbsp;<a href="javascript:goOrderBy('AA.EVAL_PLAN_CNT','ASC');">▲</a></th>
							...
							<tbody>
								<c:forEach var="result" items="${resultList}" varStatus="status">
									<tr>
										...
										<td rowspan="1"><a href="javascript:fn_traning_search('${result.companyId}','${result.traningProcessId}');">${result.noteSumCnt}/${result.weekCnt}</a></td>
