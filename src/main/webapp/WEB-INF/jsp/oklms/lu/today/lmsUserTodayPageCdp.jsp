							...
							<div class="group-area" >
								<table class="type-2  mb-020 mt-010">
									...
									<c:forEach var="result" items="${listDiscussCdp}" varStatus="status">
										<tr>
 											<td><input type="radio" onclick="javascript:setSubjectSmsDate('0','DC','${result.subjectCode}','${result.endDate}');" name="discussMemIdArr" value="${result.memIds}" class="choice" /></td>
								...
								<div class="btn-area align-right mt-010">
										<a href="#!" onclick="javascript:discussSms()" class="yellow">미제출자 SMS 발송</a>
								</div>									
 							</div>
							...
							<div class="group-area">
								<table class="type-2  mb-020 mt-010" >
									...
									<tbody>
										...
										<tr>
											<td><input type="radio" onclick="javascript:setSubjectSmsDate('${result.weekCnt}','OC','${result.subjectCode}','${result.weekEdDate}');" name="onlineMemIdArr" value="${result.memIds}" class="choice" /></td>
								...
								<div class="btn-area align-right mt-010">
										<a href="#!" onclick="javascript:onlineSms()" class="yellow">미제출자 SMS 발송</a>
								</div>										
							</div>
 
 						<c:set var="yyyy" value="" />
 						<c:set var="term" value="" />
 						<h2 id="f_5">재직증빙서류 제출현황</h2>
						<div class="group-area" >
							<table class="type-2  mb-020 mt-010">
								<colgroup>
 						...
 						<c:set var="term" value="${result.term }" />									
									<tr>
										<td><input type="radio" onclick="javascript:setSubjectSmsDate('0','WC','','${result.endTime}');" name="workCertMemIdArr" value="${result.memIds}" class="choice" /></td>
							...
							<div class="btn-area align-right mt-010">
								...
								<a href="#!" onclick="javascript:workCertSms();"  class="yellow">미제출자 SMS발송</a>
								<a href="/lu/workcert/listWorkCert.do?yyyy=${yyyy}&term=${term}&search=top"  class="orange">열기</a>
							</div><!-- E : btn-area -->
...
function discussSms(){
	var obj = document.getElementsByName("discussMemIdArr");
    var arr_value = "";
    for(var i = 0; i < obj.length; i++){
         if(obj[i].checked){
              arr_value += obj[i].value+",";
    ...
    if(!$('input:radio[name=discussMemIdArr]').is(':checked')){
		alert("선택된 교과가 없습니다.");
		return;
	}   
...
function onlineSms(){
	var obj = document.getElementsByName("onlineMemIdArr");
    var arr_value = "";
    for(var i = 0; i < obj.length; i++){
         if(obj[i].checked){
              arr_value += obj[i].value+",";
    ...
    if(!$('input:radio[name=onlineMemIdArr]').is(':checked')){
		alert("선택된 교과가 없습니다.");
		return;
	}   
...
function workCertSms(){
	var obj = document.getElementsByName("workCertMemIdArr");
    var arr_value = "";
    for(var i = 0; i < obj.length; i++){
         if(obj[i].checked){
              arr_value += obj[i].value+",";
    ...
    if(!$('input:radio[name=workCertMemIdArr]').is(':checked')){
		alert("선택된 학년이 없습니다.");
		return;
	}   
