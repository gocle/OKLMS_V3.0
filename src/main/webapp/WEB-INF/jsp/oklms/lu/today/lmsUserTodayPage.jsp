...
<input type="hidden" name="lectureMenuMarkYn"  value="N" />
								...
								<table class="type-2 mt-010 mb-010">
									...
									<c:forEach var="result" items="${listActivityNotMake}" varStatus="status">
										<tr>
											<td>${status.count}</td>
											<td>
												<c:if test="${result.subjectTraningType eq 'OFF'}">Off-JT</c:if>
												<c:if test="${result.subjectTraningType ne 'OFF'}">${result.subjectTraningType}</c:if>											
											</td>
									...
									<c:forEach var="result" items="${listWeekActivityMakeStd}" varStatus="status">
										...
										<c:if test="${result.offCountSubmit ne 'C'}">									
											...
											<tr>
												...
												<a href="/lu/activity/listWeekActivityStd.do" >미작성(${result.offCount} / ${result.offWorkCount })</a>
												</c:if> 
								...
								<table class="type-2 mt-010  mb-010" >
 								...
 								<ul class="page-sum mb-010 mt-010" id="f_5">
									<li class="float-left">
										토론 미참여건
									</li>
								</ul>
