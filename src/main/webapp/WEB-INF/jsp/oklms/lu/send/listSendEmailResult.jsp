...
//      com.pageNavi( "pageNavi", totalCount , pageSize , pageIndex );
	...
	function fn_search( pageIndex ){
		...
		var reqUrl = CONTEXT_ROOT + targetUrl + "listSendPrt.do";
	...
	<input type="hidden" id="sendType" name="sendType" value="EMAIL" /> 	
			<div class="search-box-1 mb-020">
					<select name="deptCode" id="deptCode">
						...
						<option value="Y" <c:if test="${sendVO.deptCode eq result.codeId }" > selected="selected"  </c:if>>즉시</option>
						<option value="N" <c:if test="${sendVO.deptCode eq result.codeId }" > selected="selected"  </c:if>>예약</option>
					</select>
					<select name="deptCode" id="deptCode">
						<option value="">발송상태</option>
						<option value="Y" <c:if test="${sendVO.deptCode eq result.codeId }" > selected="selected"  </c:if>>대기중</option>
						<option value="N" <c:if test="${sendVO.deptCode eq result.codeId }" > selected="selected"  </c:if>>발송완료</option>
					</select>
					<select name="deptCode" id="deptCode">
				...
				<div class="group-area mb-040">
					...
					<c:forEach var="result" items="${resultList}" varStatus="status">
						<tr>
							...
							<td>${result.templetType}</td>
							<td>${result.insertDate}</td>
							<td>${result.sendDate}</td>
							<td>${result.sendYn eq 'Y' ? '발송완료' : '발송준비'}</td>
							<td>
							<a href="#email-detail-wrap" rel="modal:open" class="btn btn-primary">${result.totCnt}</a>
							</td>
							<td>${result.successCnt}</td>
							<td>${result.failCnt}</td>
						</tr>
					</c:forEach>
