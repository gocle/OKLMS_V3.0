...
<fmt:parseNumber var="realProgressRateStdint" integerOnly="true" value="${realProgressRateStd}" />
									...
									<table class="type-2">
										...
										<c:forEach var="result" items="${resultList1}" varStatus="status">
											<tr  <c:if test="${result.nowWeekYn eq 'Y'}">class="highlight"</c:if>>
												...
												<c:set var="totTimeHour" value="${totTimeHour+offTimeHour}"/>
												<td>
												
												<c:if test="${!empty  result.reportId}">
													<c:set var="totReport" value="${totReport+1}"/>
													<%-- <a href="/lu/report/goScoreReport.do?reportId=${result.reportId}" class="btn-search-gray">과제</a> --%>
													<a href="/lu/report/listReport.do" class="btn-search-gray">과제</a>
												</c:if>
												</td>
												<td>
												...
												<c:if test="${!empty  result.discussId}">
													...
													<a href="/lu/discuss/listDiscuss.do" class="btn-search-gray">토론</a>
												</c:if>
												</td>
					...
					<c:if test="${subjectVO.onlineType ne 'NONE'}">
							...
							<dd id="tab-content-12">
								<h2>진행율</h2>
								<div class="progress-area mt-010">
									<p>전체 진행율 [  ${ getProgress.allOnAvgProgressRate}% ]</p>
									<ul>
										<li style="width:${getProgress.allOnAvgProgressRate }%;"><span><b>${ getProgress.allOnAvgProgressRate }</b> %</span></li>
									</ul>
								</div><!-- E : 진행율 -->
								...
								<div class="group-area mt-040">
									<div class="table-responsive">
									<table class="type-2">
										<caption>ON-LINE 주차 차시 콘텐츠명 학습기간 학습시간 미리보기 보조자료 정보 제공</caption>
										<colgroup>
											<col style="width:6%" />
											<col style="width:6%" />
											<col style="width:*" />
