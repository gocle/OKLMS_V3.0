...
<h2>교과목 정보</h2>
<h4 class="mb-010">
	<span>${subjectVO.subjectName} / ${subjectVO.subjectCode}</span> (${subjectVO.subjectClass}반) ㅣ ${subjectVO.yyyy}학년도 ${subjectVO.termName}
...
<div class="group-area">
	<table class="type-1 responsive-tr">
		...
		<tr>
			...
			<!-- 임시하드코딩 추후 반영시 필요없게됨 -->
			<c:choose>
				<c:when test="${sessionMemName eq '오기태'}">국민연금공단</c:when>
				<c:otherwise>${subjectVO.companyName}</c:otherwise>
			</c:choose>
			</td>
			<th>기업현장교사</th>
	...
	<div class="btn-area mt-010">
		<div class="float-right">
			<a href="#none" onclick="fn_subject_eval_form();" class="orange">체크리스트</a> 
		</div>
	</div>
	 -->
	<!-- E : btn-area -->
...
<!-- <h2 class="mt-040">훈련지원-학습근로자 훈련현황의 진행율</h2>
...
<table class="type-2">
	<caption>훈련정보 훈련일자 능력단위명 교육시간 학습자료 학습시간 훈련일지 학습활동서 정보 제공</caption>
	<colgroup>
		<col style="width:5%" />
		<col style="width:25%" />
		<col style="width:*" />
							...
							<c:choose>
								...
								<c:otherwise>
									<c:choose>
										...
										<c:otherwise>
											<a href="/lu/activity/listActivityOt.do?weekCnt=${result.weekCnt}" class="btn-line-orange">${result.actNoteCnt} / ${lessonCnt}</a>
										</c:otherwise>
									</c:choose>
								</c:otherwise>
							</c:choose>
