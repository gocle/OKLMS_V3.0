...
<c:if test="${boardVO.bbsId == 'BBSMSTR_000000000005' or boardVO.bbsId == 'BBSMSTR_000000000000'}">
	<h2>공지사항</h2>
</c:if>
	...
	<tbody>
		...
		<tr>
			...
			<td class="left" style="text-align: left;">
				...
				<c:choose>
						<c:when test="${result.isExpired eq 'T'}">
							<c:if test="${'Y' == result.topNoticeYn}">
								<c:forEach begin="0" end="${result.replyLc}" step="1">
					  			 <!-- &nbsp; -->
					  		</c:forEach>
					  		<a href="#"  class="text" onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');"><span>[공지]</span><c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</a>
							</c:if>
							<c:if test="${'N' == result.topNoticeYn}">
								<c:if test="${result.replyLc ne 0 }">
								<c:forEach begin="0" end="${result.replyLc-1}" step="1">
					  			<i class="xi-subdirectory-arrow"></i>[RE]
					  		</c:forEach></c:if><a href="#"  class="text" onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');"><span class="course"><%-- [<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반] --%></span> <c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</a>
							</c:if>
						</c:when>
						<c:when test="${result.isExpired eq 'Y'}">
							<c:if test="${'Y' == result.topNoticeYn}">
								<c:forEach begin="0" end="${result.replyLc}" step="1">
					  			 <!-- &nbsp; -->
					  		</c:forEach> 
					  		<c:choose>
					  			<c:when test="${memType == 'CDP'}">
					  				<a href="#"  class="text"  onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');" ><span>[미게시]</span><c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</a>
					  			</c:when>
					  			<c:otherwise>
					  				<a href="#"  class="text" <c:if test="${memType == 'PRT'|| memType == 'COT'}"> onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');" </c:if> >(미게시)<span class="course">[<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반]</span> <B><c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</B></a>
					  			</c:otherwise>
					  		</c:choose>
							...
							<c:if test="${'N' == result.topNoticeYn}">
					  		...
					  		<c:choose>
					  			...
					  			<c:otherwise>
					  				<a href="#"  class="text" <c:if test="${memType == 'PRT'|| memType == 'COT'}"> onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');" </c:if> >(미게시)<span class="course">[<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반1]</span> <c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</a>
					  			</c:otherwise>
					  		</c:choose>
						...
						<c:when test="${result.useAt == 'N'}">
							...
							<c:if test="${'Y' == result.topNoticeYn}">
								<a href="#"  class="text" onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');"><span class="course">[<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반1]</span> <B><c:out value="${result.nttSj}" /> [<c:out value="${result.commnetCount}" />]</B></a>
							</c:if>
							<c:if test="${'N' == result.topNoticeYn}">
						...
						<c:otherwise>
				  		...
				  		<c:if test="${'Y' == result.topNoticeYn}">
							<a href="#"  class="text" onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');"><span class="course">[<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반2]</span> <B><c:out value="${result.nttSj}"/> [<c:out value="${result.commnetCount}" />]</B></a>
						</c:if>
						<c:if test="${'N' == result.topNoticeYn}">
							<a href="#"  class="text" onclick="fn_egov_inqire_notice('${status.count}','${result.nttId}','${result.bbsId}');"><span class="course">[<c:out value="${result.subjectName}"/>(<c:out value="${result.subClass}"/>)분반3]</span> <c:out value="${result.nttSj}"/> [<c:out value="${result.commnetCount}" />]</a>
						</c:if>
	    ...
	    <!-- <tr>
	    	<td><i class="xi-subdirectory-arrow"><span class="sr-only">답변</span></i> [경영기초실습3(05)분반]Q&A에 대한 답글 작성 [0]</td>
	    </tr> -->
	</tbody>
</table><!-- E :  list-->
</div>
...
<div class="btn-area">
	...
	<div class="float-right">
		...
		<c:if test="${boardVO.bbsId eq 'BBSMSTR_000000000050'}">
			<a href="#!" onclick="fn_egov_addNotice();return false;" class="orange">작성</a>
		</c:if>
	</div>
	</c:if>	
