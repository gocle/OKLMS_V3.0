...
<div class="content">
		...
		<h1>학습자료실</h1>
			...
			<select name="searchTerm">
				...
				<option value="3" <c:if test="${searchVO.searchTerm == '3'}">selected="selected"</c:if> >여름학기</option>
				<option value="4" <c:if test="${searchVO.searchTerm == '4'}">selected="selected"</c:if> >겨울학기</option>
			</select>
<!-- 			<select id="" onchange="">
		...
		<!-- E : page-num (페이징 영역) -->
 		...
 		<c:if test="${memType == 'PRT'||memType == 'ATT'||memType == 'COT'}">
			<div class="btn_right">
				<a href="#@" onclick="fn_egov_addNotice();return false;"><input type="button" class="black" value="등록" />
			</div>
		</c:if>		
