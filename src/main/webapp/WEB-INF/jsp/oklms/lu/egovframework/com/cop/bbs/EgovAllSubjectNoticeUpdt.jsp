...
<!-- 선택자료실 및 전체자료실 게시글 수정시 값이 없어 수정이 안되더던 부분 세팅 -->
<c:if test="${bdMstr.bbsId == 'BBSMSTR_000000000006'}">
	<input type="hidden" name="yyyy" id="yyyy" value="<c:out value='${result.yyyy}'/>" />
	<input type="hidden" name="term" id="term" value="<c:out value='${result.term}'/>" />
	<input type="hidden" name="subjectCode" id="subjectCode" value="<c:out value='${result.subjectCode}'/>" />
	<input type="hidden" name="subClass" id="subClass" value="<c:out value='${result.subClass}'/>" />
	<c:if test="${'02' eq result.categoryCd}">
		<input type="hidden" name="subjectName" id="subjectName" value="<c:out value='${result.subjectName}'/>" />
		<input type="hidden" name="categoryCd" id="categoryCd" value="<c:out value='${result.categoryCd}'/>" />
		<input type="hidden" name="subjectTraningType" id="subjectTraningType" value="<c:out value='${result.subjectTraningType}'/>" />
	</c:if>
</c:if>
...
<div id="">
	...
	<table class="type-write">
		...
		<tbody>
			<c:if test="${bdMstr.bbsId ne 'BBSMSTR_000000000000' && result.bbsId ne 'BBSMSTR_000000000050'}">
				<c:if test="${bdMstr.bbsId != 'BBSMSTR_000000000007'}">
					<tr>
			...
			<c:if test="${bdMstr.bbsId ne 'BBSMSTR_000000000000'}">
					...
					<td>
					    <select name="selectedCd" id="selectedCd">
				 			...
				 			<c:forEach var="newsTypeCd" items="${newsTypeCode}" varStatus="status">
								<option value="${newsTypeCd.codeId}" ${newsTypeCd.codeName == result.selectedCdNm ? 'selected="selected"' : '' }>${newsTypeCd.codeName}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
			...
			<tr>
				...
				<c:if test="${not empty result.atchFileId}">
					<c:choose>
						<c:when test="${fn:indexOf(result.atchFileId, 'FILE_') != -1}">
							<tr>
								<th><spring:message code="cop.atchFileList" /></th>
								<td>
									<c:import url="/cmm/fms/selectFileInfsForUpdate.do" charEncoding="utf-8">
										<c:param name="param_atchFileId" value="${result.atchFileId}" />
										<c:param name="param_returnUrl" value="/lu/cop/bbs/${bdMstr.bbsId}/forUpdateBoardArticle.do?nttId=${result.nttId}" />
									</c:import>
								</td>
							</tr>
						</c:when>
						<c:otherwise>
							<c:if test="${!empty resultFile}">
								...
								<a href="javascript:com.deleteFile('${resultFile.atchFileId}|${resultFile.fileSn}', '/lu/cop/bbs/${bdMstr.bbsId}/forUpdateBoardArticle.do?nttId=${result.nttId}')">[삭제]</a><br />
							</c:if>
							<c:if test="${empty resultFile}">
							<!-- 
