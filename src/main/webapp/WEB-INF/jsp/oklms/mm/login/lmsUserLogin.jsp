...
<form id="spLoginFrm" name="spLoginFrm" method="post" action="<%= this.generateUrl(IDP_URL, LOGIN_URL) %>">
    ...
    <input type="hidden" id="sp_menuArea" name="menuArea"  value="MOBILE">
    <input type="hidden" name="<%= RELAY_STATE_NAME %>" value="/commbiz/login/ssoLoginProc.do?menuArea=MOBILE" />
				...
				<div id="login-area" class="login-area gLogin">
					...
					<form id="loginProcBodyForm" method="post" action="/commbiz/login/loginProc.do" class="gLogin">
						...
						<fieldset>
							...
							<p class="item"><label for="bodyMemId" class="iLabel">ID</label><input type="text" name="memId" type="text" id="bodyMemId" class="iText uid" /></p>
							<p class="item"><label for="bodyMemPasswordEncript" class="iLabel">PASSWORD</label><input type="password" name="memPasswordEncript" type="password" id="bodyMemPasswordEncript" class="iText upw"  /></p>
							...
							<span>
								...
								<label for="sso-login"><input name="checkbox" ${ssoYn eq 'Y' ? 'checked' : ''} type="checkbox" id="sso-login" />통합 로그인</label>
							</span>
						</fieldset>
					</form>
