...
function fn_online(yyyy,term,subjectCode,subClass,subjectName,weekId,lessonId,weekMobileNotCnt){
...
<input type="hidden" name="lessonId" id ="curri_lessonId" >
</form>
				...
				<div id="contents-area">
					...
					<table class="type-2">
						...
						<c:forEach var="rs" items="${listLmsUserMainPageStatusCnt }" varStatus="status">
							<tr>
								<td class="left"><a href="#!" onclick="fn_curri('${rs.yyyy}','${rs.term}','${rs.subjectCode}','${rs.subClass}');">${rs.subjectName}</a></td>
								<td>${rs.realProgressRate} %</td>
								<td>${rs.totalTime - rs.attend }</td>
								<td>
									<c:choose>
										<c:when test="${rs.onlineType eq '101' or rs.onlineType eq '104' or rs.onlineType eq '105'}">${rs.onTotalTime - rs.onAttend }</c:when>
					...
					<c:set var="endDate" value="${fn:replace(noticeResult.ntceEndde, '-', '')}"/>
					...
					<li class="more"><a href="#!" onclick="/mm/cop/bbs/BBSMSTR_000000000007/selectBoardList.do" title="더 보기">Q &amp; A 더 보기</a></li>
					</ul>
					</c:if>
						...
						<tbody>
							...
							<tr>
								...
								<c:if test="${offListSchedule.weekMappingCnt ne '0'}">
									<a href="javascript:fn_online('${offListSchedule.yyyy}','${offListSchedule.term}','${offListSchedule.subjectCode}','${offListSchedule.subClass}','${offListSchedule.subjectName}','${offListSchedule.weekId}','${offListSchedule.lessonId}','${offListSchedule.weekMobileNotCnt}');" class="more">${offListSchedule.onlineTypeName}</a>
								</c:if>
								</td>
							</tr>
							<tr>
