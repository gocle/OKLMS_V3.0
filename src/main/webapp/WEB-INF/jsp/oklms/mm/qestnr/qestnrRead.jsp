...
<!--
		...
		/* 설문 상세조회 페이지로 이동 */
		function fn_update(){ 
			var reqUrl = "/mm/qestnr/goUpdateQestnr.do";
	...
	<input type="hidden" id="etcAnswerAt" name="etcAnswerAt" value="${readQestnrInfoVO.etcAnswerAt}" />		
				<div id="contents-area">
						<dl class="bbs-view mt-020">
							...
							<dd>
								<span>참여인원</span> : ${readQestnrInfoVO.answerResultCnt}/${readQestnrInfoVO.lessonStdCnt} 명
							</dd>
							<dt></dt>
							<dd>
								<span>${readQestnrInfoVO.qustnrPurps}</span>									
									<c:if test="${!empty readQestnrItemVO.qestnCn1}"><dd><input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled"  value="1"> ${readQestnrItemVO.qestnCn1}</dd></c:if>
									<c:if test="${!empty readQestnrItemVO.qestnCn2}"><dd><input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled"  value="2"> ${readQestnrItemVO.qestnCn2}</dd></c:if>
									<c:if test="${!empty readQestnrItemVO.qestnCn3}"><dd><input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled"  value="3"> ${readQestnrItemVO.qestnCn3}</dd></c:if>
									<c:if test="${!empty readQestnrItemVO.qestnCn4}"><dd><input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled"  value="4"> ${readQestnrItemVO.qestnCn4}</dd></c:if>
									<c:if test="${!empty readQestnrItemVO.qestnCn5}"><dd><input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled"  value="5"> ${readQestnrItemVO.qestnCn5}</dd></c:if>
									<dd>
										<input type="radio" id="qestnAnswerSn" name="qestnAnswerSn" disabled="disabled" value="etc"> 기타
					...
					<div class="btn-area align-right mt-0 10">
							<a href="#!" onclick="javascript:fn_list(); return false" class="gray-1">목록</a>
							<c:if test="${qestnrVO.progrressStatus eq '예정'}">
							<a href="#!" onclick="javascript:fn_update(); return false" class="orange">수정</a>
							</c:if>							
					</div>    					
				</div><!-- E : contents-area -->
</form:form>				
