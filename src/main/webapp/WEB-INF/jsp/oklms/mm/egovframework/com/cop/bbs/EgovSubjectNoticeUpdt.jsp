...
====================================================================*/
	...
	function fn_egov_regist_notice() {
		...
		if (fn_egov_validateForm() && confirm('<spring:message code="common.regist.msg" />')) {
			document.board.action = "<c:url value='/mm/cop/bbs${prefix}/${pathBbsIdStr}/updateBoardArticle.do'/>";
...
<div id="container">
 	<div id="contents-area">
 		...
 		<dl class="bbs-view mt-020"> 
			<dt>
				<span>				 						
					<input type="text" id="nttSj" name="nttSj" maxlength="60" value="<c:out value="${result.nttSj}" />" placeholder="(ex)제목 입력" style="width:99%;" />				
				</span>
			</dt>
			<dd>
				<textarea id="nttCn" name="nttCn" style="width: 100%;height: 125px;"  placeholder="(ex)내용 입력"><c:out value="${result.nttCn}" /></textarea>
			</dd>
			<c:if test="${result.ntceBgnde != '10000101'}">
			<dd>
				<span>게시기간</span> : 
					<input type="text" id="ntceBgndeView" name="ntceBgndeView"  value='<c:out value="${result.ntceBgnde}" />'   style="width:100px;" readonly /> ~
					<input type="text" id="ntceEnddeView" name="ntceEnddeView"  value='<c:out value="${result.ntceEndde}" />'   style="width:100px;" readonly />
