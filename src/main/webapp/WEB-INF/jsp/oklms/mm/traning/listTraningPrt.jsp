...
<fmt:parseNumber var = "offTimeHour" integerOnly = "true" value = "${currProcVO.offTimeHour}" />
...
<c:forEach var="timeList" items="${timeList}" varStatus="status">
						...
						<form name="frmTraning${status.index}" id="frmTraning${status.index}" method="post">
							...
							<input type="hidden" id="planTime" name="planTime" value="${offTimeHourr}" />
...
<input type="hidden" id="state" name="state" value="W" />
							<dl class="learner-training">
								<dt>정규수업 : ${resultSum[status.index].traningDate}(${resultSum[status.index].dayOfWeek}) ${resultSum[status.index].traningSt}~${resultSum[status.index].traningEd} (${offTimeHour}시간)  ${result.traniningNoteMasterId } </dt>
								<dd>
									<table class="type-2">
										<colgroup>
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<c:if test="${!empty resultlist.studyTime }" >
													<input type="text" id="studyTimeArr" name="studyTimeArr" maxlength="2"  style="width:95%;" onchange="javascript:if(this.value > ${offTimeHour} ){alert('훈련시간은 주간훈련시간보다 적어야합니다.');this.value='${offTimeHour}';return;};studyTimeArrZeor(this.value,'${offTimeHour}','${resultlist.memId}','${status.index+1}');" value="${resultlist.studyTime}"/> 
													</c:if>
													<c:if test="${empty resultlist.studyTime }" >
													<input type="text" id="studyTimeArr" name="studyTimeArr" maxlength="2"   style="width:95%;" onchange="javascript:if(this.value > ${offTimeHour} ){alert('훈련시간은 주간훈련시간보다 적어야합니다.');this.value='${offTimeHour}';return;}studyTimeArrZeor(this.value,'${offTimeHour}','${resultlist.memId}','${status.index+1}');"  value="${offTimeHour}"/> 
													</c:if>
												</td>
