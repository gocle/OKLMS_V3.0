...
function fn_online(weekId,weekMobileNotCnt){
...
<input type="hidden" name="weekId" id="weekId" value="">
...
<fmt:parseNumber var = "offTimeHour" value = "${currProcVO.offTimeHour}" />
					...
					<div class="progress-area">
						<p>전체 진행율  [ 총 ${ empty progress.onRealRrogressTime  ? 0 : progress.onRealRrogressTime} 분 ]</p>
						<progress value="${ empty progress.onRealRrogressRate  ? 0 : progress.onRealRrogressRate}" max="100"><b>${ empty progress.onRealRrogressRate  ? 0 : progress.onRealRrogressRate}%</b></progress>
					</div>
							...
							<!-- O -->
								...
								<c:choose>
									<c:when test="${result.weekMappingCnt ne '0'}">
										${subjectVO.onlineTypeName} <p id="weekProgress_${result.weekId}">${result.weekProgressRate}%</p><a href="javascript:fn_online('${result.weekId}','${result.weekMobileNotCnt}');" class="btn-full-gray">${result.btnText}</a>
									</c:when>
									<c:otherwise>
									</c:otherwise>
								</c:choose>
									...
									<c:otherwise>
										<c:choose>
											<c:when test="${result.actNoteCnt eq '0'}">
												<a href="#none" onclick="fn_activity('${result.weekCnt}');" class="btn-line-orange">미작성</a>
											</c:when>
											<c:otherwise>
