...
function goWeekScheduleLesson(){
	var reqUrl = CONTEXT_ROOT + "/lu/currproc/listCurrProcStdWeekSubSchedule.json";
	var param = $("#frmLesson").serializeArray();
	com.ajax(reqUrl, param, function(obj, data, textStatus, jqXHR){			
	if (200 == jqXHR.status ) {
		var data 	= jqXHR.responseJSON.retData;
		...
		for (var i = 0; i < data.length; i++) {
			...
			if( i==0 && totPage == 0){
				var fContentUrl = list.contentUrl;
				var fWidth = list.width;
				var fHeight = list.height;
				var fElemId = list.elemId;
			}
		...
		/*     if(i == 0) {
		   ...
		   if(list.linkContentYn == "Y") {// CMS 와 연계
				//var cmsData = cmsLessonList(stdLessonId, weekId, list.subjSchId, list.cmsLessonId, weekSchId);
					...
					if(list.displayOrder == "1") {
						
						//$(".links").append("<li id='"+weekSchId+"_"+totPage+"'><a href='javascript:goViewLesson(\""+stdLessonId+"\",\""+weekId+"\",\""+weekCnt+"\",\""+list.subjSchId+"\",\""+list.linkContentYn+"\",\""+list.lesssonId+"\",\""+list.lessonItemId+"\",\""+list.lessonSubItemId+"\",\""+list.contentType+"\",\""+list.contentsDir+"\",\""+list.contentsIdxFile+"\",\""+list.url+"\",\""+cmsPageNo+"\");'><span id='"+spanId+"' name='span_name'>"+mark+"</span>"+cmsList.title+"</a></li>");
					}	
					
					var classOn = "";
					if( i==0 ) classOn="on";
					
					if(list.displayOrder == "1") titleStr += "<li><a  id='"+list.elemId+"' class='"+classOn+"' href='javascript:viewLesson(\""+list.elemId+"\",\""+list.contentUrl+"\",\""+list.width+"\",\""+list.height+"\");'><span><i class='"+weekMark+"'></i></span>"+list.title+"</a></li>";	
					contentArray.push({elemId : list.elemId, contentUrl: list.contentUrl, width: list.width, height : list.height, lessonItemId : list.lessonSubItemId});
					totPage++;	
			} /* else {
				var pageNo = (i+1);
				...
				$(".links").append("<li id='"+weekSchId+"_"+totPage+"'><a href='javascript:goViewLesson(\""+stdLessonId+"\",\""+weekId+"\",\""+weekCnt+"\",\""+list.subjSchId+"\",\""+list.linkContentYn+"\",\"\",\"\",\"\",\""+list.contentType+"\",\""+list.contentsDir+"\",\""+list.contentsIdxFile+"\",\""+list.url+"\",\""+pageNo+"\");'><span  id='"+spanId+"' name='span_name'>"+mark+"</span>"+list.subjTitle+"</a></li>");
		...
		$(".links").append(titleStr);
		
		if(weekProgressYn == 'N'){
			alert("예습, 복습시에는 진도율이 적용되지 않습니다.");
		}
		
		viewLesson(fElemId,fContentUrl,fWidth,fHeight);
...
//컨텐츠 목록에서 위치 서칭 처리
function contentArrayPositionSearch(vElemId){
	var pos=0;
	for (i = 0; i < contentArray.length; i++) {
	       if (contentArray[i].elemId == vElemId) {
	    	   pos=i;
	           break;
	       }
	}	
...
function viewLesson(elemId, contentUrl, width, height ){
	
	// 기존콘텐츠 시간업데이트
	//updateAttend();
	
	var contentArrayIndex = contentArrayPositionSearch(elemId);
	var tagVal = elemId;
	...
	if (contentUrl.indexOf('Contents') != -1 || contentUrl.indexOf('Sources/Contents') != -1) {
		...
		if(contentUrl.indexOf('olei') != -1 ){
			...
			if(contentUrl.indexOf('/videos/') != -1){ // 자체 포팅 동영상일 경우
				contentUrl = contentUrl;
				width = "1000";
				height = "660";
			} else {
...
function movieLesson(vStdLessonId, vWeekId, vWeekCnt, vSubjSchId, contentType, contentsDir, contentsIdxFile, url, vPageInfo, viewLessonId , spanId){
	...
	if (200 == jqXHR.status ) {
		...
		if(contentType == "MP4"){
			...
			$("#contentFrame").attr("src","/lu/currproc/getCurrProcMovieViewer.do?movieUrl="+movieUrl+"&width=100%&height=100%");
		} else { // 아닐경우
...
//컨텐츠 paging 처리
function setPage(){
	//$("#contentPage").html(+curPage+"</b>"+"/"+totPage);
	...
	if(contentUrl != ""){
		...
		if(fileExt == ".mp4"){
			...
			$("#contentFrame").attr("src","/lu/currproc/getCurrProcMovieViewer.do?movieUrl="+contentUrl+"&width="+width+"&height="+height);
	   		...
	   		if(jsonData.result.contentUrl.indexOf('http://oklms.koreatech.ac.kr/content/empty_content.html') != -1){
	   			$("#contentFrame").attr("src", "/content/empty_content.html");
	   		} else {
	   			$("#contentFrame").attr("src",contentUrl);
	   		}
...
function setNPMenu(vCurPage){
	...
	viewLesson(contentArray[pos].elemId,contentArray[pos].contentUrl,contentArray[pos].width,contentArray[pos].height);
}
...
function setNPMenu(vCurPage){
	...
	$("#index-schedule").find('dd').removeClass('highlight');
