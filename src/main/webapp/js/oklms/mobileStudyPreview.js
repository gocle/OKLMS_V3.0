...
function goWeekPreview(weekId, weekCnt, contentName){
	$("#weekId").val(weekId);
	$("#weekCnt").val(weekCnt);
	   ...
	   if(list.linkContentYn == "Y") {// CMS 와 연계
			...
			for (var x = 0; x < cmsData.length; x++) {
				...
				if(cmsList.display_order == "1") {
					$("#"+weekSchId).append("<li id='"+weekSchId+"_"+totPage+"'><a href='javascript:goViewLesson(\""+weekId+"\",\""+weekCnt+"\",\""+list.subjSchId+"\",\""+list.linkContentYn+"\",\""+list.cmsLessonId+"\",\""+cmsList.lesson_item_id+"\",\""+cmsList.lesson_sub_item+"\",\""+list.contentType+"\",\""+list.contentsDir+"\",\""+list.contentsIdxFile+"\",\""+list.url+"\",\""+cmsPageNo+"\");'>"+cmsList.title+"</a></li>");
				}	
			...
			$("#"+weekSchId).append("<li id='"+weekSchId+"_"+totPage+"'><a href='javascript:goViewLesson(\""+weekId+"\",\""+weekCnt+"\",\""+list.subjSchId+"\",\""+list.linkContentYn+"\",\"\",\"\",\"\",\""+list.contentType+"\",\""+list.contentsDir+"\",\""+list.contentsIdxFile+"\",\""+list.url+"\",\""+pageNo+"\");'>"+list.subjTitle+"</a></li>");
...
function goViewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId,vLinkContentYn,vlessonId,vlessonItemId,vlessonSubItem,vContentType,vContentsDir,vContentsIdxFile,vUrl ,vPageInfo, spanId){
	...
	$("#index-name").html("[ "+contentArray[contentArrayIndex].weekCnt+"주차 ] "+contentArray[contentArrayIndex].subjStep+"차시 - <b>"+contentArray[contentArrayIndex].subTitle+"</b>");
}
...
function movieLesson(vWeekId, vWeekCnt, vSubjSchId, contentType, contentsDir, contentsIdxFile, url, vPageInfo, viewLessonId , spanId){
	...
	if (200 == jqXHR.status ) {
		...
		if(contentType == "MP4"){
			...
			$("#contentFrame").attr("src","/lu/currproc/getCurrProcMobileMovieViewer.do?movieUrl="+movieUrl+"&width=1000&height=660");
		} else { // 아닐경우
...
function setViewLesson(vWeekId,vWeekCnt,vSubjSchId,data){
	...
	if(contentUrl != ""){
		...
		// 동영상일 경우
		if(fileExt == ".mp4"){
			$("#contentFrame").attr("src","/lu/currproc/getCurrProcMobileMovieViewer.do?movieUrl="+contentUrl+"&width="+width+"&height="+height);
		} else { // 아닐경우
...
function setNPMenu(vCurPage){
	...
	$("#index-name").html("[ "+contentArray[pos].weekCnt+"주차 ] "+contentArray[pos].subjStep+"차시 - <b>"+contentArray[pos].subTitle+"</b>");
}
...
function startView(){
	...
	$("#index-name").html("[ "+contentArray[0].weekCnt+"주차 ] "+contentArray[0].subjStep+"차시 - <b>"+contentArray[0].subTitle+"</b>");
}
