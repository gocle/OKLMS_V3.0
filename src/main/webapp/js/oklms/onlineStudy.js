...
function goWeekLesson(){
	...
	if (200 == jqXHR.status ) {
		...
		addTree( $("#stdLessonId").val(), retData, $("#weekProgressRate").val() );
	} else {
...
function addTree(stdLessonId,data, weekProgressRate){
	...
	var row = 0;
	var weekMark = "";
	//alert(weekProgressRate);
	//alert(attendProgress);
	...
	$("#lesson_week").html(weekCnt+"주차");
	$("#lesson_check").html("출석 : "+weekMark);
	...
	for (var i = 0; i < data.length; i++) {
	   ...
	   if(list.linkContentYn == "Y") {// CMS 와 연계
			...
			for (var x = 0; x < cmsData.length; x++) {
				...
				var cmsPageNo = (x+1);
				var spanId = list.subjSchId+"_"+cmsList.lessonItemId;
				
				//var cmsData = cmsLessonList(stdLessonId, weekId, list.subjSchId, list.cmsLessonId, weekSchId);
				
				
				
				if(cmsList.displayOrder == "1") {
					
					console.log("lessonProgress : "+cmsList.lessonProgress);
					...
					$(".links").append("<li id='"+weekSchId+"_"+totPage+"'><a href='javascript:goViewLesson(\""+stdLessonId+"\",\""+weekId+"\",\""+weekCnt+"\",\""+list.subjSchId+"\",\""+list.linkContentYn+"\",\""+list.cmsLessonId+"\",\""+cmsList.lessonItemId+"\",\""+cmsList.lessonSubItemId+"\",\""+list.contentType+"\",\""+list.contentsDir+"\",\""+list.contentsIdxFile+"\",\""+list.url+"\",\""+cmsPageNo+"\",\""+cmsList.contentUrl+"\");'><span><i id='"+spanId+"' name='span_name' class='"+mark+"'></i></span>"+cmsList.title+"</a></li>");
				}	
				contentArray.push({stdLessonId : stdLessonId, weekId : weekId, weekCnt : weekCnt, subjSchId : list.subjSchId, subjStep : list.subjStep , subTitle : list.subjTitle, linkContentYn : list.linkContentYn, lessonId: list.cmsLessonId, lessonItemId: cmsList.lessonItemId, subitemListId : cmsList.lessonSubItemId, contentType : list.contentType, contentsDir : list.contentsDir, contentsIdxFile : list.contentsIdxFile, url : list.url, viewLessonId : weekSchId+"_"+totPage, pageInfo : cmsPageNo , spanId : spanId, spanId : spanId, contentUrl : cmsList.contentUrl});
				totPage++;	
			}
		} else {
			var pageNo = (i+1);
...
//컨텐츠 이동
function goViewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId,vLinkContentYn,vlessonId,vlessonItemId,vlessonSubItem,vContentType,vContentsDir,vContentsIdxFile,vUrl ,vPageInfo, spanId, contentUrl){
	...
	var pos = curPage-1;
	var tagVal1 = contentArray[pos].viewLessonId;
	
	$("#"+tagVal1).parent().parent().find('a').removeClass('on');
	$("#"+tagVal1).find('a').addClass('on');
	
	
	console.log("contentArray[contentArrayIndex].contentUrl : "+contentArray[contentArrayIndex].contentUrl);
	
	if(vLinkContentYn == "Y"){
		viewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId,vlessonId,vlessonItemId,contentArray[contentArrayIndex].subitemListId,vPageInfo,contentArray[contentArrayIndex].viewLessonId, contentArray[contentArrayIndex].spanId, contentArray[contentArrayIndex].contentUrl);
	} else {
		movieLesson(vStdLessonId, vWeekId, vWeekCnt, vSubjSchId, vContentType, vContentsDir, vContentsIdxFile, vUrl, vPageInfo, contentArray[contentArrayIndex].viewLessonId, contentArray[contentArrayIndex].spanId, contentArray[contentArrayIndex].contentUrl);
	}
...
//컨텐츠 내용 조회
function viewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId, vlessonId,vlessonItemId,vlessonSubItemId, vPageInfo, viewLessonId, spanId, contentUrl){
	...
	//console.log("viewLesson 0000000000000000000000000000000");
	...
	if (200 == jqXHR.status ) {
 		//com.alert( jqXHR.responseJSON.retMsg );
		setViewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId, jqXHR.responseJSON.retMsg, contentUrl);
		//console.log("viewLesson 11111111111111111111111111111111");
	...
	//console.log("viewLesson 2222222222222222222222222222222222");
}


function movieLesson(vStdLessonId, vWeekId, vWeekCnt, vSubjSchId, contentType, contentsDir, contentsIdxFile, url, vPageInfo, viewLessonId , spanId, contentUrl){
	 attendInterVal(false);
...
//컨텐츠 paging 처리
function setPage(){
	$("#contentPage").html("<span class='orange_text'>"+curPage+"</span>/ <span>"+totPage+"</span>");
}


//컨텐츠 동영상/플래쉬 변경처리
function setViewLesson(vStdLessonId,vWeekId,vWeekCnt,vSubjSchId,data,contentUrl){
	...
	if (contentUrl.indexOf('Contents') != -1 || contentUrl.indexOf('Sources/Contents') != -1) {
		if(contentUrl.indexOf('http://') != -1){
			...
			contentUrl = 'https://cdn.e-koreatech.ac.kr/Portal/'+contentUrl;
		}
	} else {
		if(contentUrl.indexOf('olei') != -1 ){
			contentUrl = 'https://cdn.e-koreatech.ac.kr/Portal/'+contentUrl;
		} else {
			if(contentUrl.indexOf('202401170_25v1') != -1){
				contentUrl = 'https://cdn.e-koreatech.ac.kr/Portal'+contentUrl;
			} else {
				contentUrl = 'https://cdn.e-koreatech.ac.kr/Portal/Sources/Contents/'+contentUrl;
			}
		}
	}
	...
	if(contentUrl != ""){
		...
		if(fileExt == ".mp4"){
			...
			$("#contentFrame").attr("src","/lu/currproc/getCurrProcMovieViewer.do?movieUrl="+contentUrl+"&width="+width+"&height="+height);
	   		...
	   		if(contentUrl.indexOf('http://oklms.koreatech.ac.kr/content/empty_content.html') != -1){
	   			$("#contentFrame").attr("src", "/content/empty_content.html");
	   		} else {
...
function openMenu(tagVal){
	$("#"+tagVal).parent().parent().siblings().find('a').removeClass('on');
...
function setNPMenu(vCurPage){
	...
	$("#"+tagVal).parent().parent().find('a').removeClass('on');
	$("#"+tagVal).find('a').addClass('on');
	
	//2020WEEK0064218_2020WSCH0015257_8
	...
	if(linkContentYn == "Y"){
		viewLesson(contentArray[pos].stdLessonId,contentArray[pos].weekId,contentArray[pos].weekCnt,contentArray[pos].subjSchId,contentArray[pos].lessonId,contentArray[pos].lessonItemId,contentArray[pos].subitemListId, contentArray[pos].pageInfo,contentArray[pos].viewLessonId,contentArray[pos].spanId,contentArray[pos].contentUrl);
	} else {
		movieLesson(contentArray[pos].stdLessonId, contentArray[pos].weekId, contentArray[pos].weekCnt, contentArray[pos].subjSchId, contentArray[pos].contentType, contentArray[pos].contentsDir, contentArray[pos].contentsIdxFile, contentArray[pos].url, contentArray[pos].pageInfo,contentArray[pos].spanId,contentArray[pos].contentUrl);
	}
...
function startView(){
	...
	$("#"+tagVal).find('a').addClass('on');
	$("#"+tagVal).siblings().removeClass('on');
	//메뉴접기
	$("#"+tagVal).parent().parent().siblings().find('li').removeClass('on');
	$("#"+tagVal).parent().parent().siblings().find('ul').removeClass('on');
	$("#"+tagVal).parent().addClass('on');
	
	if(linkContentYn == "Y"){
		viewLesson(contentArray[0].stdLessonId,contentArray[0].weekId,contentArray[0].weekCnt,contentArray[0].subjSchId,contentArray[0].lessonId,contentArray[0].lessonItemId,contentArray[0].subitemListId,contentArray[0].pageInfo,contentArray[0].viewLessonId,contentArray[0].spanId,contentArray[0].contentUrl);
	} else {
		movieLesson(contentArray[0].stdLessonId, contentArray[0].weekId, contentArray[0].weekCnt, contentArray[0].subjSchId, contentArray[0].contentType, contentArray[0].contentsDir, contentArray[0].contentsIdxFile, contentArray[0].url, contentArray[0].pageInfo, contentArray[0].viewLessonId,contentArray[0].spanId,contentArray[0].contentUrl);
	}
			...
			if(progressAttendId != "" && progressTimeId != "" && iframeSrc != "#"){
				...
				if (200 == jqXHR.status ) {
					...
					// 클립별 시간진도율
			 		...
			 		console.log("timeProgressRatio : "+timeProgressRatio);
			 		...
			 		$("#"+spanId).attr('class',mark);
			 		
			 		var markProgress = 0;
			 		var markLen = $("i[name='span_name']").length;
			 		
			 		//console.log("============= markLen : "+markLen);
			 		
			 		$("i[name='span_name']").each(function(index) { 
			 			var str = $(this).text();
			 			if(str.indexOf("○") != -1){
