 ...
 *
 *******************************************************************************/ 
package kr.co.gocle.oklms.la.member.web;
...
import kr.co.gocle.oklms.la.authority.service.AuthGroupService;
...
public class MemberController extends BaseController{
	...
	public String listMember(@ModelAttribute("frmMember") MemberVO memberVO, ModelMap model) throws Exception {
		...
		loginInfo.getLoginInfo().getAuthGroupId();
		
		if(!loginInfo.getLoginInfo().getAuthGroupId().equals("2016AUTH0000001")) {			
				return "redirect:/lu/main/lmsUserMainPage.do";
		}
		...
		List<AuthGroupVO> searchAuthGroupCodeList = authGroupService.listDistAuthGroup(authGroupVO);
	...
	public String goInsertMember(@ModelAttribute("frmMember") MemberVO memberVO,  ModelMap model)  throws Exception {
		...
		// 학과 공통코드 조회
		List<CommonCodeVO> deptCodeList = commonCodeService.selectDeptCodeList(codeVO);
...
*/		
	...
	@RequestMapping(value = "/lu/member/insertExcelCompMember.do")
	public String insertExcelCompMember(@ModelAttribute("frmMember") @Valid ExcelMemberVO excelMemberVO, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status, MultipartHttpServletRequest multiRequest) throws Exception {
		
		String retMsg = "";
		
		 String result = memberService.insertExcelCompMember(excelMemberVO, multiRequest);
		...
		return "redirect:/lu/member/listCompanyMember.do?companyId="+excelMemberVO.getCompanyId()+"&memberType="+excelMemberVO.getMemberType();
	}
	...
	@RequestMapping(value = "/lu/member/listStdMember.do")
	public String listStdMember(@ModelAttribute("frmMember") MemberVO memberVO, ModelMap model) throws Exception {
		...
		List<MemberVO> resultList = memberService.listStdMember( memberVO );
		
		model.addAttribute("loginAuthGroupId", loginInfo.getAuthGroupId());
        model.addAttribute("memberVO", memberVO);
        model.addAttribute("resultList", resultList);
       if( memberVO.getSessionMemType().equals("CDP")){
    	   // View호출
    	   return "oklms/lu/member/stdMemberCdpList";
       } else {
    	   
    	   // View호출
   			return "oklms/lu/member/stdMemberList";
       }
	}
	
	@RequestMapping(value = "/lu/member/updateStdMember.do")
	public String updateStdMember(@ModelAttribute("frmMember") MemberVO memberVO, ModelMap model, RedirectAttributes redirectAttributes) throws Exception {
		...
		iResult = memberService.updateStdMember( memberVO );
		...
		// View호출
		return "redirect:/lu/member/listStdMember.do";
	}
	
	@RequestMapping(value = "/lu/member/saveStdMember.do")
	public String saveStdMember(@ModelAttribute("frmMember") MemberVO memberVO, ModelMap model, RedirectAttributes redirectAttributes) throws Exception {
		...
		iResult = memberService.saveStdMember( memberVO );
	...
	@RequestMapping(value = "/lu/member/insertCompMemberPage.do")
	public String insertCompMemberPage(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status) throws Exception {

		CompanyVO companyVO = new CompanyVO();
		companyVO.setCompanyId(memberVO.getCompanyId());
		companyVO = companyService.getCompany(companyVO);
    	model.addAttribute("companyVO", companyVO);
		
		model.addAttribute("memberVO", memberVO);

		return "oklms/lu/member/memberCompInsert";
	}
	
	
	@RequestMapping(value = "/lu/member/insertCompMember.do")
	public String insertCompMember(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status,final MultipartHttpServletRequest multiRequest) throws Exception {

		String retMsg = "입력값을 확인해주세요";

		 String insertPK = memberService.insertCompMember(memberVO,multiRequest);
	     retMsg = "정상적으로 (저장)처리되었습니다.!";

		redirectAttributes.addFlashAttribute("retMsg", retMsg);
		return "redirect:/lu/member/listCompanyMember.do?companyId="+memberVO.getCompanyId()+"&memberType="+memberVO.getMemberType();
	}
	
	
	
	@RequestMapping(value = "/lu/member/updateCompMemberPage.do")
	public String updateCompMemberPage(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status) throws Exception {
    	...
    	model.addAttribute("memberType", memberVO.getMemberType());
		...
		if(memberVO!=null){
			...
			atchFileVO.setAtchFileId(memberVO.getAtchFileId());
			//첨부파일
	...
	@RequestMapping(value = "/lu/member/updateCompMember.do")
	public String updateCompMember(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status,final MultipartHttpServletRequest multiRequest) throws Exception {
		...
		if( StringUtils.isBlank( memberVO.getMemSeq() ) ){
	    	...
	    	int updateCnt = memberService.updateCompMember( memberVO, multiRequest );
	    	retMsg = "정상적으로 (수정)처리되었습니다.!";
		}
	...
	@RequestMapping(value = "/lu/member/updateStdInfoPage.do")
	public String updateStdMemberPage(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status) throws Exception {
		...
		memberVO = memberService.getStdMember( memberVO );
		model.addAttribute("memberVO", memberVO);
		

		return "oklms/lu/member/memberStdUpdate";
	}
	
	@RequestMapping(value = "/lu/member/updateStdInfo.do")
	public String updateCompMember(@ModelAttribute("frmMember") @Valid MemberVO memberVO, BindingResult bindingResult, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status) throws Exception {
		...
		if( StringUtils.isBlank( memberVO.getMemSeq() ) ){
	    	...
	    	int updateCnt = memberService.updateCcnStdMember( memberVO );
	    	/*<select id="stdStatus" name="stdStatus">
			<option value="">선택 </option>
			<option value="1" selected="">진행중</option>
			<option value="2">이수</option>
			<option value="3">수료</option>
			<option value="4">중탈</option>
			<option value="5">참여철회</option>
			<option value="7">훈련중지</option>
			<option value="6">미이수</option>
		</select>*/
	    	if(updateCnt > 0) memberService.updateTraningStatus( memberVO );
	    	
	    	retMsg = "정상적으로 (수정)처리되었습니다.!";
		}

		redirectAttributes.addFlashAttribute("retMsg", retMsg);
		return "redirect:/lu/member/updateStdInfoPage.do?memSeq="+memberVO.getMemSeq()+"&companyId="+memberVO.getCompanyId()+"&memberType="+memberVO.getMemberType();
	}
