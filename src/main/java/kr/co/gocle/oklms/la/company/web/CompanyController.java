 ...
 *
 *******************************************************************************/
package kr.co.gocle.oklms.la.company.web;
...
public class CompanyController extends BaseController{
	...
	public String listCompany(@PathVariable("userType") String userType,@ModelAttribute("frmCompany") CompanyVO companyVO, ModelMap model) throws Exception {
	    ...
	    if(companyVO.getSessionMemType().equals("PRT")){
	    	 resultList = companyService.listCompany(companyVO);
		 } else {
			 resultList = companyService.listAllCompany(companyVO);
		 }
        ...
        if(companyVO.getSessionMemType().equals("PRT")){
        	// View호출
    		return "oklms/"+userType+"/company/companyListPrt";
        } else {
        	// View호출
	...
	//CompanyVO companyVO
	@RequestMapping(value = "/{userType}/company/listAllCompany.do")
	public String listAllCompany(@PathVariable("userType") String userType,@ModelAttribute("frmCompany") CompanyVO companyVO, ModelMap model) throws Exception {
		/*
		 * Role 따른 조회기준 변경처리
		 * -전체기업 조회
		 * 	2016AUTH0000001	관리자
		...
		List<CompanyVO> resultList = companyService.listAllCompany(companyVO);
		Integer pageSize = companyVO.getPageSize();
		Integer page = companyVO.getPageIndex();
        ...
        return "oklms/"+userType+"/company/companyListAll";
	...
	public String getCompany(@PathVariable("userType") String userType,@ModelAttribute("frmCompany") CompanyVO companyVO,  ModelMap model) throws Exception {
        ...
        if(companyVO.getSessionMemType().equals("PRT")){
        	 ...
        	 model.addAttribute("companyVO", companyVO);
        	// View호출
    		return "oklms/"+userType+"/company/companyReadPrt";
        } else if(companyVO.getSessionMemType().equals("CCN") ){
	       	 companyVO = companyService.getCompany(companyVO);
	       	 model.addAttribute("companyVO", companyVO);
	       	// View호출
	   		return "oklms/"+userType+"/company/companyReadAll";
	       } else {
        	 companyVO = companyService.getCompany(companyVO);
        	 model.addAttribute("companyVO", companyVO);
        	// View호출
	...
	//CompanyVO companyVO
		@RequestMapping(value = "/{userType}/company/listCcnCompany.do")
		public String listCcnCompany(@PathVariable("userType") String userType,@ModelAttribute("frmCompany") CompanyVO companyVO, ModelMap model) throws Exception {

			List<CompanyVO> resultList = companyService.listCcnMember(companyVO);

	        model.addAttribute("companyVO", companyVO);
	        model.addAttribute("resultList", resultList);

			// View호출
			return "oklms/"+userType+"/company/companyCcnList";
		}
		
		
		@RequestMapping(value = "/la/popup/popup/popupCcnCompany.do")
		public String goPopupReport(@ModelAttribute("frmCompany") CompanyVO companyVO, ModelMap model , HttpServletRequest request) throws Exception {
			
	  		LOG.debug("#### URL = /la/company/popupCcnCompany.do" );

	        List<CompanyVO> resultList = companyService.listCcnCompany(companyVO);
	        model.addAttribute("companyVO", companyVO);
	        model.addAttribute("resultList", resultList);

			// View호출
	  		return "oklms_popup/la/popup/popupCcn";
	  		
		}  	
		
		
		@RequestMapping(value = "/la/company/saveCcnCompany.do")
		public String saveCcnCompany(@ModelAttribute("frmCompany") CompanyVO companyVO, ModelMap model, RedirectAttributes redirectAttributes) throws Exception {
			
	  		LOG.debug("#### URL = /la/company/saveCompanyCcn.do" );
			String retMsg = "";
			int iResult = companyService.saveCcnCompany(companyVO);
			...
			redirectAttributes.addFlashAttribute("retMsg", retMsg);
			redirectAttributes.addFlashAttribute("iResult", iResult);
			redirectAttributes.addFlashAttribute("pageIndex", companyVO.getPageIndex());
			redirectAttributes.addFlashAttribute("searchKeyword", companyVO.getSearchKeyword());
	
			// View호출
	  		return "redirect:/la/popup/popup/popupCcnCompany.do";
		}  
		
		@RequestMapping(value = "/lu/company/insertExcelCompany.do")
		public String insertCompanyAllExcel(@ModelAttribute("frmCompany")  CompanyVO companyVO, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status, MultipartHttpServletRequest multiRequest) throws Exception {
			
			String retMsg = "";
			
			 String result = companyService.insertCompanyAllExcel(companyVO, multiRequest);
			 
			 if(result == "SUCCESS"){
				 retMsg = "정상적으로 일괄업로드 처리되었습니다.!";
			 }else {
				 retMsg = "기업 일괄등록시 에러가 발생하였습니다.!";
			 }
		     
			redirectAttributes.addFlashAttribute("retMsg", retMsg);
			
			return "redirect:/lu/company/listCompany.do";
		}
		
		@RequestMapping(value = "/lu/company/insertExcelMemberCompany.do")
		public String insertExcelMemberCompany(@ModelAttribute("frmCompany")  CompanyVO companyVO, ModelMap model, RedirectAttributes redirectAttributes, SessionStatus status, MultipartHttpServletRequest multiRequest) throws Exception {
			
			String retMsg = "";
			
			 String result = companyService.insertExcelMemberCompany(companyVO, multiRequest);
			 
			 if(result == "SUCCESS"){
				 retMsg = "정상적으로 일괄업로드 처리되었습니다.!";
			 }else {
				 retMsg = "일괄등록시 에러가 발생하였습니다.!";
			 }
		     
			redirectAttributes.addFlashAttribute("retMsg", retMsg);
			
			return "redirect:/lu/member/listExcelCompanyMember.do";
		}
}
