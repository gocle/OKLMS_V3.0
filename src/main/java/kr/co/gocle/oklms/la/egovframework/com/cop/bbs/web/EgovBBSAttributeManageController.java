package kr.co.gocle.oklms.la.egovframework.com.cop.bbs.web;
...
public class EgovBBSAttributeManageController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/addBBSMaster.do")
	public String addBBSMaster(@ModelAttribute("searchVO") BoardMasterVO boardMasterVO, ModelMap model) throws Exception {
		...
		return "oklms/la/egovframework/com/cop/bbs/EgovBoardMstrRegist";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/insertBBSMasterInf.do")
	public String insertBBSMasterInf(@ModelAttribute("searchVO") BoardMasterVO boardMasterVO,
		...
		if (isAuthenticated == true) {
			...
			//게시판마스터 테이블에 생성
			String bbsId = "";
			bbsId = bbsAttrbService.insertBBSMastetInf(boardMaster);
			//게시판활용 테이블에 생성
			if(!"".equals(bbsId)){
				boardUseInf.setBbsId(bbsId);
				boardUseInf.setUseAt("Y");
				boardUseInf.setFrstRegisterId(user.getUniqId());
				boardUseInf.setRegistSeCode("REGC01");
				boardUseInf.setTrgetId("SYSTEM_DEFAULT_BOARD");
				
				bbsUseService.insertBBSUseInf(boardUseInf);
			}
			model.addAttribute("retMsg", "게시판 사용조건\\n 1.생성한 게시판을 메뉴에 등록.\\n 2.권한 그룹에 메뉴 접근 권한 추가.");
		} else {
			model.addAttribute("retMsg", "게시판 생성하기위한 회원 세션정보가 없습니다.\\n 관리자에게 문의하세요.");
		}

		return "forward:/la/cop/bbs/selectBBSMasterInfs.do";
	}
	...
	@RequestMapping("/la/cop/bbs/selectBBSMasterInfs.do")
	public String selectBBSMasterInfs(@ModelAttribute("searchVO") BoardMasterVO boardMasterVO, ModelMap model) throws Exception {
		...
		return "oklms/la/egovframework/com/cop/bbs/EgovBoardMstrList";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/selectBBSMasterInf.do")
	public String selectBBSMasterInf(@ModelAttribute("searchVO") BoardMasterVO searchVO, ModelMap model) throws Exception {
		...
		return "oklms/la/egovframework/com/cop/bbs/EgovBoardMstrUpdt";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/updateBBSMasterInf.do")
	public String updateBBSMasterInf(@ModelAttribute("searchVO") BoardMasterVO boardMasterVO,
		...
		if (isAuthenticated == true) {
			...
			model.addAttribute("retMsg", "정상 저장되었습니다.");
		} else {
			model.addAttribute("retMsg", "게시판 사용하기위한 회원 세션정보가 없습니다.\\n 관리자에게 문의하세요.");
		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/deleteBBSMasterInf.do")
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/la/cop/bbs/selectBBSMasterInfsPop.do")
	public String selectBBSMasterInfsPop(@ModelAttribute("searchVO") BoardMasterVO boardMasterVO, ModelMap model)
		...
		return "oklms/la/egovframework/com/cop/bbs/EgovBoardMstrListPop";
	}
}
