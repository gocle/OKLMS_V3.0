...
public class TraningServiceImpl extends EgovAbstractServiceImpl implements TraningService{
	...
	public List<TraningVO> listTraningProcess(TraningVO traningVO) throws Exception {
  	...
  	@Override
	public TraningVO getTraningProcessInfo(TraningVO traningVO) throws Exception {
  		TraningVO data = traningMapper.getTraningProcessInfo(traningVO);
		return data;
	}
	...
	public int getTdOffJtTotalCnt(TraningVO traningVO) throws Exception {
  	...
  	@Override
	public int saveTraningProcessManage(TraningVO traningVO) throws Exception {
		return traningMapper.saveTraningProcessManage(traningVO);
	}
	...
	@Override
	public String insertTraningBatchScheduleExcel(TraningScheduleVO traningScheduleVO, MultipartHttpServletRequest multiRequest, HttpServletRequest request) throws Exception {
		int intWeekCnt1 = 0; //1주차
		int intWeekCnt2 = 0; //2주차
		int intWeekCnt3 = 0; //3주차
		int intWeekCnt4 = 0; //4주차
		...
		if (!mfile.isEmpty() ) {
			...
			for (int i = 0; i < dataSet.size(); i++) {
				...
				//주차
				String subjectCode = "";
				subjectCode = row.getColumn("교과목코드").trim();
				if ("".equals(subjectCode)) {
					throw new Exception("교과목코드 항목에 값이 없습니다 ");
				}

				//훈련일자
				String subClass = "";
				subClass = row.getColumn("분반코드").trim();
				if ("".equals(subClass)) {
					throw new Exception("분반코드 항목에 값이 없습니다 ");
				}
			...
			/*	//교육-시작-분
				...
				traningSchVO.setYyyy(traningScheduleVO.getYyyy());
				traningSchVO.setTerm(traningScheduleVO.getTerm());
				traningSchVO.setSubjectCode(subjectCode);
				traningSchVO.setSubClass(subClass);
				traningSchVO.setSubjctTitle("최초등록");
				traningSchVO.setRetunReason("훈련시간표 일괄 신규등록");
				traningSchVO.setWeekCnt(weekCnt);
				traningSchVO.setTraningDate(traningDate);
				traningSchVO.setTraningStHour(traningStHour);
				traningSchVO.setTraningStMin("00");
				...
				// 시간표가 등록되어 있는 교과있는지 확인
				int checkSubjectCnt = traningMapper.getTraningSubjectCnt(traningSchVO);
				
				System.out.println("##### checkSubjectCnt="+checkSubjectCnt);

				// 해당 교과가 있을경우 진행
				if(checkSubjectCnt > 0){
					
					//int checkWeekCnt = traningMapper.getTraningSubjectScheduleCnt(traningSchVO);
					
					String subjWeekId = StringUtils.defaultString( traningMapper.getTraningSubjectWeekId(traningSchVO) , "" );
					String useWeekId  = StringUtils.defaultString( traningMapper.getTraningSubjectScheduleWeekId(traningSchVO) , "" );
					
					//=========== ojt 교과인즈 검증필요
					
					// 사용중인 주차아이디가 있다면 주차ID 세팅
					if(!"".equals(subjWeekId)){
						traningSchVO.setNewWeekId(subjWeekId);
					}
					
					// 사용중인 주차아이디가 있다면 주차ID 세팅
					if(!"".equals(useWeekId)){
						traningSchVO.setWeekId(useWeekId);
					}
					
					egovLogger.debug("========================  useWeekId :  "+useWeekId);
					
					//훈련시간표-주차정보 저장
					if("1".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt1;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
						...
						//================  실제 주차정보 등록 ==================
						newWeekId = subjWeekIdGnrService.getNextStringId();
						traningSchVO.setNewWeekId(newWeekId);
						sqlResultInt1 += traningMapper.insertSubjWeekBatch(traningSchVO);
						
					} else if("2".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt2;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("3".equals(weekCnt) &&  "".equals(useWeekId) ){
						++intWeekCnt3;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("4".equals(weekCnt) &&  "".equals(useWeekId) ){
						++intWeekCnt4;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("5".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt5;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("6".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt6;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("7".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt7;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("8".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt8;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("9".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt9;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("10".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt10;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("11".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt11;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("12".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt12;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("13".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt13;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("14".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt14;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					} else if("15".equals(weekCnt) && "".equals(useWeekId) && "".equals(subjWeekId) ){
						++intWeekCnt15;
						weekId = traningSubjWeekSchIdGnrService.getNextStringId();
						traningSchVO.setWeekId(weekId);
						System.out.println("##### weekId="+weekId);
					...
					if( sqlResultInt1 > 0 ){
						...
						//================  실제 주차별 시간표 등록 ==================
						newTimeId = subjWeekTimeIdGnrService.getNextStringId();
						traningSchVO.setNewTimeId(newTimeId);
						sqlResultInt2 += traningMapper.insertSubjWeekTimeBatch(traningSchVO);
					...
					if("SUCCESS".equals(sqlResult)){
						...
						//1.주차별 NCS단위 테이블에 시간아이디를 주차별시간 테이블에서 시간아이디 항목을 가지고와서 업데이트한다.
						//2.주차별 NCS요소 테이블에 시간아이디를 주차별시간 테이블에서 시간아이디 항목을 가지고와서 업데이트한다.
						traningMapper.updateTraningSubjWeekSchStatus(traningSchVO);
					}
	...
	public int deleteTraningSchedule(TraningScheduleVO traningScheduleVO) throws Exception {
		...
		sqlResultInt += traningMapper.deletetTraningScheduleWeek(traningScheduleVO);
		sqlResultInt += traningMapper.deletetTraningScheduleWeekTime(traningScheduleVO);
		
		return sqlResultInt;
	}
	
	@Override
	public int getTraningScheduleCnt(TraningScheduleVO traningScheduleVO) throws Exception {
		int data = traningMapper.getTraningScheduleCnt(traningScheduleVO);
		return data;
	}
	
	@Override
	public int getTraningScheduleUseCnt(TraningScheduleVO traningScheduleVO) throws Exception {
		int data = traningMapper.getTraningScheduleUseCnt(traningScheduleVO);
		return data;
	}
	
	@Override
  	public List<TraningVO> listTraningPrt(TraningVO traningVO) throws Exception {
		List<TraningVO> data = traningMapper.listTraningPrt(traningVO);
		return data;
	}
	
	@Override
  	public List<TraningVO> listTraningCcn(TraningVO traningVO) throws Exception {
		List<TraningVO> data = traningMapper.listTraningCcn(traningVO);
		return data;
	}
	
	@Override
  	public List<TraningVO> listTraningSubjectPrt(TraningVO traningVO) throws Exception {
		List<TraningVO> data = traningMapper.listTraningSubjectPrt(traningVO);
		return data;
	}
	
	@Override
	public int updateTraningPrt(TraningVO traningVO) throws Exception {
		
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.putSessionToVo(traningVO);
		int sqlResultInt = 0;
		
		System.out.println("================= traningVO.getTraningProcessManageIds().length : "+traningVO.getTraningProcessManageIds().length);
		
		for( int i = 0; i < traningVO.getTraningProcessManageIds().length; i++ ){
			traningVO.setTraningProcessManageId(traningVO.getTraningProcessManageIds()[i]);
			traningVO.setPrtGrade(traningVO.getPrtGrades()[i]);
			traningVO.setPrtBigo(traningVO.getPrtBigos()[i]);
			sqlResultInt += traningMapper.updateTraningPrt(traningVO);
		}
	...
	@Override
	public int saveTraningPrt(TraningVO traningVO) throws Exception {
		
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.putSessionToVo(traningVO);
		int sqlResultInt = 0;
		
		for( int i = 0; i < traningVO.getTraningProcessManageIdss().length; i++ ){
			traningVO.setTraningProcessManageId(traningVO.getTraningProcessManageIdss()[i]);
			traningVO.setPrtGrade(traningVO.getPrtGrades()[i]);
			traningVO.setPrtBigo(traningVO.getPrtBigos()[i]);
			sqlResultInt += traningMapper.updateTraningPrt(traningVO);
		}
	...
	@Override
	public int updateTraningCcn(TraningVO traningVO) throws Exception {
		...
		for( int i = 0; i < traningVO.getTraningProcessManageIds().length; i++ ){
			traningVO.setTraningProcessManageId(traningVO.getTraningProcessManageIds()[i]);
			traningVO.setCcnGrade(traningVO.getCcnGrades()[i]);
			traningVO.setCcnBigo(traningVO.getCcnBigos()[i]);
			sqlResultInt += traningMapper.updateTraningCcn(traningVO);
		}
	...
	@Override
	public int saveTraningCcn(TraningVO traningVO) throws Exception {
