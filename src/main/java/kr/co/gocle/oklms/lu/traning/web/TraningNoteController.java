...
public class TraningNoteController extends BaseController{
	...
	public String listTraningNote(@ModelAttribute("frmTraning") TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session  ) throws Exception {
		...
		if(subjectTraningType!=null && subjectTraningType.equals("OJT")){
			...
			for(int i = 0; i < timeList.size(); i++){
				...
				resultList = traningNoteSerivce.listTraningRegularClassMember(traningNoteVO);			
			...
			String offTimeHour = "";
			if(traningNoteSerivce.getTraningRegularTime(traningNoteVO)!=null){
				offTimeHour = traningNoteSerivce.getTraningRegularTime(traningNoteVO).getLeadTime();
			}
			model.put("offTimeHour", offTimeHour);	
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/lu/traning/listTraningNoteCdp.do")
	public String listTraningNoteCdp(@ModelAttribute("frmTraning") TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session  ) throws Exception {
		...
		/*if(subjectTraningType!=null && subjectTraningType.equals("OJT")){
			...
			return "oklms/lu/traning/listTraningNoteCdp";
		//}
	}
	...
	public String goInsertTraningNote(@ModelAttribute("frmTraning")TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session,  RedirectAttributes redirectAttributes ) throws Exception {
		...
		*/
		...
		traningNoteVO.setTraningType( StringUtils.defaultIfBlank( traningNoteVO.getTraningType() , (String)session.getAttribute(Globals.SUBJECT_TRANING_TYPE )) );
		...
		if(currProcVO.getSubjectTraningType().equals("OJT")){
			...
			if(currProcVO.getSubjectType().equals("HSKILL")){ // OJT 고숙련일 경우
				insertCnt  = traningNoteSerivce.goInsertOjtTraningNoteDetail(traningNoteVO);
			} else { // 일반 OJT
		...
		// 고숙련 학생 등록
		if(traningNoteVO.getSessionMemType()!=null &&traningNoteVO.getSessionMemType().equals("STD")){
			return "redirect:/lu/traning/listTraningNoteStd.do?weekCnt="+traningNoteVO.getWeekCnt();
		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/lu/traning/saveTraningNoteTime.do")
	public String saveTraningNoteTime(@ModelAttribute("frmTime")TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session,  RedirectAttributes redirectAttributes ) throws Exception {

		LOGGER.debug("#### URL = /lu/traning/saveTraningNoteTime.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(traningNoteVO); // session의 정보를 VO에 추가.
		...
		*/
		...
		insertCnt  = traningNoteSerivce.saveTraningNoteTime(traningNoteVO);
		...
		if( result.getSubjectTraningType().equals("OJT") && result.getSubjectType().equals("HSKILL") ){
			return "redirect:/lu/traning/listTraningNoteStd.do?weekCnt="+traningNoteVO.getWeekCnt()+"&classId="+traningNoteVO.getClassId();
		} else {
			return "redirect:/lu/traning/listTraningNoteOt.do?weekCnt="+traningNoteVO.getWeekCnt()+"&classId="+traningNoteVO.getClassId();
		}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/lu/traning/updateTraningNoteTime.do")
	public String updateTraningNoteTime(@ModelAttribute("frmSch")TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session,  RedirectAttributes redirectAttributes ) throws Exception {

		LOGGER.debug("#### URL = /lu/traning/updateTraningNoteTime.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(traningNoteVO); // session의 정보를 VO에 추가.
		...
		*/
		...
		insertCnt  = traningNoteSerivce.updateTraningNoteTime(traningNoteVO);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/lu/traning/deleteTraningNoteTime.do")
	public String deleteTraningNoteTime(@ModelAttribute("frmSch")TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session,  RedirectAttributes redirectAttributes ) throws Exception {

		LOGGER.debug("#### URL = /lu/traning/deleteTraningNoteTime.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(traningNoteVO); // session의 정보를 VO에 추가.
		...
		*/
		...
		insertCnt  = traningNoteSerivce.deleteTraningNoteTime(traningNoteVO);
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/lu/traning/saveTraningNote.do")
	public String saveTraningNote(@ModelAttribute("frmTraning")TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session,  RedirectAttributes redirectAttributes ) throws Exception {

		LOGGER.debug("#### URL = /lu/traning/saveTraningNote.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(traningNoteVO); // session의 정보를 VO에 추가.
		...
		*/
		...
		insertCnt  = traningNoteSerivce.applovalOjtTraningNoteOne(traningNoteVO);
	...
	public String listTraningNoteStd(@ModelAttribute("frmTraning") TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session ) throws Exception {
		...
		*/
		...
		if(result == null){
			result = traningNoteSerivce.getSubject(traningNoteVO);
			model.put("TraningNoteVO", result);
		}
	
		if( result.getSubjectTraningType().equals("OJT") && result.getSubjectType().equals("HSKILL") ){
			
			timeList =traningNoteSerivce.listHsTraningRegularTime(traningNoteVO);
			model.put("timeList", timeList);
			
			
			AunuriLinkSubjectWeekNcsVO aunuriLinkSubjectWeekNcsVO = new AunuriLinkSubjectWeekNcsVO();
			
			aunuriLinkSubjectWeekNcsVO.setYyyy(currProcVO.getYyyy());
			aunuriLinkSubjectWeekNcsVO.setTerm(currProcVO.getTerm());
			aunuriLinkSubjectWeekNcsVO.setSubjectCode(currProcVO.getSubjectCode());
			aunuriLinkSubjectWeekNcsVO.setSubjectClass(currProcVO.getSubClass());
			List<AunuriLinkSubjectWeekNcsVO> nscList = aunuriLinkService.listAunuriWeekNcsUnit(aunuriLinkSubjectWeekNcsVO);
			model.put("nscList", nscList);
			
			//return "oklms/lu/traning/listTraningNoteOtHsStd_20200325";
			return "oklms/lu/traning/listTraningNoteOtHsStd";
		} else {
...
//		model.put("TraningEnrichmentTimeVO",resultWnrich);
	...
	@RequestMapping(value = "/lu/traning/excelTraningNoteOt.do")
	public String excelTraningNoteOt(@ModelAttribute("frmTraning") TraningNoteVO  traningNoteVO, ModelMap model, HttpSession session, HttpServletRequest request ,  RedirectAttributes redirectAttributes ) throws Exception {

		LOGGER.debug("#### URL = /lu/traning/excelTraningNoteOt.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(traningNoteVO); // session의 정보를 VO에 추가.
		/*
		...
		*/
		...
		traningNoteVO.setTerm(StringUtils.defaultIfBlank( traningNoteVO.getTerm() ,(String)session.getAttribute(Globals.TERM)));  
		traningNoteVO.setSubjectCode(StringUtils.defaultIfBlank( traningNoteVO.getSubjectCode() ,(String)session.getAttribute(Globals.SUBJECT_CODE)));
		traningNoteVO.setSubjectClass(StringUtils.defaultIfBlank( traningNoteVO.getClassId() ,(String)session.getAttribute(Globals.CLASS)));
		
		TraningNoteVO result = traningNoteSerivce.getMonthTraningNoteOt(traningNoteVO);
		List<TraningNoteVO> resultList = traningNoteSerivce.listMonthTraningNoteOt(traningNoteVO);
		
		//공통상단 데이터
		TraningNoteVO traning = traningNoteSerivce.getTraningNote(traningNoteVO);
		
		if(result == null){
			String retMsg = "출력 내용이 없습니다.";
			redirectAttributes.addFlashAttribute("retMsg", retMsg);
			
			
			if( traning.getSubjectTraningType().equals("OJT") && traning.getSubjectType().equals("HSKILL") ){
				return "redirect:/lu/traning/listTraningNoteStd.do?weekCnt="+traningNoteVO.getWeekCnt()+"&classId="+traningNoteVO.getClassId();
			} else {
				return "redirect:/lu/traning/listTraningNoteOt.do?weekCnt="+traningNoteVO.getWeekCnt()+"&classId="+traningNoteVO.getClassId();
			}
		...
		request.setAttribute("ExcelName", URLEncoder.encode( result.getSubjectName()+"_"+traningNoteVO.getSearchMonth()+"월 훈련일지".replaceAll(" ", "_") ,"UTF-8") );
		...
		return "oklms_excel/lu/traning/excelTraningNoteMonthOt";
