package kr.co.gocle.oklms.lu.today.web;
...
import kr.co.gocle.oklms.lu.today.service.TodayService; 
import kr.co.gocle.oklms.lu.today.vo.TodayVO;
...
public class TodayController  extends BaseController{
	...
	public String lmsUserTodayPage(HttpServletRequest request, HttpServletResponse response, @RequestParam Map<String, Object> commandMap, ModelMap model) throws Exception {
		...
		if(loginVO.getMemType().equals("STD")){
			...
			//QNA현황
			List<BoardVO> listQnAPrt =  todayService.listQnAPrt (todayVO);
			model.addAttribute("listQnAPrt", listQnAPrt);
			...
			//주차별 학습활동서 미제출건 
			//List<TodayVO> listWeekActivityNoteCompany =  todayService.listWeekActivityNoteCompany (todayVO);
			//model.addAttribute("listWeekActivityNoteCompany", listWeekActivityNoteCompany);			
