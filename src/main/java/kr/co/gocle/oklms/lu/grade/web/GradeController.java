package kr.co.gocle.oklms.lu.grade.web;
...
public class GradeController  extends BaseController{
	...
	public String listGradeCdp(@ModelAttribute("frmGrade") GradeVO gradeVO,HttpSession session, ModelMap model, HttpServletRequest request) throws Exception {
		...
		if(gradeVO.getYyyy()==null || gradeVO.getYyyy().equals("")){
			...
			gradeVO.setYyyy(StringUtils.defaultString(commonCodeVO.getYyyy(),""));				
			gradeVO.setTerm(StringUtils.defaultString(commonCodeVO.getTerm(),""));
			gradeVO.setSearch("top");
		}
		
		List <WorkCertVO> listMyDept = workCertService.listMyDept(gradeVO.getSessionMemSeq());
		model.addAttribute("listMyDept", listMyDept);
		
		
		// 학과코드
		gradeVO.setDeptNo(StringUtils.defaultString(gradeVO.getDeptNo(),listMyDept.get(0).getDeptNo()));
		
		if(gradeVO.getDeptNo().equals("")){
			gradeVO.setDeptNo(listMyDept.get(0).getDeptNo());
		}
	   
		logger.debug("@@@@@@@@@@@@@@@@@ gradeVO.getDeptNo() : "+gradeVO.getDeptNo() );
		logger.debug("@@@@@@@@@@@@@@@@@ listMyDept.get(0).getDeptNo() : "+listMyDept.get(0).getDeptNo() );
		
		
		// 세션
		session.setAttribute(Globals.DEPT_NO  ,gradeVO.getDeptNo()); 
		gradeVO.setSessionDeptNo(gradeVO.getDeptNo());
		
		logger.debug("@@@@@@@@@@@@@@@@@ gradeVO.getDeptNo() : "+gradeVO.getDeptNo() );
		logger.debug("@@@@@@@@@@@@@@@@@ gradeVO.getSessionDeptNo() : "+gradeVO.getSessionDeptNo() );
	...
	public String insertSendGradeCdp(@ModelAttribute("frmGrade") GradeVO gradeVO, ModelMap model, RedirectAttributes redirectAttributes) throws Exception {
		...
		logger.debug("========================= gradeVO.getSubmitComment : "+gradeVO.getSubmitComment() );
