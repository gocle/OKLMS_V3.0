package kr.co.gocle.oklms.lu.activity.web;
...
public class ActivityController  extends BaseController{
	...
	public String listActivityStd(@ModelAttribute("frmActivity") ActivityVO activityVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		List<ActivityVO> resultMember = activityService.getMyActivity(activityVO);
	...
	@RequestMapping(value = "/lu/activity/excelActivityStd.do")
	public String excelTraningNoteOt(@ModelAttribute("frmActivity") ActivityVO  activityVO, ModelMap model, HttpSession session, HttpServletRequest request ,  RedirectAttributes redirectAttributes ) throws Exception {

		LOG.debug("#### URL = /lu/activity/excelActivityStd.do" );
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(activityVO); // session의 정보를 VO에 추가.
		
		activityVO.setYyyy(StringUtils.defaultIfBlank( activityVO.getYyyy() ,(String)session.getAttribute(Globals.YYYY)));
		activityVO.setTerm(StringUtils.defaultIfBlank( activityVO.getTerm() ,(String)session.getAttribute(Globals.TERM)));  
		activityVO.setSubjectCode(StringUtils.defaultIfBlank( activityVO.getSubjectCode() ,(String)session.getAttribute(Globals.SUBJECT_CODE)));
		activityVO.setSubjectClass(StringUtils.defaultIfBlank( activityVO.getClassId() ,(String)session.getAttribute(Globals.CLASS)));
		
		List<ActivityVO> resultList = activityService.listMonthActivityStd(activityVO);
		
		if(resultList == null || resultList.size() == 0){
			String retMsg = "출력 내용이 없습니다.";
			redirectAttributes.addFlashAttribute("retMsg", retMsg);
			return "redirect:/lu/activity/listActivityStd.do?weekCnt="+activityVO.getWeekCnt()+"&classId="+activityVO.getClassId();
		}
		
		request.setAttribute("ExcelName", URLEncoder.encode( resultList.get(0).getSubjectName()+"_"+activityVO.getSearchMonth()+"월  학습활동서".replaceAll(" ", "_") ,"UTF-8") );
		
		
	     model.addAttribute("resultList", resultList);
		

		return "oklms_excel/lu/activity/excelMonthActivityStd";
 
	}	
  	
  	@RequestMapping(value = "/lu/activity/saveActivity.do")
	public String saveActivity(@ModelAttribute("frmActivity")  ActivityVO activityVO, ModelMap model,RedirectAttributes redirectAttributes) throws Exception {
  		LOG.debug("#### URL = /lu/activity/saveActivity.do" );
		//세션자동복사
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(activityVO); // session의 정보를 VO에 추가.
		...
		// View호출
  		return "redirect:/lu/activity/listActivityPrt.do?weekCnt="+activityVO.getWeekCnt()+"&searchId="+activityVO.getSearchId();
	}  	
...
// OJT과정			
			...
			if(companylist!=null && companylist.size()>0){
				if(classId_temp==null|| classId_temp.equals("")){				
					if ( (String)httpSession.getAttribute(Globals.CLASS) == null ){
			...
			memberVO.setSearchName(activityVO.getSearchName());
			memberVO.setSearchId(activityVO.getSearchId());
			
			List<MemberVO> memberlist = activityService.listActivityOjtMember(memberVO);
			model.addAttribute("memberlist", memberlist);
			
			List<MemberVO> lessonList = activityService.listLessonMember(memberVO);
 ...
 // 담당교수
   	@RequestMapping(value = "/lu/activity/listActivityCdp.do")
 	public String listActivityCdp(@ModelAttribute("frmActivity") ActivityVO activityVO, ModelMap model, HttpServletRequest request) throws Exception {
   		LOG.debug("#### URL = /lu/activity/listActivityPrt.do" );
 ...
 // OFF 과정			
 		...
 		model.addAttribute("memberlist", memberlist);
   			...
   			// View호출
 	  		return "oklms/lu/activity/listActivityCdpOff";	
 ...
 // OJT과정			
	...
	public String listActivityHrd(@ModelAttribute("frmActivity") ActivityVO activityVO, ModelMap model) throws Exception {
		...
		// =================   조회 기업체 세팅 ==================
  		if( activityVO.getBaseCheckCompanyIds() != null && activityVO.getBaseCheckCompanyIds().indexOf(",") != -1 ){
  			interviewCompanyVO.setBaseSearchCompanyIds(CommonUtil.toDataStr(activityVO.getBaseCheckCompanyIds()));
  		}
  		// =================  조회 기업체 세팅 ==================
	...
	public String listActivityOt(@ModelAttribute("frmActivity") ActivityVO activityVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		// 과정정보
  		CurrProcVO currProcVO=new CurrProcVO();
		currProcVO.setYyyy(StringUtils.defaultIfBlank( activityVO.getYyyy() , (String)httpSession.getAttribute(Globals.YYYY)) );
		currProcVO.setTerm(StringUtils.defaultIfBlank( activityVO.getTerm() , (String)httpSession.getAttribute(Globals.TERM)) );
		currProcVO.setSubClass(StringUtils.defaultIfBlank( activityVO.getClassId() , (String)httpSession.getAttribute(Globals.CLASS)) );
		currProcVO.setSubjectCode(StringUtils.defaultIfBlank( activityVO.getSubjectCode() , (String)httpSession.getAttribute(Globals.SUBJECT_CODE)) );
		currProcVO = reportService.getCurrproc(currProcVO);
...
// OJT과정			
			...
			if(subjweekStdVO != null){
				if(subjweekStdVO.getSubjectTraningType().equals("OJT")){
					List<MemberVO> memberlist = activityService.listActivityOjtMember(memberVO);
					model.addAttribute("memberlist", memberlist);
				} else {
					List<MemberVO> memberlist = activityService.listActivityMember(memberVO);
