package kr.co.gocle.oklms.lu.interview.web;
...
public class InterviewController  extends BaseController{
	...
	public String listInterviewCenter(@ModelAttribute("frmInterview") InterviewVO interviewVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		// =================   조회 기업체 세팅 ==================
  		if( interviewVO.getBaseCheckCompanyIds() != null && interviewVO.getBaseCheckCompanyIds().indexOf(",") != -1 ){
  			interviewCompanyVO.setBaseSearchCompanyIds(CommonUtil.toDataStr(interviewVO.getBaseCheckCompanyIds()));
  		}
		...
		commandMap.put("companyId", interviewVO.getCompanyId());
		commandMap.put("traningProcessId", interviewVO.getTraningProcessId());
	...
	public String getInterviewCenter(@ModelAttribute("frmInterview") InterviewVO interviewVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		String sendYn = interviewService.getInterviewCenterSendYn(interviewVO);
		String preMonth = interviewService.getInterviewCenterPreMonth(interviewVO);
		String nxtMonth = interviewService.getInterviewCenterNxtMonth(interviewVO);
		
		if(interviewVO.getSendYn() == null){
			interviewVO.setSendYn(sendYn);
		}
		
		model.addAttribute("preMonth", preMonth );		
		model.addAttribute("nxtMonth", nxtMonth );
	...
	public String ajaxInterviewCompany(@ModelAttribute("frmInterview") InterviewCompanyVO interviewCompanyVO, ModelMap model, HttpServletRequest request) throws Exception {
  	...
  	/** 기업체조회 */
  	@RequestMapping(value = "/lu/interview/ajaxInterviewCompanyTraning.do")
	public String ajaxInterviewCompanyTraning(@ModelAttribute("frmInterview") InterviewCompanyVO interviewCompanyVO, ModelMap model, HttpServletRequest request) throws Exception {
  		LOG.debug("#### URL = /lu/interview/ajaxInterviewCompanyTraning.do" );
		//세션자동복사
		LoginInfo loginInfo = new LoginInfo();
		loginInfo.getLoginInfo();
		loginInfo.putSessionToVo(interviewCompanyVO); // session의 정보를 VO에 추가.
		
		List<InterviewCompanyVO> result=interviewService.InterviewTraningCompanys(interviewCompanyVO);
    	...
    	*//** paging *//*
		...
		// View호출
		return "oklms_import/lu/interview/ajaxInterviewCompanyTraning";
	}
}
