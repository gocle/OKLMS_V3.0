package kr.co.gocle.oklms.mm.traning.web;
...
public class TraningMMController  extends BaseController {
	...
	public String listTraning(@ModelAttribute("frmTraning")  TraningNoteVO  traningNoteVO, ModelMap model, HttpServletRequest request) throws Exception {
		...
		if("STD".equals(loginInfo.getMemType())){ //학습근로자 개설교과
			...
			// 로그인권한별 URL 분기처리
			retView = "oklms/mm/traning/listTraningPrt";
...
*/			
		...
		if(currProcVO != null && currProcVO.getSubjectTraningType().equals("OFF")){
			String offTimeHour =  traningNoteSerivce.getTraningRegularTime(traningNoteVO).getLeadTime();
