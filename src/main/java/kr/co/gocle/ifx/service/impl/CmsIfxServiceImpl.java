...
import kr.co.gocle.ifx.vo.CmsCourseCodeVO;
import kr.co.gocle.ifx.vo.CmsCourseContentItemItemListVO;
import kr.co.gocle.ifx.vo.CmsCourseContentItemLessonVO;
import kr.co.gocle.ifx.vo.CmsCourseContentItemListVO;
import kr.co.gocle.ifx.vo.CmsCourseContentItemSubItemListVO;
import kr.co.gocle.ifx.vo.CmsCourseContentPropertiesVO;
import kr.co.gocle.ifx.vo.CmsCourseContentVO;
import kr.co.gocle.oklms.comm.util.StringUtil;
import kr.co.gocle.oklms.lu.online.web.OnlineTraningController;
...
//		        System.out.println("cmsCourseContentItemListVO:"+cmsCourseContentItemListVO.toString());
	...
	public HashMap<String, String> viewLesson(CmsCourseBaseVO cmsCourseBaseVO) throws Exception{
		...
		try {
		    ...
		    for (int i = 0; i < jsonArray.size(); i++) {
		        ...
		        if(f0Obj.get("title") != null ) title = f0Obj.get("title").toString();
		        if(f0Obj.get("required_learning_time_in_seconds") != null ) required_learning_time_in_seconds = f0Obj.get("required_learning_time_in_seconds").toString();
		        if(f0Obj.get("display_order") != null ) display_order = f0Obj.get("display_order").toString();
      	        ...
      	        if(subitem_list != null && subitem_list.isJsonArray()){
	  				...
	  				for (int y = 0; y < jsonArray2.size(); y++) {
	  			       ...
	  			       if(f1Obj.get("title") != null ) title = f1Obj.get("title").toString();
	  			        if(f1Obj.get("required_learning_time_in_seconds") != null ) required_learning_time_in_seconds = f1Obj.get("required_learning_time_in_seconds").toString();
	  			        if(f1Obj.get("display_order") != null ) display_order = f1Obj.get("display_order").toString();
	  					...
	  					if(cmsCourseBaseVO.getLessonSubItemId().equals(f1Obj.get("id").toString())){
	  						...
	  						if(body2 != null && body2.isJsonArray()){
	  			  				...
	  			  				for (int z = 0; z < jsonArray3.size(); z++) {
		  		  				  ...
		  		  				  if(f2Obj.get("title") != null ) title = f2Obj.get("title").toString();
		  		  				    if(f2Obj.get("required_learning_time_in_seconds") != null ) required_learning_time_in_seconds = f2Obj.get("required_learning_time_in_seconds").toString();
		  		  				    if(f2Obj.get("display_order") != null ) display_order = f2Obj.get("display_order").toString();
			  		  				...
			  		  				retMap.put("display_order", display_order);
		  		  					retMap.put("title", title.replaceAll("\"", ""));
		  		  					retMap.put("required_learning_time_in_seconds", required_learning_time_in_seconds);
	...
	@Override
	public int initCourseContentSummary(CmsCourseBaseVO cmsCourseBaseVO) throws Exception {
		int iResult = 0;
		
		iResult = cmsIfxMapper.deleteCourseContentSummary();
